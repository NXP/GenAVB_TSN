...
int genavb_control_open(struct genavb_handle const *genavb, struct genavb_control_handle **control, genavb_control_id_t id)
{
	...
	*control = os_malloc(sizeof(struct genavb_control_handle));
	if (!*control) {
		rc = (-GENAVB_ERR_NO_MEMORY);
		goto err_alloc;
	}
	os_memset((*control), 0, sizeof(struct genavb_control_handle));

	(*control)->id = id;

	rc = control_init(*control, id);
	if (rc < 0)
		goto err_control_init;
	...
	if (ipc_id[id][CTRL_RX_SYNC] != IPC_ID_NONE) {
		if (ipc_rx_init(&(*control)->rx_sync, ipc_id[id][CTRL_RX_SYNC], NULL, (unsigned long)control_priv_data(*control)) < 0) {
			rc = (-GENAVB_ERR_CTRL_INIT);
...
static int avb_control_valid(struct genavb_control_handle const *handle, genavb_msg_type_t msg_type, void const *msg, unsigned int msg_len)
{
	...
	case GENAVB_CTRL_AVDECC_MEDIA_STACK:
		...
		case GENAVB_MSG_MEDIA_STACK_PERSISTENT_PARAM:
			if (msg_len == sizeof(struct genavb_msg_media_stack_persistent_param))
				rc = 1;
			break;
		case GENAVB_MSG_MEDIA_STACK_MEMORY_OBJECT:
			if (msg_len == sizeof(struct genavb_msg_media_stack_memory_object))
				rc = 1;
			break;
		case GENAVB_MSG_MEDIA_STACK_ENTITY_START:
			if (msg_len == sizeof(struct genavb_msg_media_stack_start))
				rc = 1;
			break;
		case GENAVB_MSG_MEDIA_STACK_ENTITY_IDENTIFY:
			if (msg_len == sizeof(struct genavb_msg_media_stack_identify))
				rc = 1;
			break;
		default:
			break;
	...
	case GENAVB_CTRL_AVDECC_CONTROLLED:
		...
		{
			unsigned int aecp_msg_offset = offsetof(struct genavb_aecp_msg, buf);

			if (msg_len < aecp_msg_offset)
				break;
			...
			case AECP_AEM_RESPONSE:
			{
				...
				avb_u16 response_type;

				if (msg_len < (aecp_msg_offset + sizeof(struct aecp_aem_pdu)))
					break;

				pdu = (struct aecp_aem_pdu *)((struct genavb_aecp_msg *)msg)->buf;
				response_type = AECP_AEM_GET_CMD_TYPE(pdu);
			...
			{
				if (msg_len < (aecp_msg_offset + sizeof(struct aecp_addr_access_pdu)))
					break;
	...
	case GENAVB_CTRL_AVDECC_CONTROLLER:
		switch (msg_type) {
		case GENAVB_MSG_AECP:
			if (msg_len >= (offsetof(struct genavb_aecp_msg, buf) + sizeof(struct aecp_aem_pdu))) {
				inner_type = ((struct genavb_aecp_msg *)msg)->msg_type;
