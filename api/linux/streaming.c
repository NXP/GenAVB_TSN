...
static void genavb_set_exit(struct genavb_set_handle *set);
static void genavb_set_destroy(struct genavb_handle *genavb, struct genavb_set_handle *set);

int __avb_stream_destroy(struct genavb_stream_handle *handle)
{
	disconnect_avtp(handle->genavb, &handle->params);

	genavb_set_exit(handle->set);

	genavb_set_destroy(handle->genavb, handle->set);
...
/* Should be called with avb_mutex held. */
static struct genavb_set_handle *genavb_set_find(struct genavb_handle *genavb, unsigned short set_id)
{
	struct list_head *entry, *next;

	for (entry = list_first(&genavb->sets); next = list_next(entry), entry != &genavb->sets; entry = next) {
		struct genavb_set_handle *set = container_of(entry, struct genavb_set_handle, list);
...
/* Should be called with avb_mutex held. */
static int genavb_set_create(struct genavb_handle *genavb, struct genavb_stream_params *params, struct genavb_set_handle **set)
{
	int rc;

	if (!(params->flags & IPC_AVTP_FLAGS_SET_ID_VALID)) {
		/* Redundant static streams are not supported. */
		if (params->flags & GENAVB_STREAM_FLAGS_REDUNDANT) {
			rc = GENAVB_ERR_STREAM_PARAMS;
			goto err_flags;
		}

		/* Allocate set ID for (non redundant) static streams */
		if (static_set_id_alloc(genavb, &(params->set_id), params->direction) < 0) {
			rc = -GENAVB_ERR_STREAM_SET_ID;
			goto err_flags;
		}

		params->set_index = 0;

	} else if (IS_STATIC_STREAM_SET_ID(params->set_id)) {
		/* Valid set_id in stream_params coming from the app should
		 * not overlap with the range of set_ids reserved for static streams
		 */
		rc = -GENAVB_ERR_STREAM_SET_ID;
		goto err_flags;
	}

	*set = genavb_set_find(genavb, params->set_id);
	if (!(*set)) {
		*set = malloc(sizeof(struct genavb_set_handle));
		if (!(*set)) {
			rc = GENAVB_ERR_NO_MEMORY;
			goto err_malloc;
		}

		memset(*set, 0, sizeof(struct genavb_set_handle));

		(*set)->id = params->set_id;
		(*set)->genavb = genavb;
		(*set)->stream_direction = params->direction;

		list_add(&genavb->sets, &(*set)->list);
	} else if (!(params->flags & GENAVB_STREAM_FLAGS_REDUNDANT)) {
		/* a non-redundant stream with a previsouly allocated set, report error. */
		rc = GENAVB_ERR_STREAM_PARAMS;
		goto err_set_params;
	}
...
err_malloc:
err_set_params:
	if (IS_STATIC_STREAM_SET_ID(params->set_id))
		static_set_id_free(genavb, params->set_id, params->direction);
...
static void genavb_set_destroy(struct genavb_handle *genavb, struct genavb_set_handle *set)
{
	...
	if (set->num_streams == 0) {

		if (IS_STATIC_STREAM_SET_ID(set->id))
			static_set_id_free(genavb, set->id, set->stream_direction);
...
static int genavb_set_init(struct genavb_set_handle *set, struct genavb_stream_params const *params, genavb_stream_create_flags_t flags,
			   unsigned int max_payload_size, unsigned int stream_batch, unsigned int *batch_size)
{
	struct media_queue_api_params msg;
	u64 media_id;
	int fd, rc;

	/* No need to check stream params conformity between the
	 * streams of the same set as it's done on the stack side
	 * upon connect_avtp() call.
	 */
	if (set->num_streams > 1)
		goto out;
	...
	if (params->subtype == AVTP_SUBTYPE_CRF) {
		...
		*/
		...
		if (params->direction == AVTP_DIRECTION_TALKER) {
			if (flags & AVTP_DGRAM) {
				msg.max_payload_size = max_payload_size;
			} else {
				/* Align batch size to AVTP thread batch size */
				unsigned int min_batch_size = stream_batch * max_payload_size;
		...
		*/
		media_id = params->set_id;

		msg.port = params->port;
		copy_64(msg.stream_id, &media_id);
		...
		*batch_size = msg.batch_size;
		set->max_payload_size = msg.max_payload_size;
		set->expect_new_frame = 1;
...
{
	unsigned int max_payload_size, stream_batch;
	struct genavb_set_handle *set;
	u64 stream_id;
	int rc;
	...
	copy_64(&stream_id, &params->stream_id);
	...
	*stream = malloc(sizeof(struct genavb_stream_handle));
	...
	rc = genavb_set_create(genavb, &(*stream)->params, &set);
	...
	* connect avtp
	*/
	rc = connect_avtp(genavb, *stream, &max_payload_size, &stream_batch);
	if (rc != GENAVB_SUCCESS) {
		goto err_connect;
	}

	rc = genavb_set_init(set, &(*stream)->params, flags, max_payload_size, stream_batch, batch_size);
	if (rc != GENAVB_SUCCESS) {
		goto err_set_init;
	}
				...
				struct genavb_iovec const *event_iov, unsigned int event_iov_len, unsigned int *event_len)
{
	...
	if (ioctl(handle->set->fd, MEDIA_IOC_RX, &msg) < 0)
				...
				struct genavb_event *event, unsigned int event_len)
{
	...
	max_fu_payload_size = handle->set->max_payload_size - FU_HEADER_SIZE;
	...
	if(!data_len && !data) {
		...
		if ((event && event_len) && ((event->event_mask & AVTP_FLUSH) || (event->event_mask & AVTP_FRAME_END))) {
			handle->set->partial_iovec = 0;
			handle->set->expect_new_frame = 1;
		}
	...
	if (start_code && handle->set->partial_iovec)
		return -GENAVB_ERR_STREAM_TX;

	if (start_code && !handle->set->expect_new_frame)
		return -GENAVB_ERR_STREAM_TX;
	...
	if(start_code) {
		if (data_len - start_code <= handle->set->max_payload_size) {
	...
	if ((handle->set->partial_iovec)) {
		data_to_send = (left_data > handle->set->partial_iovec) ? handle->set->partial_iovec : left_data;
		...
		handle->set->partial_iovec -= data_to_send;
...
second_iovec_member:
		...
		if(iov_idx > (IOV_MAX - 2)) {
			...
			if (is_end_of_frame) {
				...
				handle->set->partial_iovec = max_fu_payload_size - data_iov[iov_idx - 1].iov_len;
			}
			...
			if (event->event_mask & AVTP_FRAME_END)
				handle->set->expect_new_frame = 1; /*Successfully sent the last bytes with an end of frame*/
			else if (!left_data)
				handle->set->expect_new_frame = 0; /*Successfully sent the last bytes without an end of frame*/
...
incomplete_send:
	...
	if (sent > rc) {
		...
		/*reset the handle->set->partial_iovec, it was not sent anyway*/
		handle->set->partial_iovec = 0;
	}
				...
				struct genavb_event const *event, unsigned int event_len)
{
	...
	rc = ioctl(handle->set->fd, MEDIA_IOC_TX, &msg);
	if (rc < 0) {
