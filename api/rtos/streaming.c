	...
	struct genavb_stream_params const *params, unsigned int *batch_size, genavb_stream_create_flags_t flags)
{
	...
	*stream = rtos_malloc(sizeof(struct genavb_stream_handle));
	...
	if (!(params->flags & IPC_AVTP_FLAGS_SET_ID_VALID)) {
		...
		if (static_set_id_alloc(genavb, &(*stream)->params.set_id, (*stream)->params.direction) < 0) {
			rc = -GENAVB_ERR_STREAM_SET_ID;
			goto err_set_id;
		}

		(*stream)->params.set_index = 0;
	...
	* connect avtp
	*/
	rc = connect_avtp(genavb, *stream, &(*stream)->max_payload_size, &(*stream)->batch);
...
err_open:
	disconnect_avtp(genavb, &(*stream)->params);

err_connect:
	if (IS_STATIC_STREAM_SET_ID((*stream)->params.set_id))
		static_set_id_free(genavb, (*stream)->params.set_id, (*stream)->params.direction);
...
int genavb_stream_set_callback(struct genavb_stream_handle const *handle, int (*callback)(void *data), void *data)
{
	if (!handle->mqueue.id)
...
static int __avb_stream_destroy(struct genavb_stream_handle *handle)
{
	...
	if (IS_STATIC_STREAM_SET_ID(handle->params.set_id))
		static_set_id_free(handle->genavb, handle->params.set_id, handle->params.direction);
