...
static int handle_avdecc_event(struct avb_control_handle *ctrl_h, unsigned int *msg_type, const char *binding_file_name)
{
			...
			for (n = 0, i = 0; i < nfds && n < ready; i++) {
				if (poll_fds[i].revents & POLLIN) {
					if (poll_fds[i].fd == ctrl_rx_fd) {
						...
						*/
						if (handle_avdecc_event(app.ctrl_h, &event_type, app.binding_file_name) == AVB_SUCCESS) {
							if (event_type == GENAVB_MSG_MEDIA_STACK_DISCONNECT) {
								rc = 0;
		...
		* Listen to AVDECC events to get stream parameters
		...
		while (1) {
			...
			if (ctrl_poll.revents & POLLIN) {
				...
				*/
				if (handle_avdecc_event(app.ctrl_h, &event_type, app.binding_file_name) == AVB_SUCCESS) {
					if (event_type == GENAVB_MSG_MEDIA_STACK_CONNECT)
