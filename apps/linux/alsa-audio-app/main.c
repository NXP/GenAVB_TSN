...
/*
 * 0 - AVDECC Media Stack channel
 * 1 - AVDECC Controlled Stack channel
 * 2 - Clock Domain status channel
 */
...
#define LISTENER_MAX_FDS	4
static int run_listener(struct avb_stream_handle *stream_h, int stream_fd, int clk_domain_rx_fd, unsigned int batch_size, void *alsa_h)
{
	struct pollfd poll_fds[LISTENER_MAX_FDS];
	...
	*/
	...
	poll_fds[1].fd = clk_domain_rx_fd;
	...
	if (app.controlled_h) {
		...
		poll_fds[nfds].fd = controlled_rx_fd;
		poll_fds[nfds].events = POLLIN;
		poll_fds[nfds].revents = 0;
		...
		if (ready > 0) {
			for (n = 0, i = 0; i < nfds && n < ready; i++) {
				if (poll_fds[i].revents & POLLIN) {
					...
					} else if (poll_fds[i].fd == clk_domain_rx_fd) {
						n++;
...
int main(int argc, char *argv[])
{
	...
	/*
	 * Open clock domain channel
	 */
	rc = clock_domain_init(avb_h, &clk_domain_rx_fd);
	if (rc < 0)
	...
	if (app.mode == MODE_AVDECC) {
		...
		ctrl_poll[1].revents = 0;
		nfds++;

		/*
		 * Clock domain channel for CLOCK_DOMAIN status
		 */
		ctrl_poll[2].fd = clk_domain_rx_fd;
		ctrl_poll[2].events = POLLIN;
		ctrl_poll[2].revents = 0;
		nfds++;

		/* Parse only the persistent bindings params and send them to avdecc, then start the entity.
		 * An entity ID of 0 starts the first non-controller entity.
		 */
		if (avdecc_entity_start_with_persistent_params(app.ctrl_h, 0, app.binding_file_name, NULL, NULL, NULL, NULL, 0) < 0) {
...
wait_new_stream:
	if (app.mode == MODE_AVDECC) {
			...
			if (ready > 0) {
				for (n = 0, i = 0; i < nfds && n < ready; i++) {
					if (ctrl_poll[i].revents & POLLIN) {
						...
						} else if (ctrl_poll[i].fd == clk_domain_rx_fd) {
							n++;

							/*
							 * Read clock domain status
							 */
							handle_clock_domain_event();
						}
					}
		...
		if (app.mode == MODE_TALKER) {
			rc = clock_domain_set_role(MEDIA_CLOCK_MASTER, default_stream_params.clock_domain, NULL);
			if (rc != AVB_SUCCESS) {
			...
			rc = clock_domain_set_role(MEDIA_CLOCK_SLAVE, default_stream_params.clock_domain, &default_stream_params);
			if (rc != AVB_SUCCESS) {
				printf("clock_domain_set_role failed: %s\n", avb_strerror(rc));
				goto err_clock_role;
			}
...
start_stream:
	...
	if (app.stream_params.direction & AVTP_DIRECTION_TALKER) {
		...
		rc = run_listener(stream_h, stream_fd, clk_domain_rx_fd, app.stream_batch_size, alsa_h);
	}
