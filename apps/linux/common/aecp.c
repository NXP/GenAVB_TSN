...
/** Sends an AECP SET_NAME command.
 *
 * \return AVB_SUCCESS on success, or genavb error code.
 * \param	ctrl_h				Handle to an avb_control_handle, opened for an AVB_CTRL_AVDECC_CONTROLLER channel.
 * \param	entity_id			Pointer to the entity ID [Network Order] to send the message.
 * \param	configuration_index		Index of the Configuration to read the descriptor from.
 * \param	descriptor_type			Type of the descriptor to rename.
 * \param	descriptor_index		Index of the descriptor to rename.
 * \param	name_index			Index of the descriptor's name to edit. (For AEM_DESC_TYPE_ENTITY either 0:entity_name or 1:group_name. For the others 0:object_name).
 * \param	name				Pointer to the AEM_STR_LEN_MAX bytes long buffer containing the new name + 1 byte for the trailing null character.
 * \param	status				On return in sync mode, will contain the status from the AECP response if command was successful. Ignored otherwise.
 * \param	sync				If sync == 0, the function will return without waiting for a response from the target entity. Otherwise, the function will block until receiving a response
 * 						and descriptor, len and status will be updated to match the response.
 *
 */
int aecp_aem_send_set_name(struct avb_control_handle *ctrl_h, avb_u64 *entity_id, avb_u16 configuration_index,
			    avb_u16 descriptor_type, avb_u16 descriptor_index, avb_u16 name_index, const char *name, avb_u8 *status, int sync)
{
	struct avb_aecp_msg aecp_msg;
	struct aecp_aem_pdu *aecp_cmd = (struct aecp_aem_pdu *) aecp_msg.buf;
	struct aecp_aem_set_name_pdu *set_name_cmd = (struct aecp_aem_set_name_pdu *)(aecp_cmd + 1);
	int rc;

	memset(aecp_cmd, 0, sizeof(struct aecp_aem_pdu));
	memset(set_name_cmd, 0, sizeof(struct aecp_aem_set_name_pdu));
	aecp_msg.len = sizeof(struct aecp_aem_pdu) + sizeof(struct aecp_aem_set_name_pdu);
	AECP_AEM_SET_U_CMD_TYPE(aecp_cmd, 0, AECP_AEM_CMD_SET_NAME);
	aecp_msg.msg_type = AECP_AEM_COMMAND;
	memcpy(&aecp_cmd->entity_id, entity_id, 8);

	set_name_cmd->descriptor_type = htons(descriptor_type);
	set_name_cmd->descriptor_index = htons(descriptor_index);
	set_name_cmd->configuration_index = htons(configuration_index);
	set_name_cmd->name_index = htons(name_index);
	memcpy(set_name_cmd->name, name, AEM_STR_LEN_MAX);
	...
	if (rc != AVB_SUCCESS)
		printf("%s: avb_control_send_sync() failed: AVB_MSG_AECP SET_NAME entity_id(%" PRIx64 ") desc_type(%d) desc_idx(%d) error %s\n", __func__, ntohll(*entity_id), descriptor_type, descriptor_index, avb_strerror(rc));

	return rc;
}

/** Sends an AECP SET_CLOCK_SOURCE command.
 *
 * \return AVB_SUCCESS on success, or genavb error code.
 * \param	ctrl_h				Handle to an avb_control_handle, opened for an AVB_CTRL_AVDECC_CONTROLLER channel.
 * \param	entity_id			Pointer to the entity ID [Network Order] to send the message.
 * \param	domain_index			Index of the clock domain index
 * \param	clock_source_index		Index of the clock source to set.
 * \param	status				On return in sync mode, will contain the status from the AECP response if command was successful. Ignored otherwise.
 * \param	sync				If sync == 0, the function will return without waiting for a response from the target entity. Otherwise, the function will block until receiving a response
 * 						and descriptor, len and status will be updated to match the response.
 *
 */
int aecp_aem_send_set_clock_source(struct avb_control_handle *ctrl_h, avb_u64 *entity_id, avb_u16 domain_index,
				   avb_u16 clock_source_index, avb_u8 *status, int sync)
{
	struct avb_aecp_msg aecp_msg;
	struct aecp_aem_pdu *aecp_cmd = (struct aecp_aem_pdu *) aecp_msg.buf;
	struct aecp_aem_set_clock_source_pdu *set_clock_source_cmd = (struct aecp_aem_set_clock_source_pdu *)(aecp_cmd + 1);
	int rc;

	memset(aecp_cmd, 0, sizeof(struct aecp_aem_pdu));
	memset(set_clock_source_cmd, 0, sizeof(struct aecp_aem_set_clock_source_pdu));
	aecp_msg.len = sizeof(struct aecp_aem_pdu) + sizeof(struct aecp_aem_set_clock_source_pdu);
	AECP_AEM_SET_U_CMD_TYPE(aecp_cmd, 0, AECP_AEM_CMD_SET_CLOCK_SOURCE);
	aecp_msg.msg_type = AECP_AEM_COMMAND;
	memcpy(&aecp_cmd->entity_id, entity_id, 8);

	set_clock_source_cmd->descriptor_type = htons(AEM_DESC_TYPE_CLOCK_DOMAIN);
	set_clock_source_cmd->descriptor_index = htons(domain_index);
	set_clock_source_cmd->clock_source_index = htons(clock_source_index);
	...
	if (rc != AVB_SUCCESS)
		printf("%s: avb_control_send_sync() failed: AVB_MSG_AECP SET_CLOCK_SOURCE entity_id(%" PRIx64 ")"
		       "clock_domain(%u) clock_source(%u) error %s\n",
		       __func__, ntohll(*entity_id), domain_index, clock_source_index, avb_strerror(rc));
...
#define APPEND_STR(...)	do { \
	...
	case AVTP_SUBTYPE_CRF:
	{
		...
		APPEND_STR("CRF (Type %u)", crf_fmt->type);
		APPEND_STR(" %uHz", AVDECC_FMT_CRF_BASE_FREQUENCY(format));
		APPEND_STR(" timestamp_interval(%u)", AVDECC_FMT_CRF_TIMESTAMP_INTERVAL(format));
		APPEND_STR(" timestamps_per_pdu(%u)", crf_fmt->timestamps_per_pdu);
		APPEND_STR(" pull(%u)", crf_fmt->pull);
		break;
	}
	default:
		APPEND_STR("undetermined");
