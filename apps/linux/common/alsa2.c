...
int alsa_rx_start(aar_alsa_handle_t *handle, aar_avb_stream_t *avbstream)
{
	snd_pcm_t *snd_handle = handle->handle;
	...
	err = genavb_clock_gettime64(avbstream->clock_avtp, &now64);
	if (err < 0) {
		ERR("alsa(%p) genavb_clock_gettime64(%d) failed", handle, avbstream->clock_avtp);
		return -1;
	}

	handle->start_time = (unsigned int) (now64 & 0x00000000ffffffffULL);
...
int alsa_rx(aar_alsa_handle_t *alsa_handle, aar_avb_stream_t *avbstream)
{
	...
	if (genavb_clock_gettime64(avbstream->clock_avtp, &now64) >= 0) {

		gptp_time = (unsigned int) (now64 & 0x00000000ffffffffULL);

		if (avbstream->set->is_first_wakeup) {
			// First wakeup, just store the time
			avbstream->set->is_first_wakeup = 0;
			avbstream->set->last_gptp_time = gptp_time;
		} else {
			// calculate stats
			stats_update(&avbstream->set->stats.gptp_2cont_wakeup, gptp_time - avbstream->set->last_gptp_time);
			avbstream->set->last_gptp_time = gptp_time;
		}
	}
	...
	if (ret < 0) {
		...
		ret = alsa_rx_start(alsa_handle, avbstream);
		if (ret < 0) {
	...
	do {
		...
		do {
			...
			if (nbytes <= 0) {
				 ...
				 */
				avbstream->set->stats.counter_stats.tx_err++;
				break;
			}
			avbstream->set->stats.counter_stats.batch_tx++;
...
int alsa_tx_start(aar_alsa_handle_t *handle, aar_avb_stream_t *avbstream)
{
	...
	avbstream->set->last_exchanged_frames = 0;
	avbstream->set->last_event_frame_offset = 0;
	...
	do {
		...
		now = (unsigned int) (now64 & 0x00000000ffffffffULL);
...
start:
	...
	err = genavb_clock_gettime64(avbstream->clock_avtp, &now64);
	if (err < 0) {
		ERR("alsa(%p) genavb_clock_gettime64(%d) failed for now", handle, avbstream->clock_avtp);
		goto exit;
	}
...
#if ALSA_EXTRA_DEBUG
	err = genavb_clock_gettime64(avbstream->clock_avtp, &then);
	if (err < 0) {
		ERR("alsa(%p) genavb_clock_gettime64(%d) failed for then", handle, avbstream->clock_avtp);
		goto exit;
	}

	ptp_intvl = then - now64;
	now64 += (ptp_intvl / 2);
	...
	INF("alsa(%p) now = %u     avtp_ts = %u",
			handle, now, tstamp);
	INF("alsa(%p) gettime accuracy intvl = %"PRIu64" ns     avtp_ts-now = %u us ",
	...
	while (frames_remaining > 0) {
...
#endif
			avbstream->set->stats.counter_stats.rx_err++;
		...
		if (event_len) {
			...
			if (i < event_len) {
				...
				if (avbstream->set->is_first_wakeup) {
					avbstream->set->last_event_ts = event[i].ts;
					avbstream->set->last_event_frame_offset = avbstream->set->last_exchanged_frames + event[i].index / alsa_handle->frame_size;
				} else if (is_first_event) {
					unsigned int dt_elapsed = event[i].ts - avbstream->set->last_event_ts;
					unsigned int event_frame_offset = avbstream->set->last_exchanged_frames + event[i].index / alsa_handle->frame_size;
					unsigned int frames_elapsed = event_frame_offset - avbstream->set->last_event_frame_offset;

					avbstream->set->last_event_ts = event[i].ts;
					avbstream->set->last_event_frame_offset = event_frame_offset;

					if ( avbstream->set->last_exchanged_frames && ( abs(dt_elapsed - frames_elapsed * alsa_handle->frame_duration) > alsa_handle->frame_duration)) {
						ret = snd_pcm_mmap_commit(snd_handle, offset, 0);
					...
					is_first_event = 0;
					stats_update(&avbstream->set->stats.event_2cont_wakeup, dt_elapsed);
					stats_update(&avbstream->set->stats.event_gptp, event[i].ts - gptp_time);
				}
			} else // Invalid time stamp
				alsa_handle->stats.counter_stats.tx_err++;
		}

		avbstream->set->stats.counter_stats.batch_rx++;
...
#endif
		exchanged += nbytes;
		avbstream->set->last_exchanged_frames += nbytes / alsa_handle->frame_size;
