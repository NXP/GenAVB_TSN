...
static struct audio_mapping_entry stream_port_input_audio_mappings[MAX_CONFIGURATIONS][MAX_STREAM_PORT_INPUT][MAX_AUDIO_MAPS][MAX_AUDIO_MAP_SIZE];
static struct audio_mapping_entry stream_port_output_audio_mappings[MAX_CONFIGURATIONS][MAX_STREAM_PORT_OUTPUT][MAX_AUDIO_MAPS][MAX_AUDIO_MAP_SIZE];
...
int audio_mappings_remove(avb_u16 config_index, avb_u16 desc_index, avb_u16 desc_type, avb_u16 mappings_count, void *audio_mappings)
{
	struct audio_mapping_entry (*stream_port_audio_mappings)[MAX_STREAM_PORT][MAX_AUDIO_MAPS][MAX_AUDIO_MAP_SIZE];
	...
	if (config_index >= MAX_CONFIGURATIONS) {
		printf("%s: Error: Unsupported config index(%u). Max supported(%u)\n", __func__, config_index, MAX_CONFIGURATIONS - 1);
	...
	for (i = 0; i < mappings_count; i++) {
		...
		for (j = 0; j < MAX_AUDIO_MAP_SIZE; j++) {
			...
			if (!stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][j].valid)
				continue;

			if (!memcmp(&((struct aecp_aem_get_audio_map_mappings_format *)audio_mappings)[i], &stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][j].mapping, sizeof(struct aecp_aem_get_audio_map_mappings_format))) {
	...
	for (i = 0; i < remove_entries_count; i++) {
		stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][remove_entries[i].registered_index].valid = false;
	}

	if (strlen(nvm_audio_mappings_file_name))
		avdecc_nvm_audio_mappings_remove(nvm_audio_mappings_file_name, config_index, desc_type, desc_index, AUDIO_MAP_IDX,
						 remove_entries_count, remove_entries, stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX]);

exit:
	return rc;
}

int audio_mappings_add(avb_u16 config_index, avb_u16 desc_index, avb_u16 desc_type, avb_u16 mappings_count, void *audio_mappings)
{
	struct audio_mapping_entry (*stream_port_audio_mappings)[MAX_STREAM_PORT][MAX_AUDIO_MAPS][MAX_AUDIO_MAP_SIZE];
	...
	for (i = 0; i < mappings_count; i++) {
		...
		for (j = 0; j < MAX_AUDIO_MAP_SIZE; j++) {
			...
			if (!memcmp(&((struct aecp_aem_get_audio_map_mappings_format *)audio_mappings)[i],
				    &stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][j].mapping,
				    sizeof(struct aecp_aem_get_audio_map_mappings_format))) {
		...
		for (j = 0; j < MAX_AUDIO_MAP_SIZE; j++) {
			...
			if (desc_type == AEM_DESC_TYPE_STREAM_PORT_INPUT) {
				 ...
				 */
				if ((mapping_cluster_offset == stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][j].mapping.mapping_cluster_offset) &&
				    (mapping_cluster_channel == stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][j].mapping.mapping_cluster_channel) &&
				    ((mapping_stream_index != stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][j].mapping.mapping_stream_index) ||
				    (mapping_stream_channel != stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][j].mapping.mapping_stream_channel))) {
					 ...
					 */
					stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][j].valid = false;
				 ...
				 * As per MILAN Specification v1.2 5.4.2.27
				 */
				if ((mapping_stream_index == stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][j].mapping.mapping_stream_index) &&
				    (mapping_stream_channel == stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][j].mapping.mapping_stream_channel) &&
				    ((mapping_cluster_offset != stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][j].mapping.mapping_cluster_offset) ||
				    (mapping_cluster_channel != stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][j].mapping.mapping_cluster_channel))) {
					new_entries[new_entries_count].registered_index = j;
					new_entries[new_entries_count].mapping_index = i;
					new_entries_count++;
		...
		index = get_free_audio_mapping_index(stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX]);
		if (index < 0) {
...
next_mapping:
	...
	for (i = 0; i < new_entries_count; i++) {
		memcpy(&stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][new_entries[i].registered_index].mapping,
			&((struct aecp_aem_get_audio_map_mappings_format *)audio_mappings)[new_entries[i].mapping_index],
			sizeof(struct aecp_aem_get_audio_map_mappings_format));

		stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][new_entries[i].registered_index].valid = true;
	}

	if (strlen(nvm_audio_mappings_file_name))
		avdecc_nvm_audio_mappings_update(nvm_audio_mappings_file_name, config_index, desc_type, desc_index, AUDIO_MAP_IDX,
						 new_entries_count, new_entries, stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX]);

mappings_full:
	/* Reset selected state of audio_mapping_entries, they either became valid or the status is AECP_AEM_NOT_SUPPORTED */
	for (i = 0; i < MAX_AUDIO_MAP_SIZE; i++) {
		stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][i].selected = false;
	}
...
int audio_mappings_get(avb_u16 config_index, avb_u16 desc_index, avb_u16 desc_type, avb_u16 map_index, avb_u16 *number_of_maps, avb_u16 *mappings_count, void *audio_mappings)
{
	struct audio_mapping_entry (*stream_port_audio_mappings)[MAX_STREAM_PORT][MAX_AUDIO_MAPS][MAX_AUDIO_MAP_SIZE];
	...
	for (i = 0; i < MAX_AUDIO_MAP_SIZE; i++) {
		if (stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][i].valid) {
			memcpy(&((struct aecp_aem_get_audio_map_mappings_format *)audio_mappings)[mapping_index], &stream_port_audio_mappings[config_index][desc_index][AUDIO_MAP_IDX][i].mapping, sizeof(struct aecp_aem_get_audio_map_mappings_format));
			mapping_index++;
		}
	}
...
int audio_mappings_set(avb_u16 config_index, avb_u16 desc_index, avb_u16 desc_type, avb_u16 audio_map_index, avb_u16 mapping_index, struct audio_mapping_entry *audio_mapping_entry)
{
	struct audio_mapping_entry (*stream_port_audio_mappings)[MAX_STREAM_PORT][MAX_AUDIO_MAPS][MAX_AUDIO_MAP_SIZE];
	...
	memcpy(&stream_port_audio_mappings[config_index][desc_index][audio_map_index][mapping_index], audio_mapping_entry, sizeof(struct audio_mapping_entry));

	stream_port_audio_mappings[config_index][desc_index][audio_map_index][mapping_index].valid = true;
	stream_port_audio_mappings[config_index][desc_index][audio_map_index][mapping_index].selected = false;
	stream_port_audio_mappings[config_index][desc_index][audio_map_index][mapping_index].reserved = 0;
...
{
	unsigned int i, j, cfg;
	int rc = AECP_AEM_SUCCESS;

	for (cfg = 0; cfg < MAX_CONFIGURATIONS; cfg++) {
		for (i = 0; i < MAX_STREAM_PORT_INPUT; i++) {
			for (j = 0; j < MAX_AUDIO_MAP_SIZE; j++) {
				stream_port_input_audio_mappings[cfg][i][AUDIO_MAP_IDX][j].valid = false;
				stream_port_input_audio_mappings[cfg][i][AUDIO_MAP_IDX][j].selected = false;
			}
		}
	}
	...
	for (cfg = 0; cfg < MAX_CONFIGURATIONS; cfg++) {
		for (i = 0; i < MAX_STREAM_PORT_OUTPUT; i++) {
			for (j = 0; j < MAX_AUDIO_MAP_SIZE; j++) {
				stream_port_output_audio_mappings[cfg][i][AUDIO_MAP_IDX][j].valid = false;
				stream_port_output_audio_mappings[cfg][i][AUDIO_MAP_IDX][j].selected = false;
			}
		}
	}
