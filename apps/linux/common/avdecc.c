...
/* BINDING PARAMS ENTRY: [ListenerEntityID ListenerConfigIndex ListenerStreamIndex TalkerEntityID TalkerStreamIndex ControllerEntityID StreamStarted] */
#define NVRAM_BINDING_PARAMS_PER_ENTRY		7
#define MAX_BINDING_LISTENER_STREAMS_PER_CONFIG	16 /* 16 Streams per configuration */
#define MAX_BINDING_LISTENER_STREAMS		(MAX_BINDING_LISTENER_STREAMS_PER_CONFIG * MAX_CONFIGURATIONS) /* 16 Streams per configuration */
#define TMP_BINDING_FILENAME			"/tmp_milan_binding_params"

/* AUDIO MAPPINGS ENTRY: [ConfigIndex DescriptorType DescriptorIndex AudioMapIndex AudioMappingIndex StreamIndex StreamChannel ClusterOffset ClusterChannel] */
#define NVRAM_AUDIO_MAPPINGS_PER_ENTRY		9
...
/* PERSISTENT PARAMS ENTRY: [EntityID ConfigIdx DescriptorType DescriptorIndex PersistentParamType PersistentParamValue] */
#define NVRAM_PERSISTENT_PARAMS_PER_ENTRY	6
...
static int nvram_update_bindings_params(const char *binding_filename, avb_u64 entity_id, avb_u16 listener_config_index, avb_u16 listener_stream_index,
					avb_u64 talker_entity_id, avb_u16 talker_stream_index, avb_u64 controller_entity_id, avb_u16 started)
{
	avb_u16 orig_listener_stream_index, orig_listener_config_index, orig_talker_stream_index, orig_started;
	...
	if (talker_entity_id) {
		...
		rc = snprintf(new_entry, NVRAM_ENTRY_MAX_LEN, "%016"PRIx64" %u %u %016"PRIx64" %u %016"PRIx64" %u\n",
			    entity_id , listener_config_index, listener_stream_index, talker_entity_id,
	...
	while(((read_size = getline(&orig_entry, &len, fOrig)) != -1)) {
		...
		if (sscanf(orig_entry, "%016"PRIx64" %hu %hu %016"PRIx64" %hu %016"PRIx64" %hu", &orig_entity_id, &orig_listener_config_index, &orig_listener_stream_index,
				&orig_talker_entity_id, &orig_talker_stream_index, &orig_controller_entity_id, &orig_started) == NVRAM_BINDING_PARAMS_PER_ENTRY) {

			if (orig_listener_config_index == listener_config_index && orig_listener_stream_index == listener_stream_index && orig_entity_id == entity_id) {
				/* If matching listener entity ID, config and stream indexes, either remove or update it
...
static int nvram_update_audio_mappings(const char *audio_mappings_file_name, avb_u16 config_index, avb_u16 desc_type, avb_u16 desc_index,
					avb_u16 audio_map_index, unsigned int new_entries_count, struct mapping_entry *new_entries,
					struct audio_mapping_entry *stream_port_audio_mappings)
{
	avb_u16 orig_config_index, orig_stream_index, orig_stream_channel, orig_cluster_offset, orig_cluster_channel;
	...
	while(((read_size = getline(&orig_entry, &len, fOrig)) != -1)) {
		...
		if (sscanf(orig_entry, "%hu %hu %hu %hu %hu %hu %hu %hu %hu",
			    &orig_config_index, &orig_desc_type, &orig_desc_index, &orig_audio_map_index, &orig_mapping_index,
			...
			for (i = 0; i < new_entries_count; i++) {
				...
				case AEM_DESC_TYPE_STREAM_PORT_INPUT:
					if ((orig_config_index == config_index) && (orig_desc_type == desc_type) && (orig_desc_index == desc_index) &&
	...
	for (i = 0; i < new_entries_count; i++) {
		...
		if (!entries_found[i]) {
			...
			rc = snprintf(new_entry, NVRAM_ENTRY_MAX_LEN, "%hu %hu %hu %hu %hu %hu %hu %hu %hu\n",
				    config_index, desc_type, desc_index, audio_map_index, new_entries[i].registered_index,
				    stream_index, stream_channel, cluster_offset, cluster_channel);
...
static int nvram_remove_audio_mappings(const char *audio_mappings_file_name, avb_u16 config_index, avb_u16 desc_type, avb_u16 desc_index,
					avb_u16 audio_map_index, unsigned int entries_count, struct mapping_entry *removed_entries,
					struct audio_mapping_entry *stream_port_audio_mappings)
{
	avb_u16 orig_config_index, orig_stream_index, orig_stream_channel, orig_cluster_offset, orig_cluster_channel;
	avb_u16 orig_desc_type, orig_desc_index, orig_audio_map_index, orig_mapping_index;
...
{
	char entry_value[AEM_STR_LEN_MAX + 1] = {'\0'}; /* data buffer of at max AEM_STR_LEN_MAX u8 + trailing null */
	avb_u16 orig_desc_type, orig_desc_index, orig_config_index;
	...
	{
		rc = snprintf(new_entry, NVRAM_ENTRY_MAX_LEN, "%016"PRIx64" %hu %hu %hu %u %." VAL_TO_STR(AEM_STR_LEN_MAX) "s\n",
			    persistent_param->entity_id, persistent_param->config_index, persistent_param->desc_type, persistent_param->desc_index,
	...
	{
		rc = snprintf(new_entry, NVRAM_ENTRY_MAX_LEN, "%016"PRIx64" %hu %hu %hu %u %hu\n",
			    persistent_param->entity_id, persistent_param->config_index, persistent_param->desc_type, persistent_param->desc_index,
	...
	{
		rc = snprintf(new_entry, NVRAM_ENTRY_MAX_LEN, "%016"PRIx64" %hu %hu %hu %u %016"PRIx64"\n",
			    persistent_param->entity_id, persistent_param->config_index, persistent_param->desc_type, persistent_param->desc_index,
	...
	{
		rc = snprintf(new_entry, NVRAM_ENTRY_MAX_LEN, "%016"PRIx64" %hu %hu %hu %u %u\n",
			    persistent_param->entity_id, persistent_param->config_index, persistent_param->desc_type, persistent_param->desc_index,
	...
	case AVDECC_PERSISTENT_PARAM_ENTITY_CONFIGURATION_INDEX:
	{
		/* Set the configuration index as param value even if it's redundant. It will be discarded on read. */
		rc = snprintf(new_entry, NVRAM_ENTRY_MAX_LEN, "%016"PRIx64" %hu %hu %hu %u %u\n",
			    persistent_param->entity_id, persistent_param->config_index, persistent_param->desc_type, persistent_param->desc_index,
			    persistent_param->param_type, persistent_param->config_index);
	...
	while(((read_size = getline(&orig_entry, &len, fOrig)) != -1)) {
		...
		if (sscanf(orig_entry, "%016"PRIx64" %hu %hu %hu %u %" VAL_TO_STR(AEM_STR_LEN_MAX) "[^\n]",
			    &orig_entity_id, &orig_config_index, &orig_desc_type, &orig_desc_index,
			    &orig_param_type, entry_value) == NVRAM_PERSISTENT_PARAMS_PER_ENTRY) {

			entry_value[AEM_STR_LEN_MAX] = '\0';

			if ((orig_entity_id == persistent_param->entity_id) &&
			    ((persistent_param->param_type == AVDECC_PERSISTENT_PARAM_ENTITY_CONFIGURATION_INDEX) || (orig_config_index == persistent_param->config_index)) &&
...
{
	avb_u16 listener_config_index, listener_stream_index, talker_stream_index, started;
	...
	while (((read_char = getline(&nvram_entry, &len, fBind)) != -1)) {
		...
		if (sscanf(nvram_entry, "%016"PRIx64" %hu %hu %016"PRIx64" %hu %016"PRIx64" %hu", &entity_id, &listener_config_index, &listener_stream_index,
				&talker_entity_id, &talker_stream_index, &controller_entity_id, &started) == NVRAM_BINDING_PARAMS_PER_ENTRY) {

			printf("Read EntityID %016"PRIx64" Listener (Config ID %u unique ID %u) Talker entity ID %016"PRIx64" Talker unique ID %u Controller entity ID%016"PRIx64" Started %u\n",
				entity_id , listener_config_index, listener_stream_index, talker_entity_id, talker_stream_index,
				controller_entity_id, started);

			index = MAX_BINDING_LISTENER_STREAMS_PER_CONFIG * listener_config_index + listener_stream_index;

			if (index < size) {
				binding_params[index].entity_id = entity_id;
				binding_params[index].listener_config_index = listener_config_index;
				binding_params[index].listener_stream_index = listener_stream_index;
				binding_params[index].talker_entity_id = talker_entity_id;
				binding_params[index].talker_stream_index = talker_stream_index;
				binding_params[index].controller_entity_id = controller_entity_id;
				binding_params[index].started = started;
			} else
				printf("error on nvram entry %s, out of bound index %u (Listener config %u, Listener index %u)\n",
					nvram_entry, index, listener_config_index, listener_stream_index);
		} else
...
{
	avb_u16 config_index, desc_type, desc_index, audio_map_index, mapping_index;
	...
	while (((read_char = getline(&nvram_entry, &len, fAudMap)) != -1)) {
		...
		if (sscanf(nvram_entry, "%hu %hu %hu %hu %hu %hu %hu %hu %hu",
			    &config_index, &desc_type, &desc_index, &audio_map_index, &mapping_index,
			...
			audio_mappings_set(config_index, desc_index, desc_type, audio_map_index, mapping_index, &audio_mapping_entry);
		} else {
			printf("error: can not read data from nvram\n");
		}
	}
...
{
	avb_u16 desc_type, desc_index, cfg_index;
	avb_u32 param_type;
	avb_u64 entity_id;
	char entry_value[AEM_STR_LEN_MAX + 1] = {'\0'}; /* data buffer of at max AEM_STR_LEN_MAX u8 + trailing null */
	...
	while (((read_char = getline(&nvram_entry, &len, fPersist)) != -1)) {
		...
		memset(entry_value, 0, (AEM_STR_LEN_MAX + 1));

		if (sscanf(nvram_entry, "%016"PRIx64" %hu %hu %hu %u %" VAL_TO_STR(AEM_STR_LEN_MAX) "[^\n]",
			    &entity_id, &cfg_index, &desc_type, &desc_index, &param_type, entry_value) == NVRAM_PERSISTENT_PARAMS_PER_ENTRY) {

			/* At max, from a SET_NAME command, a 64 bytes long data without trailing null can be passed,
			 * As per IEEE 1722_1 2021, 7.4.17.1.
			 * Just make sure that entry_value is null terminated as it is used as a string for conversions.
			 */
			entry_value[AEM_STR_LEN_MAX] = '\0';

			printf("Read EntityID %016"PRIx64" ConfigIndex %hu descType %hu descID %hu paramType %u value %s\n",
				entity_id, cfg_index, desc_type, desc_index, param_type, entry_value);
			...
			if (i < size) {
				...
				case AVDECC_PERSISTENT_PARAM_ENTITY_NAME:
				{
					memcpy(persistent_params[i].u.name, entry_value, AEM_STR_LEN_MAX);
					break;
				}
				case AVDECC_PERSISTENT_PARAM_CLOCK_SOURCE_INDEX:
				{
				...
				case AVDECC_PERSISTENT_PARAM_ENTITY_CONFIGURATION_INDEX:
				{
					/* Ignore the param value for the configuration index. */
					break;
				}
				default:
				...
				persistent_params[i].config_index = cfg_index;
...
static int avdecc_listener_stream_bind_send_ipc(struct avb_control_handle *s_avdecc_handle, struct genavb_msg_media_stack_bind *binding_params)
{
	...
	media_stack_bind.listener_config_index = binding_params->listener_config_index;
	...
	printf("Sent binding parameters: EntityID %016"PRIx64" Listener (Config ID %u, unique ID %u) Talker entity ID %016"PRIx64" Talker unique ID %u Controller entity ID%016"PRIx64" Started %u\n",
		binding_params->entity_id, binding_params->listener_config_index, binding_params->listener_stream_index,
...
static int avdecc_persistent_params_send_ipc(struct avb_control_handle *s_avdecc_handle, struct genavb_msg_media_stack_persistent_param *persistent_param)
{
	...
	case AVDECC_PERSISTENT_PARAM_ENTITY_NAME:
	{
		memcpy(media_stack_persistent_param.u.name, persistent_param->u.name, AEM_STR_LEN_MAX);
		break;
	}
	case AVDECC_PERSISTENT_PARAM_CLOCK_SOURCE_INDEX:
	{
	...
	case AVDECC_PERSISTENT_PARAM_ENTITY_CONFIGURATION_INDEX:
	{
		/* The configuration index information is set below. */
		break;
	}
	default:
	...
	media_stack_persistent_param.config_index = persistent_param->config_index;
	...
	printf("Sent persistent parameter: EntityID %016"PRIx64" configIndex %hu descType %hu descID %hu paramType %u\n",
		persistent_param->entity_id, persistent_param->config_index, persistent_param->desc_type, persistent_param->desc_index, persistent_param->param_type);

exit:
	return rc;
}

static int avdecc_memory_objects_send_ipc(struct avb_control_handle *s_avdecc_handle, struct genavb_msg_memory_object_desc_attr *memory_object, avb_u64 entity_id)
{
	struct genavb_msg_media_stack_memory_object media_stack_memory_object;
	genavb_msg_type_t msg_type = GENAVB_MSG_MEDIA_STACK_MEMORY_OBJECT;
	unsigned int msg_len = sizeof(media_stack_memory_object);
	int rc = GENAVB_SUCCESS;

	if (!memory_object || !s_avdecc_handle) {
		rc = -GENAVB_ERR_INVALID_PARAMS;
		goto exit;
	}

	memcpy(&media_stack_memory_object.desc_attr, memory_object, sizeof(struct genavb_msg_memory_object_desc_attr));

	media_stack_memory_object.entity_id = entity_id;

	rc = genavb_control_send(s_avdecc_handle, msg_type, &media_stack_memory_object, msg_len);
	if (rc != GENAVB_SUCCESS) {
		printf("genavb_control_send (GENAVB_CTRL_AVDECC_MEDIA_STACK) failed: %s\n", avb_strerror(rc));
		goto exit;
	}

	printf("Sent Memory Object: configID %hu descID %hu type %hu len %"PRIu64" max len %"PRIu64"\n",
		memory_object->cfg_idx, memory_object->desc_idx, memory_object->object_type, memory_object->len, memory_object->max_len);
...
{
	int i;
	struct genavb_msg_media_stack_bind listener_streams_binding_params[MAX_BINDING_LISTENER_STREAMS] = {0};
	...
	if (parse_binding_params_file(binding_filename, listener_streams_binding_params, MAX_BINDING_LISTENER_STREAMS) < 0) {
			printf("failed to parse binding file %s\n", binding_filename);
			goto err;
	}

	/* Send them to the AVDECC stack. */
	for (i = 0; i < MAX_BINDING_LISTENER_STREAMS; i++) {
...
/** Init the memory objects according to their configuration file and send them to the avdecc stack
 *
 * \return 0 on success, negative otherwise.
 * \param	ctrl_h				Handle to an avb_control_handle, opened for an AVB_CTRL_AVDECC_MEDIA_STACK channel.
 * \param	memory_objects_file_name	path to the file used as configuration for memory objects.
 * \param	entity_id			entity's ID in network order.
 */
static int avdecc_memory_objects_init(struct genavb_control_handle *s_avdecc_handle, const char *memory_objects_file_name, avb_u64 entity_id)
{
	struct genavb_msg_memory_object_desc_attr memory_objects[AEM_NUM_MEMORY_OBJECTS_MAX] = {0};
	int num_memory_objects, i;
	...
	if (!memory_objects_file_name) {
		printf("Invalid memory objects filename\n");
		goto err;
	}

	if ((num_memory_objects = memory_objects_init(memory_objects_file_name, memory_objects)) < 0) {
		printf("failed to parse memory objects file %s\n", memory_objects_file_name);
		goto err;
	}

	/* Send memory objects' configuration to the AVDECC stack. */
	for (i = 0; i < num_memory_objects; i++) {
		if (avdecc_memory_objects_send_ipc(s_avdecc_handle, &memory_objects[i], entity_id) < 0) {
			printf("failed to send memory object %u\n", i);
			goto err_send;
		}
	}
...
void avdecc_nvm_bindings_update(const char *binding_filename, struct genavb_msg_media_stack_bind *binding_params)
{
	...
	nvram_update_bindings_params(binding_filename, binding_params->entity_id, binding_params->listener_config_index, binding_params->listener_stream_index,
				binding_params->talker_entity_id, binding_params->talker_stream_index,
				binding_params->controller_entity_id, binding_params->started);

	printf("update %s with entry: EntityID %016"PRIx64" Listener (Config ID %u, unique ID %u) Talker entity ID %016"PRIx64" Talker unique ID %u Controller entity ID%016"PRIx64" Started %u\n",
		binding_filename, binding_params->entity_id, binding_params->listener_config_index, binding_params->listener_stream_index,
		binding_params->talker_entity_id, binding_params->talker_stream_index,
		binding_params->controller_entity_id, binding_params->started);
}
...
void avdecc_nvm_bindings_remove(const char *binding_filename, struct genavb_msg_media_stack_unbind *unbinding_params)
{
	...
	nvram_update_bindings_params(binding_filename, unbinding_params->entity_id, unbinding_params->listener_config_index, unbinding_params->listener_stream_index, 0, 0, 0, 0);

	printf("remove entry: EntityID %016"PRIx64" Listener (Config ID %u, unique ID %u) from file %s\n",
		unbinding_params->entity_id, unbinding_params->listener_config_index, unbinding_params->listener_stream_index, binding_filename);
}
 ...
 * \return 	none
 * \param	audio_mappings_file_name	path to the file used for saved audio mappings.
 * \param	config_index			Configuration index for the descriptor concerned by the audio mappings.
 ...
 */
void avdecc_nvm_audio_mappings_update(const char *audio_mappings_file_name, avb_u16 config_index, avb_u16 desc_type, avb_u16 desc_index,
					avb_u16 audio_map_index, unsigned int new_entries_count, struct mapping_entry *new_entries,
					struct audio_mapping_entry *stream_port_audio_mappings)
{
	...
	nvram_update_audio_mappings(audio_mappings_file_name, config_index, desc_type, desc_index,
				    audio_map_index, new_entries_count, new_entries,
				    stream_port_audio_mappings);

	printf("update %s with %u entries for configIndex %hu descType %hu descID %hu audioMapID %hu\n",
		audio_mappings_file_name, new_entries_count, config_index, desc_type, desc_index, audio_map_index);
}
 ...
 * \param	stream_port_audio_mappings	audio mappings of the stream_port descriptor for the current (audio_map_index) audio map.
 */
void avdecc_nvm_audio_mappings_remove(const char *audio_mappings_file_name, avb_u16 config_index, avb_u16 desc_type, avb_u16 desc_index,
					avb_u16 audio_map_index, unsigned int entries_count, struct mapping_entry *removed_entries,
					struct audio_mapping_entry *stream_port_audio_mappings)
{
	...
	nvram_remove_audio_mappings(audio_mappings_file_name, config_index, desc_type, desc_index,
				    audio_map_index, entries_count, removed_entries,
				    stream_port_audio_mappings);

	printf("update %s, removed %u entries for configIndex %hu descType %hu descID %hu audioMapID %hu\n",
		audio_mappings_file_name, entries_count, config_index, desc_type, desc_index, audio_map_index);
}
...
void avdecc_nvm_persistent_params_update(const char *persistent_params_file_name, struct genavb_msg_media_stack_persistent_param *persistent_param)
{
	...
	printf("update %s with entry: EntityID %016"PRIx64" Configuration(%u) Descriptor (%u, %u)\n",
		persistent_params_file_name, persistent_param->entity_id, persistent_param->config_index, persistent_param->desc_type, persistent_param->desc_index);
}

static bool avdecc_is_valid_identify_control_index(int identify_index)
{
	return ((identify_index >= 0 ) && (identify_index < AEM_NUM_CONTROLS_MAX));
}

/** Send a command to the AVDECC stack to start an entity.
 * \return 	0 on success. Otherwise -1.
 * \param	s_avdecc_handle 		handle of the AVDECC control channel
 * \param	entity_id			ID of the avdecc entity to be started (in Network Order). An entity of 0 starts the first non-controller entity.
 * \param	identify_control_index		array of indexes of the identify control descriptors (one per configuration). In the range [0 .. AEM_NUM_CONTROLS_MAX-1] otherwise invalid.
 * \param	identify_indexes_num		number of identify indexes in identify_control_index (maximum of two)
 */
int avdecc_entity_start(struct genavb_control_handle *s_avdecc_handle, avb_u64 entity_id, int *identify_control_index, unsigned int identify_indexes_num)
{
	...
	int rc = GENAVB_SUCCESS, max_configs;
	...
	media_stack_start.entity_id = entity_id;
	media_stack_start.flags = 0;

	/* Supports only a maximum of two configurations. */
	max_configs = (identify_indexes_num < 2) ? identify_indexes_num : 2;

	for (int i = 0; identify_control_index && i < max_configs; i++) {
		if (!avdecc_is_valid_identify_control_index(identify_control_index[i]))
			continue;

		if (i == 0) {
			media_stack_start.identify_control_index = (avb_u16)identify_control_index[i];
			media_stack_start.flags |= GENAVB_ENTITY_START_FLAGS_IDENTIFY_IDX_CFG_0_VALID;
		} else {
			media_stack_start.identify_control_index_cfg_1 = (avb_u16)identify_control_index[i];
			media_stack_start.flags |= GENAVB_ENTITY_START_FLAGS_IDENTIFY_IDX_CFG_1_VALID;
		}
	}

	rc = genavb_control_send(s_avdecc_handle, msg_type, &media_stack_start, msg_len);
	if (rc != GENAVB_SUCCESS) {
		printf("genavb_control_send (GENAVB_CTRL_AVDECC_MEDIA_STACK) failed: %s\n", genavb_strerror(rc));
		goto err;
	}

	printf("Sent entity start: EntityID %016"PRIx64"\n", entity_id);
...
/** Send a command to the AVDECC stack to start the IDENTIFICATION_NOTIFICATION process. As per IEEE 1722.1-2021, 7.5.1.3.
 * \return 	0 on success. Otherwise -1.
 * \param	s_avdecc_handle 	handle of the AVDECC control channel
 * \param	entity_id		ID of the avdecc entity to be started (in Network Order). An entity of 0 starts the first non-controller entity.
 * \param	value			control value for the identification. 0 to stop it, non-zero to start it.
 */
int avdecc_entity_identify(struct genavb_control_handle *s_avdecc_handle, avb_u64 entity_id, avb_u8 value)
{
	genavb_msg_type_t msg_type = GENAVB_MSG_MEDIA_STACK_ENTITY_IDENTIFY;
	struct genavb_msg_media_stack_identify media_stack_identify;
	unsigned int msg_len = sizeof(media_stack_identify);
	int rc = GENAVB_SUCCESS;
	...
	media_stack_identify.entity_id = entity_id;
	media_stack_identify.control_value = value;

	rc = genavb_control_send(s_avdecc_handle, msg_type, &media_stack_identify, msg_len);
	if (rc != GENAVB_SUCCESS) {
		printf("genavb_control_send (GENAVB_CTRL_AVDECC_MEDIA_STACK) failed: %s\n", genavb_strerror(rc));
		goto err;
	}
...
/** Parses the contents of the persistent files, send them to the stack then start the AVDECC entity.
 * \return 	0 on success. Otherwise -1.
 * \param	s_avdecc_handle 		handle of the AVDECC control channel
 * \param	entity_id			ID of the avdecc entity to be started (in Network Order). An entity of 0 starts the first non-controller entity.
 * \param	binding_filename		path to the file used for saved bindingsparameters.
 * \param	persistent_params_file_name	path to the file used for saved persistent parameters.
 * \param	audio_mappings_file_name	path to the file used for saved audio mappings parameters.
 * \param	memory_objects_file_name	path to the file used for configured memory objects.
 * \param	identify_control_index		array of indexes of the identify control descriptors (one per configuration). In the range [0 .. AEM_NUM_CONTROLS_MAX-1] otherwise invalid.
 * \param	identify_indexes_num		number of identify indexes in identify_control_index
 */
int avdecc_entity_start_with_persistent_params(struct genavb_control_handle *s_avdecc_handle, avb_u64 entity_id, const char *binding_filename,
						const char *persistent_params_file_name, const char *audio_mappings_file_name, const char *memory_objects_file_name,
						int *identify_control_index, unsigned int identify_indexes_num)
{
	if (!s_avdecc_handle) {
		printf("Invalid genavb control handle\n");
		goto err;
	}

	/* If we have a filename for non-volatile binding params parse it and init the avdecc stack with its parameters. */
	if (binding_filename) {
		if (avdecc_nvm_bindings_init(s_avdecc_handle, binding_filename) < 0) {
			printf("failed to parse binding file %s\n", binding_filename);
			goto err;
		}
	}

	/* If we have a filename for non-volatile persistent params parse it and init the avdecc stack with its parameters. */
	if (persistent_params_file_name) {
		if (avdecc_nvm_persistent_params_init(s_avdecc_handle, persistent_params_file_name) < 0) {
			printf("failed to parse persistent params file %s\n", persistent_params_file_name);
			goto err;
		}
	}

	/* Parse audio mappings from non-volatile memory and init them with the values saved. */
	/* Audio mappings are mostly handled (update/get/save/restore) by the application */
	if (audio_mappings_file_name) {
		if (avdecc_nvm_audio_mappings_init(s_avdecc_handle, audio_mappings_file_name) < 0)  {
			printf("failed to init audio mappings persistence handling in (%s)\n", audio_mappings_file_name);
			goto err;
		}
	}

	/* Parse the configuration file listing and describing the memory objects available
	 * for the stack to use according to its current Entity
	 */
	if (memory_objects_file_name) {
		if (avdecc_memory_objects_init(s_avdecc_handle, memory_objects_file_name, entity_id) < 0) {
			printf("failed to init memory objects handling in (%s)\n", memory_objects_file_name);
			goto err;
		}
	}

	/* Send a message to start the AVDECC entity after sending all persistent parameters.
	 * An entity ID of 0 starts the first non-controller entity.
	 */
	if (avdecc_entity_start(s_avdecc_handle, entity_id, identify_control_index, identify_indexes_num) < 0) {
		printf("failed to start avdecc entity\n");
		goto err_start;
	}
