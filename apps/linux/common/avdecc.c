...
static int nvram_update_persistent_params(const char *persistent_params_file_name, struct genavb_msg_media_stack_persistent_param *persistent_param)
{
	...
	{
		rc = snprintf(new_entry, NVRAM_ENTRY_MAX_LEN, "%016"PRIx64" %hu %hu %u %." VAL_TO_STR(AEM_STR_LEN_MAX) "s\n",
	...
	while(((read_size = getline(&orig_entry, &len, fOrig)) != -1)) {
		...
		if (sscanf(orig_entry, "%016"PRIx64" %hu %hu %u %" VAL_TO_STR(AEM_STR_LEN_MAX) "[^\n]",
...
static int parse_persistent_params_file(const char *persistent_params_file_name, struct genavb_msg_media_stack_persistent_param *persistent_params, unsigned int size)
{
	...
	while (((read_char = getline(&nvram_entry, &len, fPersist)) != -1)) {
		...
		if (sscanf(nvram_entry, "%016"PRIx64" %hu %hu %u %" VAL_TO_STR(AEM_STR_LEN_MAX) "[^\n]",
 ...
 */
int avdecc_entity_start(struct genavb_control_handle *s_avdecc_handle, avb_u64 entity_id)
{
						...
						const char *persistent_params_file_name, const char *audio_mappings_file_name, const char *memory_objects_file_name)
{
	 ...
	 */
	if (avdecc_entity_start(s_avdecc_handle, entity_id) < 0) {
