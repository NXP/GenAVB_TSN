...
int handle_clock_domain_event(void)
{
	...
	case AVB_MSG_CLOCK_DOMAIN_STATUS:
		...
		if ((msg.status.status == GENAVB_CLOCK_DOMAIN_STATUS_LOCKED) || (msg.status.status == GENAVB_CLOCK_DOMAIN_STATUS_FREE_WHEELING)) {
			INF("GENAVB_MSG_CLOCK_DOMAIN_STATUS - domain: %s, source_type: %s, status: %s, Setting clk domain validity to TRUE",
			    get_genavb_clock_domain_t_name(msg.status.domain), get_genavb_clock_source_type_t_name(msg.status.source_type),
			    get_genavb_clock_domain_status_t_name(msg.status.status));

			set_clk_domain_validity(msg.status.domain, 1);
		}

		if (msg.status.status == GENAVB_CLOCK_DOMAIN_STATUS_UNLOCKED) {
			INF("GENAVB_MSG_CLOCK_DOMAIN_STATUS - domain: %s, source_type: %s, status: %s, Setting clk domain validity to FALSE",
			    get_genavb_clock_domain_t_name(msg.status.domain), get_genavb_clock_source_type_t_name(msg.status.source_type),
			    get_genavb_clock_domain_status_t_name(msg.status.status));
...
int clock_domain_set_source(struct avb_msg_clock_domain_set_source *set_source)
{
