...
int aem_remove_audio_mappings_handler(struct avdecc_controlled *controlled, struct aecp_aem_pdu *pdu)
{
	...
	if (controlled->aem_remove_audio_mappings_handler)
		rc = controlled->aem_remove_audio_mappings_handler(controlled, descriptor_index, descriptor_type, mappings_count, audio_mappings);
...
int aem_add_audio_mappings_handler(struct avdecc_controlled *controlled, struct aecp_aem_pdu *pdu)
{
	...
	if (controlled->aem_add_audio_mappings_handler)
		rc = controlled->aem_add_audio_mappings_handler(controlled, descriptor_index, descriptor_type, mappings_count, audio_mappings);
...
int aem_get_audio_mappings_handler(struct avdecc_controlled *controlled, struct aecp_aem_pdu *pdu, unsigned int *pdu_len)
{
	...
	*pdu_len = sizeof(struct aecp_aem_pdu) + sizeof(struct aecp_aem_get_audio_map_rsp_pdu);
	...
	if (controlled->aem_get_audio_mappings_handler) {
		rc = controlled->aem_get_audio_mappings_handler(controlled, descriptor_index, descriptor_type, map_index, &number_of_maps, &mappings_count, audio_mappings);
...
/** Generic AECP AEM SET_CONFIGURATION command handler.
 *
 * Simply validate the configuration index and store it on the app side.
 * \return		AECP_AEM return code, according to IEEE 1722.1-2013 table 7.126 (AECP_AEM_SUCCESS in case of success).
 * \param controlled	avdecc_controlled context that received the command.
 * \param pdu		Pointer to received AECP AEM PDU.
 */
static int aem_set_configuration_handler(struct avdecc_controlled *controlled, struct aecp_aem_pdu *pdu)
{
	struct aecp_aem_set_configuration_pdu *set_configuration_cmd  = (struct aecp_aem_set_configuration_pdu *)(pdu + 1);
	avb_u16 configuration_index = ntohs(set_configuration_cmd->configuration_index);
	int rc = AECP_AEM_NOT_IMPLEMENTED;

	if (controlled->aem_set_configuration_handler)
		rc = controlled->aem_set_configuration_handler(controlled, configuration_index);

	return rc;
}

int aecp_address_access_command_handler(struct avdecc_controlled *controlled, struct aecp_addr_access_pdu *pdu)
{
	struct aecp_addr_access_tlv *tlv_data = (struct aecp_addr_access_tlv *)(pdu + 1);
	int rc = AECP_ADDRESS_ACCESS_UNSUPPORTED;

	if (controlled->aecp_address_access_handler)
		rc = controlled->aecp_address_access_handler(ntohs(pdu->tlv_count), tlv_data);

	return rc;
}

int avdecc_controlled_handler(void *data, unsigned int events)
{
	struct avdecc_controlled *controlled = (struct avdecc_controlled *)data;
	...
	case AVB_MSG_AECP:
		if (msg.aecp.msg_type == AECP_AEM_COMMAND) {
			...
			printf("%s: Received AVB AECP command %d, current config %u\n", __func__, cmd_type, controlled->current_config_index);
			...
			case AECP_AEM_CMD_SET_CONFIGURATION:
				aem_set_configuration_handler(controlled, pdu);
				/* SET_CONFIGURATION is treated as notification, no need to send back a response. */
				goto exit;
		...
		} else if (msg.aecp.msg_type == AECP_ADDRESS_ACCESS_COMMAND) {
			struct aecp_addr_access_pdu *pdu = (struct aecp_addr_access_pdu *)msg.aecp.buf;

			status = aecp_address_access_command_handler(controlled, pdu);

			msg.aecp.msg_type = AECP_ADDRESS_ACCESS_RESPONSE;
			msg.aecp.status = status;

			rc = avb_control_send(controlled->handle, msg_type, &msg, msg_len);
			if (rc != AVB_SUCCESS) {
				printf("%s: WARNING: Got error message %d(%s) while trying to send response to AECP address access.\n",
					__func__, rc, avb_strerror(rc));
				rc = -1;
			}

		} else {
			printf("%s: Error: Received AVB AECP unsupported message type(%u).\n", __func__, msg.aecp.msg_type);
			rc = -1;
		}
...
int media_thread_loop(struct media_thread *thread)
{
	...
	if (thread->clk_domain.handle) {
		thread->poll_fd[3].fd = avb_control_rx_fd(thread->clk_domain.handle);
		thread->poll_fd[3].events = POLLIN;
		thread->clk_domain.data = thread;
	} else
		thread->poll_fd[3].fd = -1;
	...
	while (1) {
		...
		if (thread->poll_fd[1].revents & POLLIN) {
			if (media_control_handler(&thread->ctrl) != AVB_SUCCESS)
				break;
		...
		if (thread->poll_fd[3].revents & POLLIN) {
