...
{
	PRINT("stats(%p) %s min %d mean %d max %d rms^2 %llu stddev^2 %llu absmin %d absmax %d\n",
	      s, (char *)s->priv, s->min, s->mean, s->max, s->ms, s->variance, s->abs_min, s->abs_max);
}
...
void stats_update(struct stats *s, int val)
{
	...
	s->current_mean += val;
	s->current_ms += (long long)val * val;
...
void stats_compute(struct stats *s)
{
	if (s->current_count) {
		...
		s->variance = s->ms - (s->current_mean * s->current_mean) / ((long long)s->current_count * s->current_count);
...
void hist_print(struct hist *hist)
{
	...
	PRINT("n_slot %d slot_size %d \n", hist->n_slots, hist->slot_size);

	for (i = 0; i < (hist->n_slots + 1); i++)
		_PRINT("%u ", hist->slots[i]);
