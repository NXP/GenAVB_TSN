...
void display_stream_inputs_outputs(struct avb_control_handle *ctrl_h, avb_u64 *entity_id,  avb_u16 configuration_index, avb_u16 descriptor_type, avb_u16 total)
{
	...
	for (i = 0; i < total; i++) {
			...
			if ((rc == AVB_SUCCESS) && (status == AECP_AEM_SUCCESS)) {
				...
				printf("         Stream  %2d: name = %20s    number of formats = %d    flags = 0x%x    current_format = 0x%016" PRIx64 " ",
						i, stream_desc.object_name, ntohs(stream_desc.number_of_formats), ntohs(stream_desc.stream_flags), ntohll(format));
...
void display_entity_info(struct avb_control_handle *ctrl_h, struct entity_info *info)
{
	...
	printf("Entity ID = 0x%" PRIx64 "     Model ID = 0x%" PRIx64 "     MAC address: %02X:%02X:%02X:%02X:%02X:%02X     Capabilities = 0x%x Association ID = 0x%" PRIx64 "\n",
			ntohll(info->entity_id), ntohll(info->entity_model_id),
			info->mac_addr[0], info->mac_addr[1], info->mac_addr[2], info->mac_addr[3], info->mac_addr[4], info->mac_addr[5],
			ntohl(info->entity_capabilities), ntohll(info->association_id));
