...
static void usage (void)
{
	printf("\nUsage:\napp [options]\n");
	printf("\nOptions:\n"
		"\t-m, --mapped-channel <volume ctrl>         Specify alsa volume control (default: DAC1)\n"
		"\t                                           \"dummy\" Can be used as a fake alsa control to have the controlled socket opened (All avdecc controls will be ignored)\n"
		"\t-c, --sound-card-name <sound card name>    Specify sound card name (default: default)\n"
		"\t--ctrl-volume <desc index>                 The index of the Control descriptor, in the Entity model, to be used to control the volume\n"
		"\t-h, --help                                 Print this help text\n");
}

static int volume_control_index = -1;
 ...
 * Descriptor index = volume_control_index
...
int aecp_aem_cmd_rcv(struct aecp_aem_pdu *msg)
{
	...
	case AECP_AEM_CMD_SET_CONTROL:
	{
		...
		/* We only handle volume control in this app */
		if ((volume_control_index < 0) || (descriptor_index != volume_control_index)) {
...
#define CFG_CTRL_VOLUME_VAL	1000

static struct option long_options[] = {
	{"mapped-channel", required_argument, 0, 'm'},
	{"sound-card-name", required_argument, 0, 'c'},
	{"help", no_argument, 0, 'h'},
	{"ctrl-volume", required_argument, 0, CFG_CTRL_VOLUME_VAL},
	{0, 0, 0, 0}
};
...
int main(int argc, char *argv[])
{
	...
	while ((option = getopt_long(argc, argv, "hm:c:", long_options, &long_index)) != -1) {
		switch (option) {
		case 'm':
		...
		case CFG_CTRL_VOLUME_VAL:
			if (h_strtoul(&optval_ul, optarg, NULL, 0) < 0)
				goto exit;

			if (optval_ul >= AEM_NUM_CONTROLS_MAX) {
				printf("[Error]: Volume control index out of range\n");
				goto exit;
			}

			volume_control_index = (int)optval_ul;
