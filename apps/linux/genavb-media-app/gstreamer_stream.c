...
static int timeout_handler(void *data, unsigned int events)
{
...
int talker_gstreamer_connect(struct gstreamer_stream *talker, struct avb_stream_params *params, unsigned int avdecc_stream_index)
{
	...
	if (thread_slot_add(THR_CAP_TIMER, talker->timer_fd, EPOLLIN, talker, &timeout_handler, NULL, 0, (thr_thread_slot_t **)&talker->thread_timer) < 0)
