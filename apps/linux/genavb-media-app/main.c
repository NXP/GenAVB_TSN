...
#ifdef CFG_AVTP_1722A
	while ((option = getopt(argc, argv,"sc:vaBS:d:r:hp:t:f:lLTH:I:FD:A:Mg:m:z:C:P:")) != -1) {
#else
	while ((option = getopt(argc, argv,"sc:vad:r:hp:t:f:lLTFD:A:S:g:m:z:C:P:")) != -1) {
#endif
		switch (option) {
		case 'L':
		...
		case 'P':
			...
			if (current_alsa_config) {
				current_alsa_config->alsa_device = optarg;

				break;
			}

			/* If no specific handler is specified, set for all avdecc talker or listener alsa handlers */
			if (option == 'C')
