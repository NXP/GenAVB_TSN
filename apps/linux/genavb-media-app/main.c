...
static void usage (void)
{
	...
	printf("\nOptions:\n"
		...
		"\t-c <crf handler>      Index of the CRF handler for AVDECC streams or the clock domain (which act as the CRF handler) for static streams\n"
		...
		"\t-s <file name>        The file used to save AEM descriptors' persistent parameters\n"
		"\t-o <file name>        The file used to save the Audio Mappings\n"
...
int main(int argc, char *argv[])
{
...
#ifdef CFG_AVTP_1722A
	while ((option = getopt(argc, argv,"c:vaBS:d:r:hp:t:f:lLTH:I:FD:A:Mg:m:z:C:P:b:s:o:")) != -1) {
#else
	while ((option = getopt(argc, argv,"c:vad:r:hp:t:f:lLTFD:A:S:g:m:z:C:P:b:s:o:")) != -1) {
#endif
		switch (option) {
		case 'L':
	 ...
	 */
	if (avdecc_entity_start_with_persistent_params(ctrl_h, 0, binding_file_name, persistent_params_file_name, audio_mappings_file_name) < 0) {
