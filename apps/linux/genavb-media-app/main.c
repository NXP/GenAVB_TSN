...
#include "../common/audio_mappings.h"
#include "../common/memory_objects.h"
...
#define CONTROL_FIFO_FILE	"/var/run/genavb-media-app_control.fifo"
...
#define MAX_CRF_TALKERS        		6
#define MAX_CRF_LISTENERS      		6

static int signal_terminate = 0;

static struct avb_handle *avb_handle;

static struct alsa_stream alsa_talker_stream[MAX_ALSA_TALKERS] = { [0 ... MAX_ALSA_TALKERS - 1 ].lock = PTHREAD_MUTEX_INITIALIZER };
static struct alsa_stream alsa_listener_stream[MAX_ALSA_LISTENERS] = { [0 ... MAX_ALSA_LISTENERS - 1 ].lock = PTHREAD_MUTEX_INITIALIZER };
static aar_crf_stream_t crf_talker_stream[MAX_CRF_TALKERS];
static aar_crf_stream_t crf_listener_stream[MAX_CRF_LISTENERS];
...
enum control_type {
	CONTROL_INVALID,
	CONTROL_MEDIA_TRACK_NUMBER,
	CONTROL_MEDIA_TRACK_NAME,
	CONTROL_IDENTIFY,
	CONTROL_VOLUME,
	CONTROL_PLAY,
};

/* The app currently supports one control descriptor per valid control type per configuration. */
static int media_track_number_control_index[CFG_MAX_AEM_CONFIGS] = { [0 ... CFG_MAX_AEM_CONFIGS - 1] = -1 };
static int media_track_name_control_index[CFG_MAX_AEM_CONFIGS] = { [0 ... CFG_MAX_AEM_CONFIGS - 1] = -1 };
static int identify_control_index[CFG_MAX_AEM_CONFIGS] = { [0 ... CFG_MAX_AEM_CONFIGS - 1] = -1 };
static int play_control_index[CFG_MAX_AEM_CONFIGS] = { [0 ... CFG_MAX_AEM_CONFIGS - 1] = -1 };

static enum control_type control_types_supported[CFG_MAX_AEM_CONFIGS][AEM_NUM_CONTROLS_MAX] =
{
	[0 ... (CFG_MAX_AEM_CONFIGS - 1)] =
	{
		[0 ... (AEM_NUM_CONTROLS_MAX - 1)] = CONTROL_INVALID
	}
};
...
/*Control FDs :
 * 0- AVB_CTRL_AVDECC_MEDIA_STACK
 * 1- AVB_CTRL_AVDECC_CONTROLLED
 * 2- CLOCK_DOMAIN EVENTS
 * 3- IDENTIFY FIFO
 */
#define NUM_CONTROL_FDS	4
...
struct media_generic_stream {
	...
	unsigned short set_id; /* id of the set. */
	unsigned short set_index; /* index in the set. */
...
{
	int current_control_index = media_track_name_control_index[controlled->current_config_index];
	if ((current_control_index >= 0)
		&& (control_types_supported[controlled->current_config_index][current_control_index] == CONTROL_MEDIA_TRACK_NAME)) {
		aecp_aem_send_set_control_utf8_unsolicited_response(controlled->handle, current_control_index, track);
	}
}

static int aem_set_configuration_handler(struct avdecc_controlled *controlled, avb_u16 config_index)
{
	int rc = AECP_AEM_SUCCESS;

	if (config_index >= MAX_CONFIGURATIONS) {
		printf("Error: AECP Command SET_CONFIGURATION with index (0x%x) not supported in this app\n", config_index);
		rc = AECP_AEM_NOT_SUPPORTED;
		goto exit;
	}

	controlled->current_config_index = config_index;

	printf("controlled(%p): current configuration (%u)\n", controlled, config_index);
exit:
	return rc;
}

static int aem_add_audio_mappings_handler(struct avdecc_controlled *controlled, avb_u16 desc_index, avb_u16 desc_type, avb_u16 mappings_count, void *audio_mappings)
{
	return audio_mappings_add(controlled->current_config_index, desc_index, desc_type, mappings_count, audio_mappings);
}

static int aem_get_audio_mappings_handler(struct avdecc_controlled *controlled, avb_u16 desc_index, avb_u16 desc_type,
					  avb_u16 map_index, avb_u16 *number_of_maps, avb_u16 *mappings_count, void *audio_mappings)
{
	return audio_mappings_get(controlled->current_config_index, desc_index, desc_type, map_index, number_of_maps, mappings_count, audio_mappings);
}

static int aem_remove_audio_mappings_handler(struct avdecc_controlled *controlled, avb_u16 desc_index, avb_u16 desc_type, avb_u16 mappings_count, void *audio_mappings)
{
	return audio_mappings_remove(controlled->current_config_index, desc_index, desc_type, mappings_count, audio_mappings);
}
...
static int aem_set_control_handler(struct avdecc_controlled *controlled, avb_u16 ctrl_index, void *ctrl_value)
{
	...
	if (ctrl_index  >= AEM_NUM_CONTROLS_MAX) {
		printf("AECP Command SET_CONTROL with index (0x%x) exceeds max(%u)\n", ctrl_index, AEM_NUM_CONTROLS_MAX);
		rc = AECP_AEM_NO_SUCH_DESCRIPTOR;
		goto out;
	}

	switch (control_types_supported[controlled->current_config_index][ctrl_index])
	{
	case CONTROL_PLAY:
		if (value) { /* PLAY <--> 255 */
	...
	case CONTROL_MEDIA_TRACK_NUMBER:
		...
		*(avb_u8 *)ctrl_value = value;
		...
		break;
	case CONTROL_IDENTIFY:
		printf("Received SET_CONTROL IDENTIFY command from AVDECC with value(%u)\n", value);

		/* Do something to identify */

		break;

	default:
		printf("AECP Command SET_CONTROL with index (0x%x) not handled/supported in this app, skip\n", ctrl_index);
...
static void avdecc_streams_init(void)
{
	...
	for (i = 0; i < MAX_AVDECC_LISTENERS; i++) {
		...
		listener_streams[i].crf.index = i;
	}
	...
	for (i = 0; i < MAX_AVDECC_TALKERS; i++) {
		...
		talker_streams[i].crf.index = i;
	}
}

static void print_available_multi_handlers(void) {
...
static void usage (void)
{
	...
	printf("\nOptions:\n"
		"\t-L, --listener                                    The following options are for listener config\n"
		"\t-T, --talker                                      The following options are for Talker config\n"
		"\t-A, --avdecc-stream <stream index>                The stream with avdecc stream index\n"
		"\t-S, --static-stream <stream index>                The static stream index\n"
		"\t-g, --gst-handler <gst handler>                   Index of the gstreamer handler\n"
		"\t-m, --gst-multi-handler-io <input:output>         Input and output indexes for the gstreamer multi handler\n"
		"\t-z, --alsa-handler <alsa handler>                 Index of the alsa handler\n"
		"\t-c, --crf-handler <crf handler>                   Index of the CRF handler for AVDECC streams or the clock domain\n"
		"\t                                                  (which act as the CRF handler) for static streams\n"
		"\t-v, --video-rendering                             [L] video rendering of IEC_61883_CIP_FMT_4\n"
		"\t-a, --audio-rendering                             [T/L] For Listener :audio rendering of IEC_61883_CIP_FMT_4\n"
		"\t                                                  For Talker: sending audio stream only over IEC_61883_CIP_FMT_4\n"
		"\t-d, --display-device <device>                     [T/L] display device (lvds (default), hdmi)\n"
		"\t-r, --resolution <resolution>                     [T/L] scaling (1080, 720, 768 (default), 480)\n"
		"\t-p, --presentation-time-offset <offset>           [T/L] For Listener :media stack presentation time offset (in ns)\n"
		"\t                                                  For Talker: render delay for local preview (in ns)\n"
#ifdef CFG_AVTP_1722A
		"\t-M, --mjpeg-camera                                [L] MJPEG camera mode, static stream configuration\n"
		"\t-H, --h264-camera <type>                          [L] H264 camera mode, static stream configuration\n"
		...
		"\t-I, --stream-id <static stream ID>                [L] Stream ID for the H264 static stream configuration\n"
		"\t-F, --free-run                                    [T/L] Free-run mode : Disable sync to clock on gst pipeline video sinks\n"
		"\t-B, --bev                                         [L] BEV mode\n"
#endif
		"\t-f, --input-file <infile name>                    [T] input file name \n"
		"\t-l, --local-preview                               [T] local video preview\n"
		"\t-D, --dump-file <dump_file_location>              [L] dump received stream to given location (Default: /var/avb_listener_dump)\n"
		"\t-C, --alsa-capture-device <alsa_device>           ALSA capture device to be used for either gst handler or alsa handler\n"
		"\t-P, --alsa-playback-device <alsa_device>          ALSA playback device to be used for either gst handler or alsa handler\n"
		"\t-b, --binding-params-file <file name>             The file used to save Milan bindings parameters\n"
		"\t-s, --persistent-params-file <file name>          The file used to save AEM descriptors' persistent parameters\n"
		"\t-o, --audio-mappings-file <file name>             The file used to save the Audio Mappings\n"
		"\t--memory-objects-file <file name>                 The file used to enumerate the actual Memory Objects in the filesystem used by the Entity\n"
		"\t                                                  The file should list and describe each Memory Object on a new line as follow:\n"
		"\t                                                  <cfg index> <obj index> <type id> <max bytes length> <segment max bytes length> <file path>\n"
		"\t                                                  <cfg index> is the index of the corresponding AEM Configuration in which the object should be used\n"
		"\t                                                  <obj index> is the index of the AEM Descriptor matching the object\n"
		"\t                                                  <type id> is, as per IEEE 1722-1 2021 Table 7-19, one of:\n"
		"\t                                                  FIRMWARE_IMAGE(0), VENDOR_SPECIFIC(1), CRASH_DUMP(2), LOG_OBJECT(3), AUTOSTART_SETTINGS(4),\n"
		"\t                                                  SNAPSHOT_SETTINGS(5), SVG_MANUFACTURER(6), SVG_ENTITY(7), SVG_GENERIC(8), PNG_MANUFACTURER(9),\n"
		"\t                                                  PNG_ENTITY(10), PNG_GENERIC(11), DAE_MANUFACTURER(12), DAE_ENTITY(13), DAE_GENERIC(14)\n"
		"\t--ctrl-identify <cfg_index>:<desc_index>          The config and descriptor indexes of the Control descriptor(s), in the AEM, to be\n"
		"\t                                                  used to IDENTIFY the Entity (one descriptor per config)\n"
		"\t--ctrl-play <cfg_index>:<desc_index>              The config and descriptor indexes of the Control descriptor(s), in the AEM, to be\n"
		"\t                                                  used to Play/Stop the stream (one descriptor per config)\n"
		"\t--ctrl-media-track-name <cfg_index>:<desc_index>  The config and descriptor indexes of the Control descriptor(s), in the AEM, to be\n"
		"\t                                                  used to configure the Media Track name (one descriptor per config)\n"
		"\t--ctrl-media-track-num <cfg_index>:<desc_index>   The config and descriptor indexes of the Control descriptor(s), in the AEM, to be\n"
		"\t                                                  used to configure the Media Track number (one descriptor per config)\n"
...
static int controlled_slot_init(thr_thread_slot_t **controlled_thread_slot, struct avdecc_controlled *controlled)
{
	...
	controlled->aem_set_configuration_handler = &aem_set_configuration_handler;
	controlled->aem_get_audio_mappings_handler = &aem_get_audio_mappings_handler;
	controlled->aem_add_audio_mappings_handler = &aem_add_audio_mappings_handler;
	controlled->aem_remove_audio_mappings_handler = &aem_remove_audio_mappings_handler;
	controlled->aecp_address_access_handler = &aecp_address_access_handler;
#define DEFAULT_CONFIGURATION_INDEX	0
	controlled->current_config_index = DEFAULT_CONFIGURATION_INDEX;

	printf("\n %s : Add the controlled data slot \n",__func__);

	if (thread_slot_add(THR_CAP_CONTROLLED, controllled_rx_fd, EPOLLIN, controlled, &avdecc_controlled_handler, NULL, 0, (thr_thread_slot_t **)controlled_thread_slot) < 0)
		goto err;
...
static void talker_connect(struct avb_stream_params *params, unsigned int avdecc_stream_index)
{
	...
	if (avdecc_format_is_crf(&params->format)) {
		aar_crf_stream_t *talker = &crf_talker_stream[avdecc_talker->crf.index];

		if (!crf_connect(talker, params))
			avdecc_talker->handler_type = STREAM_CRF;

	} else if (avdecc_format_is_aaf_pcm(&params->format) ||
		    (avdecc_format_is_61883_6(&params->format) && (avdecc_talker->requested_type != STREAM_GST) && (avdecc_talker->requested_type != STREAM_MULTI_GST))) {
...
static void talker_disconnect(unsigned int stream_index)
{
	...
	case STREAM_CRF:
		crf_disconnect(&crf_talker_stream[avdecc_talker->crf.index]);
		break;
...
static void listener_connect(struct avb_stream_params *params, unsigned int avdecc_stream_index)
{
	...
	avdecc_listener->set_index = params->set_index;
	avdecc_listener->set_id = params->set_id;

	if (avdecc_format_is_crf(&params->format)) {
		aar_crf_stream_t *listener = &crf_listener_stream[avdecc_listener->crf.index];

		if (!crf_connect(listener, params))
			avdecc_listener->handler_type = STREAM_CRF;

	} else if (avdecc_format_is_aaf_pcm(&params->format) ||
		    (avdecc_format_is_61883_6(&params->format) && (avdecc_listener->requested_type != STREAM_GST))) {
		struct alsa_stream *listener = &alsa_listener_stream[avdecc_listener->alsa.index];

		listener->index = avdecc_listener->alsa.index;

		if (!listener_alsa_connect(listener, avdecc_stream_index, params))
...
static void listener_disconnect(int stream_index)
{
	...
	case STREAM_ALSA:
		listener_alsa_disconnect(&alsa_listener_stream[avdecc_listener->alsa.index], stream_index,
						avdecc_listener->set_index);
		break;
	...
	case STREAM_CRF:
		crf_disconnect(&crf_listener_stream[avdecc_listener->crf.index]);
		break;
...
static int multi_talker_timeout_handler(void *data, unsigned int events)
{
	struct gstreamer_talker_multi_handler *talker_multi = (struct gstreamer_talker_multi_handler *)data;
...
static int avdecc_multi_talker_streams_init(void)
{
	...
	for (i = 0; i < MAX_GST_MULTI_HANDLERS; i++) {
		...
		if (thread_slot_add(THR_CAP_TIMER | THR_CAP_GST_MULTI, talker_multi->timer_fd, EPOLLIN, talker_multi, &multi_talker_timeout_handler, NULL, 0, (thr_thread_slot_t **)&talker_multi->thread_timer) < 0)
...
static int handle_avdecc_event(struct avb_control_handle *ctrl_h, const char *binding_file_name, const char *persistent_params_file_name, bool uses_static_crf_streams)
{
	...
	case GENAVB_MSG_MEDIA_SET_CLOCK_SOURCE:
		/* Application supports a single clock domain, apply the restriction on all SET_CLOCK_SOURCE commands anyway. */
		if (uses_static_crf_streams) {
			printf("Ignored GENAVB_MSG_MEDIA_SET_CLOCK_SOURCE: domain(%u) type(%u). Application configured for static media clock domain setup\n",
				msg.media_stack_set_clock_source.domain, msg.media_stack_set_clock_source.source_type);

			break;
		}

		printf("GENAVB_MSG_MEDIA_SET_CLOCK_SOURCE: domain(%u) type(%u)\n", msg.media_stack_set_clock_source.domain, msg.media_stack_set_clock_source.source_type);

		if (clock_domain_set_source(&msg.media_stack_set_clock_source) < 0) {
			printf("Error: couldn't set the clock source for domain(%u)\n", msg.media_stack_set_clock_source.domain);
	...
	case GENAVB_MSG_MEDIA_STACK_BIND:
		printf("GENAVB_MSG_MEDIA_STACK_BIND: Controller (%016"PRIx64") bound listener stream (%016"PRIx64", cfg %u, index %u, %s) to talker stream (%016"PRIx64", %u) \n",
				msg.media_stack_bind.controller_entity_id,
				msg.media_stack_bind.entity_id, msg.media_stack_bind.listener_config_index, msg.media_stack_bind.listener_stream_index,
				(msg.media_stack_bind.started == ACMP_LISTENER_STREAM_STARTED) ? "STARTED" : "STOPPED",
	...
	case GENAVB_MSG_MEDIA_STACK_UNBIND:
		printf("GENAVB_MSG_MEDIA_STACK_UNBIND: listener stream (%016"PRIx64", cfg %u, index %u) has been unbound\n",
				msg.media_stack_unbind.entity_id, msg.media_stack_unbind.listener_config_index, msg.media_stack_unbind.listener_stream_index);
	...
	case GENAVB_MSG_MEDIA_STACK_PERSISTENT_PARAM:
		printf("GENAVB_MSG_MEDIA_STACK_PERSISTENT_PARAM: entity(%016"PRIx64") updated on descriptor(%u, %u)\n",
				msg.media_stack_persistent_param.entity_id, msg.media_stack_persistent_param.desc_type, msg.media_stack_persistent_param.desc_index);

		if (persistent_params_file_name)
			avdecc_nvm_persistent_params_update(persistent_params_file_name, &msg.media_stack_persistent_param);
		break;
...
static int identify_fifo_init(int *fifo_fd)
{
	int rc;

	rc = mkfifo(CONTROL_FIFO_FILE, 0666);
	if (rc < 0 && errno != EEXIST) {
		printf("[Error]: Couldn't create identify FIFO: %s\n", strerror(errno));
		goto err;
	}

	rc = 0;

	*fifo_fd = open(CONTROL_FIFO_FILE, O_RDWR | O_NONBLOCK);
	if (*fifo_fd < 0) {
		printf("[Error]: Couldn't open identify FIFO: %s\n", strerror(errno));
		rc = -1;
		goto err;
	}
...
static void identify_fifo_exit(int *fifo_fd)
{
	if (*fifo_fd >= 0) {
		close(*fifo_fd);
		*fifo_fd = -1;
...
#define CONTROL_FIFO_STR_MAX_LEN	16
/* Handle identify FIFO events to control Entity identification_notification process.
 * Input, in the FIFO, "identify_on" to start it, or "identify_off" to stop it.
 * FIFO is named after CONTROL_FIFO_FILE.
 */
static void handle_control_fifo_event(struct avb_control_handle *ctrl_h, int fifo_fd)
{
	char buffer[CONTROL_FIFO_STR_MAX_LEN] = {0};
	ssize_t bytes_read;

	if (fifo_fd < 0)
		goto err;

	bytes_read = read(fifo_fd, buffer, CONTROL_FIFO_STR_MAX_LEN);
	if (bytes_read == 0) {
		/* Trailing newline or empty read, ignore */
		goto err;

	} else if (bytes_read < 0) {
		printf("[Error]: Reading from Control FIFO: %s\n", strerror(errno));
		goto err;
	}

	buffer[bytes_read - 1] = '\0';

	if (!strcmp("identify_on", buffer)) {
		avdecc_entity_identify(ctrl_h, 0, 255);

	} else if (!strcmp("identify_off", buffer)) {
		avdecc_entity_identify(ctrl_h, 0, 0);

	} else {
		printf("[Error]: Reading from Control FIFO: Unknown command %s\n", buffer);
		goto err;
	}
...
#define CFG_MEMORY_OBJECTS_VAL		1000
#define CFG_CTRL_MEDIA_TRACK_NAME_VAL	1001
#define CFG_CTRL_MEDIA_TRACK_NUM_VAL	1002
#define CFG_CTRL_IDENTIFY_VAL		1003
#define CFG_CTRL_PLAY_VAL		1004

static struct option long_options[] = {
#ifdef CFG_AVTP_1722A
	{"mjpeg-camera", no_argument, 0, 'M'},
	{"bev", no_argument, 0, 'B'},
	{"h264-camera", required_argument, 0, 'H'},
	{"stream-id", required_argument, 0, 'I'},
	{"free-run", no_argument, 0, 'F'},
#endif
	{"crf-handler", required_argument, 0, 'c'},
	{"video-rendering", no_argument, 0, 'v'},
	{"audio-rendering", no_argument, 0, 'a'},
	{"listener", no_argument, 0, 'L'},
	{"talker", no_argument, 0, 'T'},
	{"avdecc-stream", required_argument, 0, 'A'},
	{"static-stream", required_argument, 0, 'S'},
	{"gst-handler", required_argument, 0, 'g'},
	{"gst-multi-handler-io", required_argument, 0, 'm'},
	{"alsa-handler", required_argument, 0, 'z'},
	{"display-device", required_argument, 0, 'd'},
	{"resolution", required_argument, 0, 'r'},
	{"presentation-time-offset", required_argument, 0, 'p'},
	{"input-file", required_argument, 0, 'f'},
	{"local-preview", no_argument, 0, 'l'},
	{"dump-file", required_argument, 0, 'D'},
	{"alsa-capture-device", required_argument, 0, 'C'},
	{"alsa-playback-device", required_argument, 0, 'P'},
	{"binding-params-file", required_argument, 0, 'b'},
	{"persistent-params-file", required_argument, 0, 's'},
	{"audio-mappings-file", required_argument, 0, 'o'},
	{"help", no_argument, 0, 'h'},
	{"memory-objects-file", required_argument, 0, CFG_MEMORY_OBJECTS_VAL},
	{"ctrl-media-track-name", required_argument, 0, CFG_CTRL_MEDIA_TRACK_NAME_VAL},
	{"ctrl-media-track-num", required_argument, 0, CFG_CTRL_MEDIA_TRACK_NUM_VAL},
	{"ctrl-identify", required_argument, 0, CFG_CTRL_IDENTIFY_VAL},
	{"ctrl-play", required_argument, 0, CFG_CTRL_PLAY_VAL},
	{0, 0, 0, 0}
};

static int update_control_descriptor(enum control_type ctrl_type, unsigned int ctrl_config_index, unsigned int ctrl_desc_index)
{
	int *desc_index;

	switch (ctrl_type) {
	case CONTROL_IDENTIFY:
		desc_index = &identify_control_index[ctrl_config_index];
		break;
	case CONTROL_MEDIA_TRACK_NAME:
		desc_index = &media_track_name_control_index[ctrl_config_index];
		break;
	case CONTROL_MEDIA_TRACK_NUMBER:
		desc_index = &media_track_number_control_index[ctrl_config_index];
		break;
	case CONTROL_PLAY:
		desc_index = &play_control_index[ctrl_config_index];
		break;
	default:
		printf("ERROR: Unsupported control type(%d)\n", ctrl_type);
		goto err;
	}

	/* Only keep one Control per type per configuration: If a different index was provided
	 * previously, invalidate it and take the new index
	 */
	if ((*desc_index >= 0) && (control_types_supported[ctrl_config_index][*desc_index] == ctrl_type))
		control_types_supported[ctrl_config_index][*desc_index] = CONTROL_INVALID;

	*desc_index = (int)ctrl_desc_index;

	if (control_types_supported[ctrl_config_index][ctrl_desc_index] != CONTROL_INVALID) {
		printf("[Error]: control (config %u, index %u, type %d) overriding another control index\n",
			ctrl_config_index, ctrl_desc_index, ctrl_type);
		goto err;
	}

	control_types_supported[ctrl_config_index][ctrl_desc_index] = ctrl_type;
...
int main(int argc, char *argv[])
{
	...
	char *config_index_s, *desc_index_s;
	unsigned int config_index, desc_index;
	int current_config_direction = -1;
	char *input_index, *output_index;
	unsigned long long optval_ull;
	unsigned long optval_ul;
	char *persistent_params_file_name = NULL;
	char *audio_mappings_file_name = NULL;
	char *memory_objects_file_name = NULL;
	char *binding_file_name = NULL;
	bool uses_static_crf_streams = false;
	int identify_fifo_fd = -1;
...
#ifdef CFG_AVTP_1722A
	while ((option = getopt_long(argc, argv,"MBH:I:Fc:vaLTA:S:g:m:z:d:r:p:f:lD:C:P:b:s:o:h", long_options, &long_index)) != -1) {
#else
	while ((option = getopt_long(argc, argv,"c:vaLTA:S:g:m:z:d:r:p:f:lD:C:P:b:s:o:h", long_options, &long_index)) != -1) {
#endif
		switch (option) {
		case 'L':
		...
		case 'c':
			...
			if (current_avdecc_stream_index >= 0) {
				if(current_config_direction == AVTP_DIRECTION_TALKER) {
					if (current_crf_handler_index >= MAX_CRF_TALKERS){
						printf("[ERROR] CRF handler index above max (MAX_CRF_TALKERS=%d)\n", MAX_CRF_TALKERS);
						goto err_sched;
					}
					...
					current_crf_config = &crf_talker_stream[current_crf_handler_index];
					current_crf_config->stream_params.direction = AVTP_DIRECTION_TALKER;
					current_crf_config->stream_params.flags = 0;

				} else if (current_config_direction ==  AVTP_DIRECTION_LISTENER) {
					if (current_crf_handler_index >= MAX_CRF_LISTENERS){
						printf("[ERROR] CRF handler index above max (MAX_CRF_LISTENERS=%d)\n", MAX_CRF_LISTENERS);
						goto err_sched;
					}
					...
					current_crf_config = &crf_listener_stream[current_crf_handler_index];
					current_crf_config->stream_params.direction = AVTP_DIRECTION_LISTENER;
					current_crf_config->stream_params.flags = AVB_STREAM_FLAGS_MCR;
		...
		case 'b':
			...
			break;
		case 's':
			persistent_params_file_name = optarg;
			break;
		case 'o':
			audio_mappings_file_name = optarg;
			break;
		case CFG_MEMORY_OBJECTS_VAL:
			memory_objects_file_name = optarg;
			break;
		case CFG_CTRL_MEDIA_TRACK_NAME_VAL:
			config_index_s = strtok(optarg, ":");
			if (!config_index_s) {
				printf("[ERROR] Bad control media track name specification.\n");
				goto err_sched;
			}

			desc_index_s = strtok(NULL, ":");
			if (!desc_index_s) {
				printf("[ERROR] Bad control media track name specification.\n");
				goto err_sched;
			}

			if (h_strtoul(&optval_ul, config_index_s, NULL, 0) < 0)
				goto err_sched;

			config_index = (unsigned int) optval_ul;

			if (h_strtoul(&optval_ul, desc_index_s, NULL, 0) < 0)
				goto err_sched;

			desc_index = (unsigned int)optval_ul;

			if (config_index >= CFG_MAX_AEM_CONFIGS) {
				printf("[Error]: Media track name control config index (%u) out of range\n", config_index);
				goto err_sched;
			}

			if (desc_index >= AEM_NUM_CONTROLS_MAX) {
				printf("[Error]: Media track name control descriptor index (%u) out of range\n", desc_index);
				goto err_sched;
			}

			if (update_control_descriptor(CONTROL_MEDIA_TRACK_NAME, config_index, desc_index) < 0)
				goto err_sched;

			break;
		case CFG_CTRL_MEDIA_TRACK_NUM_VAL:
			config_index_s = strtok(optarg, ":");
			if (!config_index_s) {
				printf("[ERROR] Bad control media track number specification.\n");
				goto err_sched;
			}
			...
			if (config_index >= CFG_MAX_AEM_CONFIGS) {
				printf("[Error]: Media track number control config index (%u) out of range\n", config_index);
				goto err_sched;
			}

			if (desc_index >= AEM_NUM_CONTROLS_MAX) {
				printf("[Error]: Media track number control descriptor index (%u) out of range\n", desc_index);
				goto err_sched;
			}

			if (update_control_descriptor(CONTROL_MEDIA_TRACK_NUMBER, config_index, desc_index) < 0)
				goto err_sched;

			break;
		case CFG_CTRL_IDENTIFY_VAL:
			config_index_s = strtok(optarg, ":");
			if (!config_index_s) {
				printf("[ERROR] Bad control identify specification.\n");
				goto err_sched;
			}
			...
			if (config_index >= CFG_MAX_AEM_CONFIGS) {
				printf("[Error]: Identify control config index (%u) out of range\n", config_index);
				goto err_sched;
			}

			if (desc_index >= AEM_NUM_CONTROLS_MAX) {
				printf("[Error]: Identify control descriptor index (%u) out of range\n", desc_index);
				goto err_sched;
			}

			if (update_control_descriptor(CONTROL_IDENTIFY, config_index, desc_index) < 0)
				goto err_sched;

			break;
		case CFG_CTRL_PLAY_VAL:
			config_index_s = strtok(optarg, ":");
			if (!config_index_s) {
				printf("[ERROR] Bad control play/stop specification.\n");
				goto err_sched;
			}
			...
			if (config_index >= CFG_MAX_AEM_CONFIGS) {
				printf("[Error]: Play/Stop control config index (%u) out of range\n", config_index);
				goto err_sched;
			}

			if (desc_index >= AEM_NUM_CONTROLS_MAX) {
				printf("[Error]: Play/Stop control descriptor index (%u) out of range\n", desc_index);
				goto err_sched;
			}

			if (update_control_descriptor(CONTROL_PLAY, config_index, desc_index) < 0)
				goto err_sched;
	...
	rc = identify_fifo_init(&identify_fifo_fd);
	if (rc < 0)
		goto err_identify_fifo;

	ctrl_poll[3].fd = identify_fifo_fd;
	ctrl_poll[3].events = POLLIN;
	...
	/* Parse all persistent params and send them to avdecc, then start the entity.
	 * An entity ID of 0 starts the first non-controller entity.
	 * Note: start avdecc entity after opening all needed control channels.
	 */
	if (avdecc_entity_start_with_persistent_params(ctrl_h, 0, binding_file_name, persistent_params_file_name,
							audio_mappings_file_name, memory_objects_file_name, identify_control_index, CFG_MAX_AEM_CONFIGS) < 0) {
		printf("failed to start avdecc entity with persistent params\n");
		rc = -1;
		goto err_channel;
	}
	...
	while (1) {
		...
		if (ctrl_poll[0].revents & POLLIN)
			handle_avdecc_event(ctrl_h, binding_file_name, persistent_params_file_name, uses_static_crf_streams);

		if (ctrl_poll[2].revents & POLLIN)
			handle_clock_domain_event();

		if (ctrl_poll[3].revents & POLLIN)
			handle_control_fifo_event(ctrl_h, identify_fifo_fd);
	}
...
err_clock_domain_init:
	identify_fifo_exit(&identify_fifo_fd);
