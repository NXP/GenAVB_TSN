...
void stream_init_stats(struct gstreamer_stream *stream)
{
	...
	stream->pipe_source.tlast = time(NULL);
...
void dump_stream_infos(struct gstreamer_stream *stream)
{
	...
	if(stream->params.format.u.s.subtype != AVTP_SUBTYPE_CVF)
...
int gst_cvf_mjpeg_warm_up_pipeline(struct gstreamer_pipeline *gst)
{
	...
	rc = read(splash_fd, &frame_size, 4);
	if (rc != 4) {
	...
	rc = read(splash_fd, buf, frame_size);
	if (rc != frame_size) {
		printf("Short read while warming up Gstreamer pipeline (%d, expected %d)\n", rc, frame_size);
		rc = -1;
		goto exit;
	...
	count = 0;
	while (read(splash_fd, &frame_size, 4) == 4) {
		printf ("Reading frame %d size %d ts %llu base_time %" G_GUINT64_FORMAT " now %" G_GUINT64_FORMAT "\n",
				count, frame_size, gsttime, gst_element_get_base_time(GST_ELEMENT(gst->pipeline)), gst_clock_get_time(gst->clock));
		rc = read(splash_fd, buf, frame_size);
		if (rc != frame_size) {
			printf("Short read while warming up Gstreamer pipeline (%d, expected %d)\n", rc, frame_size);
			rc = -1;
		...
		for (i = 0; i < gst->u.listener.num_sources; i++) {
			...
			GST_BUFFER_PTS(buffer) = gsttime + SALSA_LATENCY; // Arbitrary delay

			rc = gst_app_src_push_buffer(gst->u.listener.source[i].source, buffer);
			if (rc != GST_FLOW_OK) {
		...
		if (event_len != 0) {
			...
			if (event[event_len - 1].event_mask & AVTP_END_OF_FRAME) {
				...
				if (!(event[event_len - 1].event_mask & AVTP_TIMESTAMP_INVALID)) {
					ts_diff = (avb_s32)event[0].ts - (avb_s32)(gsttime & 0xffffffff);
					if ((ts_diff > -CVF_TS_VALID_WINDOW) && (ts_diff < CVF_TS_VALID_WINDOW)) {
						GST_BUFFER_PTS(stream->pipe_source.buffer) = genavb_avtp_ts_32to64(event[0].ts, gsttime);
					} else {
						GST_BUFFER_PTS(stream->pipe_source.buffer) = gsttime;
				...
				GST_BUFFER_PTS(stream->pipe_source.buffer) += stream->pipe_source.gst_pipeline->pts_offset + stream->pipe_source.gst_pipeline->local_pts_offset;
				GST_BUFFER_PTS(stream->pipe_source.buffer) += (avb_u64)33333333- (GST_BUFFER_PTS(stream->pipe_source.buffer) % (avb_u64)33333333);
				...
				if (GST_BUFFER_PTS(stream->pipe_source.buffer) < gst->basetime) {
					printf("stream(%u): frame in the past, dropping (frame %" G_GUINT64_FORMAT ", basetime %llu now %llu)\n",
				...
				if (!stream->pipe_source.dropping) {
					...
					//printf("Pushed %d bytes to Gstreamer ts %llu\n", stream->pipe_source.buffer_byte_count, GST_BUFFER_PTS(stream->pipe_source.buffer));
...
int listener_gst_handler(struct gstreamer_stream *stream, unsigned int events)
{
	...
	while (pushed != stream->batch_size) {
		...
		if (!(event[0].event_mask & (AVTP_TIMESTAMP_INVALID | AVTP_TIMESTAMP_UNCERTAIN))) {
			...
			GST_BUFFER_PTS(buffer) = genavb_avtp_ts_32to64(event[0].ts, gsttime);
			...
			GST_BUFFER_PTS(buffer) += stream->pipe_source.gst_pipeline->local_pts_offset;
...
void apply_config(struct gstreamer_stream *stream, struct avb_stream_params *stream_params)
{
	...
	stream->params.clock_domain = AVB_MEDIA_CLOCK_DOMAIN_STREAM;
