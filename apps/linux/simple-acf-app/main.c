...
static void set_stream_params(struct genavb_stream_params *stream_params)
{
	if (app.mode == MODE_TALKER) {
		...
		stream_params->clock_domain = GENAVB_CLOCK_DOMAIN_0;
	...
	stream_params->flags = GENAVB_STREAM_FLAGS_CUSTOM_TSPEC | GENAVB_STREAM_FLAGS_SET_ID_VALID;
	stream_params->set_id = default_set_id;
	stream_params->set_index = 0;
