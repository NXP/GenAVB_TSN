...
#define LISTENER_MAX_FDS	3
static int listener_nonblocking(struct avb_stream_handle *stream_h, int stream_fd, int clk_domain_rx_fd, unsigned int batch_size, int file_dst)
{
	...
	/*
	 * Listen to read events from the stack and clock domain events
	 */
...
#define TALKER_MAX_FDS	3
static int talker_nonblocking(struct avb_stream_handle *stream_h, int stream_fd, int clk_domain_rx_fd, unsigned int batch_size, int file_src)
{
	struct pollfd poll_fds[TALKER_MAX_FDS];
...
loop:
	...
	/*
	 * Listen to write events from the stack and clock domain events
	 */
	poll_fds[0].fd = stream_fd;
...
static int run_listener(struct avb_stream_handle *stream_h, int stream_fd, int clk_domain_rx_fd, unsigned int batch_size)
{
	int rc;
	...
	if (app.config & FLAG_BLOCKING)
		...
		rc = listener_nonblocking(stream_h, stream_fd, clk_domain_rx_fd, batch_size, app.media_fd);

	return rc;
}


static int run_talker(struct avb_stream_handle *stream_h, int stream_fd, int clk_domain_rx_fd, unsigned int batch_size)
{
	int rc;
	...
	if (app.config & FLAG_BLOCKING)
		...
		rc = talker_nonblocking(stream_h, stream_fd, clk_domain_rx_fd, batch_size, app.media_fd);
...
wait_new_stream:
	...
	if (app.mode == MODE_AVDECC) {
		...
		ctrl_poll[1].fd = clk_domain_rx_fd;
	 ...
	 */
	if (app.stream_params.direction & AVTP_DIRECTION_TALKER)
		rc = run_talker(stream_h, stream_fd, clk_domain_rx_fd, app.stream_batch_size);
	else
		rc = run_listener(stream_h, stream_fd, clk_domain_rx_fd, app.stream_batch_size);
