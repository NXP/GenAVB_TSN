...
u8 acmp_listener_stack_connect(struct entity *entity, u16 listener_unique_id, u16 flags)
{
	...
	if (avdecc_stream_input_is_clock_source(entity, listener_unique_id))
		avdecc_ipc_set_clock_source(avdecc, &avdecc->ipc_tx_media_stack, stream_input->clock_domain_index,
					    AEM_CLOCK_SOURCE_TYPE_INPUT_STREAM, 0, stream_input_dynamic->stream_id);
exit:
	return rc;
}
...
u8 acmp_listener_stack_disconnect(struct entity *entity, u16 listener_unique_id)
{
	...
	if (avdecc_stream_input_is_clock_source(entity, listener_unique_id))
		avdecc_ipc_set_clock_source(avdecc, &avdecc->ipc_tx_media_stack, stream_input->clock_domain_index,
					    AEM_CLOCK_SOURCE_TYPE_INPUT_STREAM, 0, STREAM_INPUT_ID_INVALID);
