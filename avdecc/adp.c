...
int adp_discovery_send_packet(struct adp_discovery_ctx *disc, u8 *entity_id)
{
	...
	if (entity_id) {
		...
		entity = avdecc_get_entity(avdecc, *(u64 *)entity_id);
...
int adp_advertise_send_packet(struct adp_ctx *adp, u8 message_type, unsigned int port_id)
{
	...
	if (entity->milan_mode)
		adp_hdr->available_index = entity->dynamic_desc->u.milan.available_index[port_id];
	else
	...
	adp_hdr->current_configuration_index = entity->desc->current_configuration;
	adp_hdr->identity_control_index = htons(avdecc_get_current_idenitfy_index(entity));
...
static inline void adp_discovery_put(struct entity_discovery *entity_disc)
{
	int i;
...
void adp_discovery_update(struct adp_discovery_ctx *disc, struct adp_pdu *pdu, u8 valid_time, u8 *mac_src)
{
	...
	timer_start(&entity_disc->timeout, genavb_max(1, valid_time * 2) * MS_PER_S);
...
int adp_current_config_init(struct adp_ctx *adp)
{
	struct entity *entity = container_of(adp, struct entity, adp);

	/* Configuration change is not supported for legacy avdecc mode. */
	if (!entity->milan_mode)
		goto err;
...
err_disc:
	adp_milan_advertise_exit(adp, false);
err:
	return -1;
}
...
__init int adp_init(struct adp_ctx *adp)
{
	...
	if (!entity->milan_mode) {
		...
		if (adp_current_config_init(adp) < 0)
			goto err;
	}
...
void adp_current_config_exit(struct adp_ctx *adp, bool config_change)
{
	struct entity *entity = container_of(adp, struct entity, adp);

	/* TODO: add configuration change support for legacy avdecc mode. */
	if (!entity->milan_mode)
		return;

	adp_milan_advertise_exit(adp, config_change);
...
__exit void adp_exit(struct adp_ctx *adp)
{
	...
	if (!entity->milan_mode) {
		...
		adp_current_config_exit(adp, false);
	}
}
