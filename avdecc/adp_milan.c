...
static const char *adp_milan_advertise_event2string(adp_milan_advertise_event_t event)
{
	...
	case2str(ADP_MILAN_ADV_SHUTDOWN_CONFIG_CHANGE);
	default:
...
static int adp_milan_advertise_send_available(struct entity *entity, unsigned int port_id)
{
	...
	entity->dynamic_desc->u.milan.available_index[port_id] = htonl(ntohl(entity->dynamic_desc->u.milan.available_index[port_id]) + 1);
...
int adp_milan_advertise_sm(struct entity *entity, unsigned int port_id, adp_milan_advertise_event_t event)
{
	...
	case ADP_MILAN_ADV_NOT_STARTED:
		...
		case ADP_MILAN_ADV_START:
			...
			if (!(avb_interface_dynamic->operational_state)) {
				...
				timer_start(&avb_interface_dynamic->u.milan.adp_delay_timer,
						random_range(ADP_MILAN_ADV_TMR_DELAY_MS_INIT_MIN, ADP_MILAN_ADV_TMR_DELAY_MS_INIT_MAX)); /* random between 0.1 s and 2 sec */
	...
	case ADP_MILAN_ADV_WAITING:
		...
		case ADP_MILAN_ADV_SHUTDOWN_CONFIG_CHANGE:
			/* 9.3.5.8 */
			timer_stop(&avb_interface_dynamic->u.milan.adp_advertise_timer);

			/* Custom event: Do not send ENTITY_DEPARTING ADPDU on a change of AEM configurations. */
			if (event != ADP_MILAN_ADV_SHUTDOWN_CONFIG_CHANGE) {
				/* Per IEEE1722.1-2013 6.2.1.16 */
				entity->dynamic_desc->u.milan.available_index[port_id] = htonl(0);
...
int adp_milan_listener_sink_discovery_sm(struct entity *entity, u16 listener_unique_id, adp_milan_listener_sink_event_t event, u8 valid_time, struct adp_pdu *pdu)
{
	...
	case ADP_LISTENER_SINK_TALKER_STATE_NOT_DISCOVERED:
		...
		case ADP_MILAN_LISTENER_SINK_RCV_ADP_AVAILABLE:
			...
			timer_start(&stream_input_dynamic->u.milan.adp_discovery_timer, genavb_max(1, valid_time) * MS_PER_S);
...
/* Advertise */
__exit int adp_milan_advertise_exit(struct adp_ctx *adp, bool config_change)
{
	int num_interfaces, i;
	struct entity *entity = container_of(adp, struct entity, adp);
	...
	if (entity_ready(entity)) {
		for (i = 0; i < num_interfaces; i++) {
			adp_milan_advertise_sm(entity, i, config_change ? ADP_MILAN_ADV_SHUTDOWN_CONFIG_CHANGE : ADP_MILAN_ADV_SHUTDOWN);
