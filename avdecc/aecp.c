...
static const char *aecp_aem_desctype2string(unsigned short desc_type)
{
	switch (desc_type) {
	case2str(AEM_DESC_TYPE_ENTITY);
	case2str(AEM_DESC_TYPE_CONFIGURATION);
	case2str(AEM_DESC_TYPE_AUDIO_UNIT);
	case2str(AEM_DESC_TYPE_VIDEO_UNIT);
	case2str(AEM_DESC_TYPE_SENSOR_UNIT);
	case2str(AEM_DESC_TYPE_STREAM_INPUT);
	case2str(AEM_DESC_TYPE_STREAM_OUTPUT);
	case2str(AEM_DESC_TYPE_JACK_INPUT);
	case2str(AEM_DESC_TYPE_JACK_OUTPUT);
	case2str(AEM_DESC_TYPE_AVB_INTERFACE);
	case2str(AEM_DESC_TYPE_CLOCK_SOURCE);
	case2str(AEM_DESC_TYPE_MEMORY_OBJECT);
	case2str(AEM_DESC_TYPE_LOCALE);
	case2str(AEM_DESC_TYPE_STRINGS);
	case2str(AEM_DESC_TYPE_STREAM_PORT_INPUT);
	case2str(AEM_DESC_TYPE_STREAM_PORT_OUTPUT);
	case2str(AEM_DESC_TYPE_EXTERNAL_PORT_INPUT);
	case2str(AEM_DESC_TYPE_EXTERNAL_PORT_OUTPUT);
	case2str(AEM_DESC_TYPE_INTERNAL_PORT_INPUT);
	case2str(AEM_DESC_TYPE_INTERNAL_PORT_OUTPUT);
	case2str(AEM_DESC_TYPE_AUDIO_CLUSTER);
	case2str(AEM_DESC_TYPE_VIDEO_CLUSTER);
	case2str(AEM_DESC_TYPE_SENSOR_CLUSTER);
	case2str(AEM_DESC_TYPE_AUDIO_MAP);
	case2str(AEM_DESC_TYPE_VIDEO_MAP);
	case2str(AEM_DESC_TYPE_SENSOR_MAP);
	case2str(AEM_DESC_TYPE_CONTROL);
	case2str(AEM_DESC_TYPE_SIGNAL_SELECTOR);
	case2str(AEM_DESC_TYPE_MIXER);
	case2str(AEM_DESC_TYPE_MATRIX);
	case2str(AEM_DESC_TYPE_MATRIX_SIGNAL);
	case2str(AEM_DESC_TYPE_SIGNAL_SPLITTER);
	case2str(AEM_DESC_TYPE_SIGNAL_COMBINER);
	case2str(AEM_DESC_TYPE_SIGNAL_DEMULTIPLEXER);
	case2str(AEM_DESC_TYPE_SIGNAL_MULTIPLEXER);
	case2str(AEM_DESC_TYPE_SIGNAL_TRANSCODER);
	case2str(AEM_DESC_TYPE_CLOCK_DOMAIN);
	case2str(AEM_DESC_TYPE_CONTROL_BLOCK);
	default:
		return (char *) "Unknown descriptor type";
	}
}
...
static int aecp_aem_control_desc_to_pdu(struct control_descriptor *desc, void *raw_value, u16 len)
{
	...
	case AEM_CONTROL_LINEAR_UINT8:
	{
		...
		for (i = 0; i < count; i++)
			values[i] = desc->value_details.linear_int8[i].current_val;
...
static int aecp_aem_control_pdu_to_desc(struct control_descriptor *desc, void *raw_value, u16 len)
{
	...
	case AEM_CONTROL_LINEAR_UINT8:
	{
		...
		for (i = 0; i < ntohs(desc->number_of_values); i++) {
			...
			if (desc->value_details.linear_int8[i].current_val != values[i]) {
				desc->value_details.linear_int8[i].current_val = values[i];
				rc = 1;
			}
		}
...
static int aecp_ipc_send_persistent_param(struct entity *entity, u16 descriptor_type, u16 descriptor_index, u16 parameter_type, void *parameter_value)
{
	...
	if (desc) {
		...
		desc->u.media_stack_persistent_param.config_index = ntohs(entity->desc->current_configuration);
...
{
	u16 descriptor_type, descriptor_index, configuration_index;
	struct aem_desc_hdr *aem_descs, *aem_dynamic_descs;
	int rc = 0;
	...
	configuration_index = persistent_param->config_index;

	if (!avdecc_is_valid_configuration_index(entity, configuration_index)) {
		os_log(LOG_ERR, "entity(%p) persistent_param(%u, 0x%x): invalid configuration_index(%u)\n",
			entity, descriptor_index, descriptor_type, configuration_index);
		rc = -1;
		goto err;
	}

	aem_descs = entity->aem_avail_descs[configuration_index];
	aem_dynamic_descs = entity->aem_avail_dynamic_descs[configuration_index];

	switch (descriptor_type) {
	case AEM_DESC_TYPE_ENTITY:
	{
		struct entity_descriptor *entity_desc = aem_get_descriptor(aem_descs, AEM_DESC_TYPE_ENTITY, descriptor_index, NULL);
	...
	{
		struct configuration_descriptor *configuration_desc = aem_get_descriptor(aem_descs, AEM_DESC_TYPE_CONFIGURATION, descriptor_index, NULL);
	...
	{
		struct stream_descriptor *stream_input_desc = aem_get_descriptor(aem_descs, AEM_DESC_TYPE_STREAM_INPUT, descriptor_index, NULL);
		unsigned int i;
	...
	{
		struct stream_descriptor *stream_output_desc = aem_get_descriptor(aem_descs, AEM_DESC_TYPE_STREAM_OUTPUT, descriptor_index, NULL);
		struct stream_output_dynamic_desc *stream_output_dynamic = aem_get_descriptor(aem_dynamic_descs, AEM_DESC_TYPE_STREAM_OUTPUT, descriptor_index, NULL);
		unsigned int i;
	...
	{
		struct avb_interface_descriptor *avb_interface_desc = aem_get_descriptor(aem_descs, AEM_DESC_TYPE_AVB_INTERFACE, descriptor_index, NULL);
	...
	{
		struct clock_source_descriptor *clock_source_desc = aem_get_descriptor(aem_descs, AEM_DESC_TYPE_CLOCK_SOURCE, descriptor_index, NULL);
	...
	{
		struct clock_domain_descriptor *clock_domain_desc = aem_get_descriptor(aem_descs, AEM_DESC_TYPE_CLOCK_DOMAIN, descriptor_index, NULL);
		...
		case AVDECC_PERSISTENT_PARAM_CLOCK_SOURCE_INDEX:
			...
			clock_source_desc = aem_get_descriptor(aem_descs, AEM_DESC_TYPE_CLOCK_SOURCE, clock_source_index, NULL);
			if (!clock_source_desc) {
			...
			if (clock_source_type == AEM_CLOCK_SOURCE_TYPE_INPUT_STREAM) {
				stream_input_desc = aem_get_descriptor(aem_descs, AEM_DESC_TYPE_STREAM_INPUT, clock_source_location_index, NULL);
				if (!stream_input_desc) {
	...
	{
		struct audio_unit_descriptor *audio_unit_desc = aem_get_descriptor(aem_descs, AEM_DESC_TYPE_AUDIO_UNIT, descriptor_index, NULL);
	...
	{
		struct audio_cluster_descriptor *audio_cluster_desc = aem_get_descriptor(aem_descs, AEM_DESC_TYPE_AUDIO_CLUSTER, descriptor_index, NULL);
	...
	{
		struct control_descriptor *control_desc = aem_get_descriptor(aem_descs, AEM_DESC_TYPE_CONTROL, descriptor_index, NULL);
	...
	{
		struct jack_descriptor *jack_desc = aem_get_descriptor(aem_descs, descriptor_type, descriptor_index, NULL);

		if (!jack_desc) {
			os_log(LOG_ERR, "entity(%p) invalid jack descriptor(type %u, index %u)\n", entity, descriptor_type, descriptor_index);
			rc = -1;
			goto err;
		}
		...
		switch (persistent_param->param_type) {
		case AVDECC_PERSISTENT_PARAM_OBJECT_NAME:
			os_memcpy(jack_desc->object_name, persistent_param->u.name, AEM_STR_LEN_MAX);
			break;
...
int aecp_update_memory_object(struct entity *entity, struct genavb_msg_memory_object_desc_attr *memory_object)
{
	...
	u16 descriptor_index, config_index;
	int rc = 0;
	...
	config_index = memory_object->cfg_idx;
	if (!avdecc_is_valid_configuration_index(entity, config_index)) {
		os_log(LOG_ERR, "entity(%p) invalid memory object configuration index(%u)\n", entity, config_index);
		rc = -1;
		goto err;
	}

	descriptor_index = memory_object->desc_idx;
	memory_object_desc = aem_get_descriptor(entity->aem_avail_descs[config_index], AEM_DESC_TYPE_MEMORY_OBJECT, descriptor_index, NULL);
	if (!memory_object_desc) {
		os_log(LOG_ERR, "entity(%p) invalid memory object descriptor(cfg %u, index %u)\n", entity, config_index, descriptor_index);
		rc = -1;
		goto err;
	}

	if (memory_object->object_type != ntohs(memory_object_desc->memory_object_type)) {
		os_log(LOG_ERR, "entity(%p) memory object(cfg %u, index %u): object type mismatch (%u) vs expected (%u)\n",
			entity, config_index, descriptor_index, memory_object->object_type, ntohs(memory_object_desc->memory_object_type));
		rc = -1;
		goto err;
	}
	...
	if ((memory_object->max_segment_len > AEM_MEMORY_OBJECT_START_ADDR_MASK) ||
		...
		os_log(LOG_ERR, "entity(%p) memory object(cfg %u, index %u): Invalid len(%"PRIu64") max_len(%"PRIu64") max_segment_len(%"PRIu64")\n",
			entity, config_index, descriptor_index, memory_object->len, memory_object->max_len, memory_object->max_segment_len);
		rc = -1;
		goto err;
	}
...
static u8 aecp_aem_get_name_response(struct entity *entity, void *aecp_rsp_specific_data, u16 *len, u16 descriptor_type, u16 descriptor_index, u16 name_index, u16 configuration_index)
{
	...
	*len += sizeof(struct aecp_aem_get_name_rsp_pdu);
	...
	desc = aem_get_descriptor(entity->aem_avail_descs[configuration_index], descriptor_type, descriptor_index, NULL);
	if (!desc) {
		status = AECP_AEM_NO_SUCH_DESCRIPTOR;
		goto exit;
	}
...
/** Send a multicast unsolicited IDENTIFY_NOTIFICATION on all AVDECC ports
 * \return None.
 * \param entity	Pointer to the Entity's context.
 * \param sequence_id	Sequence ID of the notification.
 */
static void aecp_aem_send_identify_unsolicited_notification(struct entity *entity, u16 sequence_id)
{
	struct aecp_aem_identify_notification_pdu *aecp_rsp;
	u64 controller_entity_id = htonll(AECP_IDENTIFY_CONTROLLER_ID);
	u8 mac_dst[6] = MC_ADDR_MULTICAST_IDENTIFY;
	struct aecp_ctx *aecp = &entity->aecp;
	u16 len	= sizeof(struct aecp_aem_pdu);
	struct aecp_aem_pdu *aecp_pdu;
	struct net_tx_desc *net_desc;
	struct avdecc_port *port_tx;
	unsigned int i;
	void *buf;

	for (i = 0; i < entity->avdecc->port_max; i++) {
		port_tx = &entity->avdecc->port[i];
		if (!port_tx->initialized)
			continue;

		net_desc = net_tx_alloc(&port_tx->net_tx, DEFAULT_NET_DATA_SIZE);
		if (!net_desc) {
			os_log(LOG_ERR,"aecp(%p) port(%u): Cannot alloc net_tx\n", aecp, port_tx->port_id);
			continue;
		}
		...
		AECP_AEM_SET_U_CMD_TYPE(aecp_pdu, 1, AECP_AEM_CMD_IDENTIFY_NOTIFICATION);

		aecp_rsp = (struct aecp_aem_identify_notification_pdu *)(aecp_pdu + 1);

		aecp_rsp->descriptor_index = htons(avdecc_get_current_idenitfy_index(entity));
		aecp_rsp->descriptor_type = htons(AEM_DESC_TYPE_CONTROL);

		len += sizeof(struct aecp_aem_identify_notification_pdu);

		if (aecp_aem_send_response(aecp, port_tx, aecp_pdu, net_desc, controller_entity_id, sequence_id, AECP_AEM_SUCCESS, 1, mac_dst, len) < 0) {
			os_log(LOG_ERR,"aecp(%p) port(%u): Couldn't send IDENTIFY_NOTIFICATION to controller(%016"PRIx64").\n",
					aecp, port_tx->port_id, ntohll(controller_entity_id));
			continue;
		}
	}
...
/** Handle the IDENTIFY_NOTIFICATION state machine as per IEEE 1722.1-2021, 7.5.1.3
 * \return None.
 * \param entity	Pointer to the Entity's context.
 * \param event		Event that triggered the state machine (BUTTON_PRESSED or TIMEOUT).
 */
void aecp_aem_identify_sm(struct entity *entity, identify_event_t event)
{
	struct aecp_ctx *aecp = &entity->aecp;
	struct identify_sm *sm = &aecp->identify_sm;
	identify_state_t current_state;
...
start:
	...
	{
	case IDENTIFY_STATE_INITIALIZE:
		switch (event)
		{
		case IDENTIFY_EVENT_BEGIN:
			sm->sequence_id = 0;
			sm->identify_to_send = 3;
			sm->button_pressed = false;
			sm->state = IDENTIFY_STATE_WAITING;
			break;
	...
	case IDENTIFY_STATE_WAITING:
		switch (event)
		{
		case IDENTIFY_EVENT_RUN:
			if (sm->button_pressed)
				sm->state = IDENTIFY_STATE_IDENTIFY;
	...
	case IDENTIFY_STATE_IDENTIFY:
		switch (event)
		{
		case IDENTIFY_EVENT_RUN:
			if (timer_is_running(&sm->timeout)) {
				if (!sm->button_pressed) {
					timer_stop(&sm->timeout);
			...
			/* Align handling of tx identify with the spec:
			 * - Treat txIdentify() as atomic: Once started always send a burst of 3 packets with the same identifySequenceID
			 * - Increment identifySequenceID after the burst (3 packets) transmit
			 * - An identify button unpressed would wait for the burst transmit to finish.
			 */
			if (!timer_is_running(&sm->identify_tx)) {
				aecp_aem_send_identify_unsolicited_notification(entity, sm->sequence_id);

				sm->identify_to_send--;

				if (sm->identify_to_send == 0) {
					sm->identify_to_send = 3;
					sm->sequence_id++;
					...
					if (!sm->button_pressed)
						...
						timer_start(&sm->timeout, AECP_IDENTIFY_TIMEOUT_MS);

				} else {
					timer_start(&sm->identify_tx, AECP_IDENTIFY_TRANSMIT_DELAY_MS);
				}
			}
	...
	if (current_state != sm->state) {
		event = IDENTIFY_EVENT_RUN;
		goto start;
	}
}

static void aecp_identify_timeout_handler(void *data)
{
...
static struct memory_object_descriptor *aecp_get_memory_object_by_addr(struct entity *entity, u64 addr)
{
	...
	if (!avdecc_is_valid_configuration_index(entity, cfg_idx)) {
		os_log(LOG_ERR, "entity(%p) addr(0x%"PRIx64") invalid configuration index(%u)\n", entity, addr, cfg_idx);
		goto err;
	}

	memory_obj_desc = aem_get_descriptor(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_MEMORY_OBJECT, desc_idx, NULL);
	if (!memory_obj_desc) {
...
static int aecp_aem_received_controller_command(struct aecp_ctx *aecp, struct aecp_aem_pdu *pdu, u16 avtp_len, u8 *mac_src, unsigned int port_id)
{
	...
	case AECP_AEM_CMD_READ_DESCRIPTOR:
	{
		...
		u16 config_index = ntohs(read_desc_cmd->configuration_index);
		u16 descriptor_type = ntohs(read_desc_cmd->descriptor_type);
		u16 descriptor_index = ntohs(read_desc_cmd->descriptor_index);
		void *desc;
		u16 desc_len;

		os_log(LOG_DEBUG, "aecp(%p) command (%x, %s) config(%u) descriptor(%x, %s) index(0x%x)\n",
			aecp, cmd_type, aecp_aem_cmdtype2string(cmd_type), config_index,
			descriptor_type, aecp_aem_desctype2string(descriptor_type),
			descriptor_index);
		...
		desc = aem_get_descriptor(entity->aem_avail_descs[config_index], descriptor_type, descriptor_index, &desc_len);
		if (desc) {
			struct aecp_aem_read_desc_rsp_pdu *read_desc_rsp = (struct aecp_aem_read_desc_rsp_pdu *)(aecp_rsp + 1);

			len += avdecc_desc_to_network((u8 *)(read_desc_rsp + 1), (struct aem_descriptor_common *)desc, descriptor_type, desc_len);

			read_desc_rsp->configuration_index = read_desc_cmd->configuration_index;
			...
			read_desc_fail->configuration_index = read_desc_cmd->configuration_index;
	...
	case AECP_AEM_CMD_SET_CONFIGURATION:
	{
		...
		os_log(LOG_INFO, "received SET_CONFIGURATION change from current(%u) to new index(%u)\n",
			ntohs(entity_desc->current_configuration), ntohs(set_configuration_cmd->configuration_index));
		...
		if (!avdecc_is_valid_configuration_index(entity, ntohs(set_configuration_cmd->configuration_index))) {
			status = AECP_AEM_BAD_ARGUMENTS;
			break;
		}
		...
		if (entity->desc->controller_capabilities & htonl(ADP_CONTROLLER_IMPLEMENTED)) {
			os_log(LOG_ERR, "entity(%p): SET_CONFIGURATION command is not supported on controller entities\n",
				aecp);
			status = AECP_AEM_NOT_SUPPORTED;
			break;
		}
		...
		if (!list_empty(&aecp->inflight_application)) {
			os_log(LOG_ERR, "entity(%p): SET_CONFIGURATION command is not supported while inflight commands are in progress.\n",
				aecp);
			status = AECP_AEM_NOT_SUPPORTED;
			break;
		}

		if (avdecc_change_config(entity, ntohs(set_configuration_cmd->configuration_index)) < 0) {
			status = AECP_AEM_ENTITY_MISBEHAVING;
			break;
		}

		set_configuration_rsp->configuration_index = set_configuration_cmd->configuration_index;

		aecp_ipc_send_persistent_param(entity, AEM_DESC_TYPE_ENTITY, 0, AVDECC_PERSISTENT_PARAM_ENTITY_CONFIGURATION_INDEX, NULL);
	...
	{
		/* Neither IEEE-1722.1-2021 nor Milan v1.2 specs states that an error should be reported.
		 * So, just print an error and reply with success.
		 */
		if (aecp_unsolicited_remove(aecp, controller_entity_id, port_id) < 0)
			os_log(LOG_ERR, "aecp(%p) received DEREGISTER_UNSOLICITED_NOTIFICATION from controller %"PRIx64" on port %u but it was not previously registered, ignoring.\n",
					aecp, ntohll(controller_entity_id), port_id);
	...
	case AECP_AEM_CMD_SET_NAME:
	{
		...
		u16 config_index = ntohs(set_name_cmd->configuration_index);
		...
		desc = aem_get_descriptor(entity->aem_avail_descs[config_index], ntohs(set_name_cmd->descriptor_type), ntohs(set_name_cmd->descriptor_index), NULL);
		if (!desc) {
			status = AECP_AEM_NO_SUCH_DESCRIPTOR;
			break;
		}
		...
		if (object_name) {
			...
			if (status == AECP_AEM_SUCCESS && os_memcmp(object_name, set_name_cmd->name, AEM_STR_LEN_MAX)) {
				...
				if (is_persistent)
					aecp_ipc_send_persistent_param(entity, ntohs(set_name_cmd->descriptor_type), ntohs(set_name_cmd->descriptor_index),
									parameter_type, object_name);
			}
...
static int aecp_config_init_timers(struct aecp_ctx *aecp)
{
	struct entity *entity = container_of(aecp, struct entity, aecp);
	struct stream_output_dynamic_desc *stream_output_dynamic;
	struct stream_input_dynamic_desc *stream_input_dynamic;
...
int aecp_init_timers(struct aecp_ctx *aecp)
{
	struct entity *entity = container_of(aecp, struct entity, aecp);

	aecp->identify_sm.timeout.func = &aecp_identify_timeout_handler;
	aecp->identify_sm.timeout.data = entity;

	if (timer_create(entity->avdecc->timer_ctx, &aecp->identify_sm.timeout, 0, AECP_IDENTIFY_TIMEOUT_GRANULARITY_MS) < 0)
		goto err_identify_timeout_timer_create;

	aecp->identify_sm.identify_tx.func = &aecp_identify_timeout_handler;
	aecp->identify_sm.identify_tx.data = entity;

	if (timer_create(entity->avdecc->timer_ctx, &aecp->identify_sm.identify_tx, 0, AECP_IDENTIFY_TRANSMIT_DELAY_GRANULARITY_MS) < 0)
		goto err_identify_tx_timer_create;

	return 0;

err_identify_tx_timer_create:
	timer_destroy(&aecp->identify_sm.timeout);

err_identify_timeout_timer_create:
	return -1;
}

int aecp_current_config_init(struct aecp_ctx *aecp)
{
	if (aecp_config_init_timers(aecp) < 0)
		goto err;
...
__exit static void aecp_exit_timers(struct aecp_ctx *aecp)
{
	...
	timer_destroy(&aecp->identify_sm.identify_tx);
}
...
__init int aecp_init(struct aecp_ctx *aecp, void *data, struct avdecc_entity_config *cfg)
{
	...
	aecp->identify_sm.state = IDENTIFY_STATE_INITIALIZE;

	if (aecp_init_timers(aecp) < 0)
		goto err_timer;

	if (aecp_current_config_init(aecp) < 0)
		goto err_config_timer;
...
static void aecp_config_exit_timers(struct aecp_ctx *aecp)
{
	struct entity *entity = container_of(aecp, struct entity, aecp);
	struct stream_output_dynamic_desc *stream_output_dynamic;
	struct stream_input_dynamic_desc *stream_input_dynamic;
...
{
	aecp_config_exit_timers(aecp);
}

__exit int aecp_exit(struct aecp_ctx *aecp)
{
	aecp_current_config_exit(aecp);
