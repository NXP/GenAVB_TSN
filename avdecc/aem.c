...
__init void aem_init(struct aem_desc_hdr *aem_desc, struct avdecc_entity_config *cfg, int entity_num)
{
	...
	int num_interfaces, num_memory_object, i;
	 ...
	 */
	num_memory_object = aem_get_descriptor_max(aem_desc, AEM_DESC_TYPE_MEMORY_OBJECT);
	for (i = 0; i < num_memory_object; i++) {
		struct memory_object_descriptor *memory_object_desc = aem_get_descriptor(aem_desc, AEM_DESC_TYPE_MEMORY_OBJECT, i, NULL);
		u16 memory_obj_type = ntohs(memory_object_desc->memory_object_type);
		u64 memory_obj_addr = AEM_MEMORY_OBJECT_BASE_ADDR(0, i, memory_obj_type); // FIXME: only one configuration supported
