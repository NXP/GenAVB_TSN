 ...
 * \param cfg				pointer to the entity configuration
 * \param aem_first_dynamic_descs	If not null, pointer to the first configuration's dynamic descriptors.
 */
__init void *aem_dynamic_descs_init(struct aem_desc_hdr *aem_descs, struct avdecc_entity_config *cfg, struct aem_desc_hdr *aem_first_dynamic_descs)
{
	...
	/* Only reserve memory storage for the entity dynamic desc on first configuration */
	if (!aem_first_dynamic_descs)
	...
	/* The Entity dynamic descriptor is shared among all configurations:
	 * - Allocate and init for the first configuration
	 * - All subsequent configurations just point to the first region.
	 */
	if (!aem_first_dynamic_descs) {
		...
		aem_single_dynamic_desc_init(aem_dynamic_descs, AEM_DESC_TYPE_ENTITY, aem_first_dynamic_descs[AEM_DESC_TYPE_ENTITY].ptr,
				aem_first_dynamic_descs[AEM_DESC_TYPE_ENTITY].size, aem_first_dynamic_descs[AEM_DESC_TYPE_ENTITY].total);
	}

	aem_single_dynamic_desc_init(aem_dynamic_descs, AEM_DESC_TYPE_AVB_INTERFACE, (char *)aem_dynamic_descs + offset,
			sizeof(struct avb_interface_dynamic_desc) + path_sequence_size, num_interfaces);
...
__init void aem_init(struct aem_desc_hdr **aem_desc, struct avdecc_entity_config *cfg, int entity_num, unsigned int num_cfgs)
{
	struct avdecc_config *avdecc_cfg = container_of(cfg, struct avdecc_config, entity_cfg[entity_num]);
	struct entity_descriptor *entity = aem_get_descriptor(aem_desc[0], AEM_DESC_TYPE_ENTITY, 0, NULL);
	unsigned int max_stream_sources = 0, max_stream_sinks = 0;
	int num_interfaces, num_memory_object, i, cfg_idx;
	char empty_sn[AEM_STR_LEN_MAX] = {'\0'};
	...
	if (os_memcmp(cfg->serial_number, empty_sn, AEM_STR_LEN_MAX))
		os_memcpy(entity->serial_number, cfg->serial_number, AEM_STR_LEN_MAX);

	/* dynamic settings */
	for (cfg_idx = 0; cfg_idx < num_cfgs; cfg_idx++) {

		if (aem_get_descriptor_max(aem_desc[cfg_idx], AEM_DESC_TYPE_STREAM_OUTPUT) > max_stream_sources)
			max_stream_sources = aem_get_descriptor_max(aem_desc[cfg_idx], AEM_DESC_TYPE_STREAM_OUTPUT);

		if (aem_get_descriptor_max(aem_desc[cfg_idx], AEM_DESC_TYPE_STREAM_INPUT) > max_stream_sinks)
			max_stream_sinks = aem_get_descriptor_max(aem_desc[cfg_idx], AEM_DESC_TYPE_STREAM_INPUT);

		num_interfaces = aem_get_descriptor_max(aem_desc[cfg_idx], AEM_DESC_TYPE_AVB_INTERFACE);
		for (i = 0; i < num_interfaces; i++) {
			struct avb_interface_descriptor *interface = aem_get_descriptor(aem_desc[cfg_idx], AEM_DESC_TYPE_AVB_INTERFACE, i, NULL);

			if (net_get_local_addr(avdecc_cfg->logical_port_list[i], local_mac) >= 0)
				os_memcpy(interface->mac_address, local_mac, 6);
		}

		/* Dynamically assign fixed start addresses to memory objects based on:
		 * The configuration index, the index of the memory object descriptor and its type.
		 * The app shall use this start address to identify the object and map it to an actual file, thus an actual address.
		 */
		num_memory_object = aem_get_descriptor_max(aem_desc[cfg_idx], AEM_DESC_TYPE_MEMORY_OBJECT);
		for (i = 0; i < num_memory_object; i++) {
			struct memory_object_descriptor *memory_object_desc = aem_get_descriptor(aem_desc[cfg_idx], AEM_DESC_TYPE_MEMORY_OBJECT, i, NULL);
			u16 memory_obj_type = ntohs(memory_object_desc->memory_object_type);
			u64 memory_obj_addr = AEM_MEMORY_OBJECT_BASE_ADDR(cfg_idx, i, memory_obj_type);

			memory_object_desc->start_address = htonll(memory_obj_addr);
		}
	}

	/* Per Milan v1.3 5.3.3.1: naximum of stream numbers among all possible configurations. */
	entity->talker_stream_sources = htons(max_stream_sources);
	entity->listener_stream_sinks = htons(max_stream_sinks);
}
