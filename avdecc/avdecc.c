...
/**
 * Checks if a stream input is the clock source of its related clock domain.
 * \return true if the stream input is a clock source of its clock domain, false otherwise
 * \param entity			pointer to the entity structure to check.
...
bool avdecc_stream_input_is_clock_source(struct entity *entity, u16 listener_unique_id)
{
	...
	if ((ntohs(clock_source_desc->clock_source_type) == AEM_CLOCK_SOURCE_TYPE_INPUT_STREAM)
	    && (ntohs(clock_source_desc->clock_source_location_index) == listener_unique_id)) {
		rc = true;
		goto exit;
	}
...
{
	unsigned int num_interfaces, num_stream_input, num_stream_output, num_control, num_clock_domain;
	u16 avb_itf_index, num_of_formats, control_value_type, clock_source_type;
	struct clock_domain_descriptor *clock_domain_desc;
	struct clock_source_descriptor *clock_source_desc;
	...
	if (entity_ready(entity)) {
		...
		for (i = 0; i < num_clock_domain; i++) {
			...
			if (clock_source_type == AEM_CLOCK_SOURCE_TYPE_INPUT_STREAM) {
				...
				/* Stream input shouldn't have a valid stream id at this point */
			}
...
void avdecc_ipc_rx_media_stack(struct ipc_rx const *rx, struct ipc_desc *desc)
{
	...
	case IPC_HEARTBEAT:
		...
		if (!(entity->channel_openmask & AVDECC_WAITMASK_MEDIA_STACK)) {
			...
			/* Make sure to always send the clock sources message after the heartbeat response. */
	...
	case GENAVB_MSG_MEDIA_STACK_PERSISTENT_PARAM:
		...
		os_log(LOG_INFO, "avdecc(%p): received GENAVB_MSG_MEDIA_STACK_PERSISTENT_PARAM: Entity (%016"PRIx64") Descriptor Type %u, Descriptor Index %u, Parameter Type %u\n",
		...
		if ((entity = avdecc_get_entity(avdecc, desc->u.media_stack_persistent_param.entity_id)) != NULL) {
 ...
 * \param	source_type	clock source type, currently only AEM_CLOCK_SOURCE_TYPE_INPUT_STREAM and AEM_CLOCK_SOURCE_TYPE_INTERNAL are supported
 * \param	local_id	clock source id in case of an INTERNAL clock source
 * \param	stream_id	stream id in case of a STREAM_INPUT clock source, in network order
 */
void avdecc_ipc_set_clock_source(struct avdecc_ctx *avdecc, struct ipc_tx *ipc, u16 domain, u16 source_type, u16 local_id, u64 stream_id)
{
	...
	if (desc) {
		...
		if (source_type == AEM_CLOCK_SOURCE_TYPE_INTERNAL) {
			set_clock_source->source_type = GENAVB_CLOCK_SOURCE_TYPE_INTERNAL;
			/* FIXME: Only PTP_CLK is supported as internal clock for now. */
			set_clock_source->local_id = GENAVB_CLOCK_SOURCE_PTP_CLK;

		} else if (source_type == AEM_CLOCK_SOURCE_TYPE_INPUT_STREAM) {
			set_clock_source->source_type = GENAVB_CLOCK_SOURCE_TYPE_INPUT_STREAM;
			os_memcpy(&set_clock_source->stream_id, &stream_id, 8);

		} else {
			os_log(LOG_ERR, "Unsupported clock source type(%u)\n", source_type);
			ipc_free(ipc, desc);
		}
