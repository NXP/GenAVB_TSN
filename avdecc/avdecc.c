...
static void avdecc_ipc_managed_parse_default_ds_response(struct entity *entity, unsigned int port_id, struct genavb_avdecc_managed_objects_default_ds_response *response)
{
	...
	for (cfg_idx = 0; cfg_idx < ntohs(entity->desc->configurations_count); cfg_idx++) {

		avb_itf = aem_get_descriptor(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_AVB_INTERFACE, port_id, NULL);
		if (!avb_itf) /* Interface index not supported in this entity's configuration.  */
			continue;

		if (response->node_status != 0) {
			os_log(LOG_ERR, "entity(%p, cfg %u): Managed object get status %u failed\n", entity, cfg_idx, response->node_status);
			return;
		}

		node_len_expected = sizeof(response->node_status) + sizeof(response->instance);
		if (response->node_len != node_len_expected) {
			os_log(LOG_ERR, "entity(%p, cfg %u): Managed object get length failed: status (%u), len : expected(%u) got(%u)\n",
					entity, cfg_idx, response->node_status, node_len_expected, response->node_len);
			goto exit;
		}
		...
		if (response->instance.entry.default_ds.clk_id_leaf_status == 0 && response->instance.entry.default_ds.clk_id_leaf_len == leaf_len_expected) {
			...
			os_log(LOG_ERR, "entity(%p, cfg %u): Managed object get clock identity failed: status (%u), len : expected(%u) got(%u)\n",
					entity, cfg_idx, response->instance.entry.default_ds.clk_id_leaf_status, leaf_len_expected,
					response->instance.entry.default_ds.clk_id_leaf_len);
			goto exit;
		}
		...
		if (response->instance.entry.default_ds.priority1_leaf_status == 0 && response->instance.entry.default_ds.priority1_leaf_len == leaf_len_expected) {
			...
			os_log(LOG_ERR, "entity(%p, cfg %u): Managed object get priority1 failed: status (%u), len : expected(%u) got(%u)\n",
					entity, cfg_idx, response->instance.entry.default_ds.priority1_leaf_status, leaf_len_expected, response->instance.entry.default_ds.priority1_leaf_len);
			goto exit;
		}
		...
		if (response->instance.entry.default_ds.priority2_leaf_status == 0 && response->instance.entry.default_ds.priority2_leaf_len == leaf_len_expected) {
			...
			os_log(LOG_ERR, "entity(%p, cfg %u): Managed object get priority2 failed: status (%u), len : expected(%u) got(%u)\n",
					entity, cfg_idx, response->instance.entry.default_ds.priority2_leaf_status, leaf_len_expected,
					response->instance.entry.default_ds.priority2_leaf_len);
			goto exit;
		}

		leaf_len_expected = sizeof(response->instance.entry.default_ds.clock_quality.clk_class_leaf_status)
					+ sizeof(response->instance.entry.default_ds.clock_quality.clk_class_leaf_value);
		if (response->instance.entry.default_ds.clock_quality.clk_class_leaf_status == 0
			&& response->instance.entry.default_ds.clock_quality.clk_class_leaf_len == leaf_len_expected) {

			avb_itf->clock_class = response->instance.entry.default_ds.clock_quality.clk_class_leaf_value;

		} else {
			os_log(LOG_ERR, "entity(%p, cfg %u): Managed object get clock class failed: status (%u), len : expected(%u) got(%u)\n",
					entity, cfg_idx, response->instance.entry.default_ds.clock_quality.clk_class_leaf_status, leaf_len_expected,
					response->instance.entry.default_ds.clock_quality.clk_class_leaf_len);
			goto exit;
		}

		leaf_len_expected = sizeof(response->instance.entry.default_ds.clock_quality.offset_scaled_log_variance_leaf_status)
					+ sizeof(response->instance.entry.default_ds.clock_quality.offset_scaled_log_variance_leaf_value);
		if (response->instance.entry.default_ds.clock_quality.offset_scaled_log_variance_leaf_status == 0
			&& response->instance.entry.default_ds.clock_quality.offset_scaled_log_variance_leaf_len == leaf_len_expected) {
			...
			os_log(LOG_ERR, "entity(%p, cfg %u): Managed object get offset scaled log variance failed: status (%u), len : expected(%u) got(%u)\n",
					entity, cfg_idx, response->instance.entry.default_ds.clock_quality.offset_scaled_log_variance_leaf_status, leaf_len_expected,
					response->instance.entry.default_ds.clock_quality.offset_scaled_log_variance_leaf_len);
			goto exit;
		}

		leaf_len_expected = sizeof(response->instance.entry.default_ds.clock_quality.clk_accuracy_leaf_status)
					+ sizeof(response->instance.entry.default_ds.clock_quality.clk_accuracy_leaf_value);
		if (response->instance.entry.default_ds.clock_quality.clk_accuracy_leaf_status == 0
			&& response->instance.entry.default_ds.clock_quality.clk_accuracy_leaf_len == leaf_len_expected) {

			avb_itf->clock_accuracy = response->instance.entry.default_ds.clock_quality.clk_accuracy_leaf_value;

		} else {
			os_log(LOG_ERR, "entity(%p, cfg %u): Managed object get clock accuracy failed: status (%u), len : expected(%u) got(%u)\n",
					entity, cfg_idx, response->instance.entry.default_ds.clock_quality.clk_accuracy_leaf_status, leaf_len_expected,
					response->instance.entry.default_ds.clock_quality.clk_accuracy_leaf_len);
			goto exit;
		}
	}
...
__init static bool avdecc_entity_check(struct entity *entity)
{
	...
	for (cfg = 0; cfg < entity->num_cfgs; cfg++) {
		num_interfaces = aem_get_descriptor_max(entity->aem_avail_descs[cfg], AEM_DESC_TYPE_AVB_INTERFACE);
		if ((num_interfaces > avdecc->port_max) || (num_interfaces > AEM_NUM_AVB_INTERFACES_MAX)) {
			os_log(LOG_ERR, "entity(%p) cfg(%u): unsupported num interfaces %u, max ports %u, max avb interfaces %u\n",
				entity, cfg, num_interfaces, avdecc->port_max, AEM_NUM_AVB_INTERFACES_MAX);
			goto err;
		}

		for (i = 0; i < num_interfaces; i++) {
			if (!avdecc->port[i].initialized) {
				os_log(LOG_ERR, "entity(%p) cfg(%u): unsupported avb interface index(%u)\n", entity, cfg, i);
				goto err;
			}
		}

		num_stream_input = aem_get_descriptor_max(entity->aem_avail_descs[cfg], AEM_DESC_TYPE_STREAM_INPUT);
		for (i = 0; i < num_stream_input; i++) {
			stream_desc = aem_get_descriptor(entity->aem_avail_descs[cfg], AEM_DESC_TYPE_STREAM_INPUT, i, NULL);
			if (!stream_desc) {
				os_log(LOG_ERR, "entity(%p) cfg(%u): invalid stream input index(%u)\n", entity, cfg, i);
				goto err;
			}
			...
			if (avb_itf_index >= avdecc->port_max || !avdecc->port[avb_itf_index].initialized) {
				os_log(LOG_ERR, "entity(%p) cfg(%u): unsupported avb interface index(%u) for stream input %u\n",
					entity, cfg, avb_itf_index, i);
				goto err;
			}

			num_of_formats = ntohs(stream_desc->number_of_formats);
			if (num_of_formats > AEM_NUM_FORMATS_MAX) {
				os_log(LOG_ERR, "entity(%p) cfg(%u): number of formats %u exceeding max %u for stream input %u\n",
					entity, cfg, num_of_formats, AEM_NUM_FORMATS_MAX, i);
				goto err;
			}

			num_of_redundant_streams = ntohs(stream_desc->number_of_redundant_streams);
			if (num_of_redundant_streams > CFG_AVTP_REDUNDANT_STREAMS_MAX) {
				os_log(LOG_ERR, "entity(%p) cfg(%u): number of redundant streams %u exceeding max %u for stream input %u\n",
					entity, cfg, num_of_redundant_streams, CFG_AVTP_REDUNDANT_STREAMS_MAX, i);
				goto err;
			}
		}

		num_stream_output = aem_get_descriptor_max(entity->aem_avail_descs[cfg], AEM_DESC_TYPE_STREAM_OUTPUT);
		for (i = 0; i < num_stream_output; i++) {
			stream_desc = aem_get_descriptor(entity->aem_avail_descs[cfg], AEM_DESC_TYPE_STREAM_OUTPUT, i, NULL);
			if (!stream_desc) {
				os_log(LOG_ERR, "entity(%p) cfg(%u): invalid stream output index(%u)\n", entity, cfg, i);
				goto err;
			}

			avb_itf_index = ntohs(stream_desc->avb_interface_index);
			if (avb_itf_index >= avdecc->port_max || !avdecc->port[avb_itf_index].initialized) {
				os_log(LOG_ERR, "entity(%p) cfg(%u): unsupported avb interface index(%u) for stream output %u\n",
					entity, cfg, avb_itf_index, i);
				goto err;
			}
			...
			num_of_formats = ntohs(stream_desc->number_of_formats);
			if (num_of_formats > AEM_NUM_FORMATS_MAX) {
				os_log(LOG_ERR, "entity(%p) cfg(%u): number of formats %u exceeding max %u for stream output %u\n",
					entity, cfg, num_of_formats, AEM_NUM_FORMATS_MAX, i);
				goto err;
			}
			...
			num_of_redundant_streams = ntohs(stream_desc->number_of_redundant_streams);
			if (num_of_redundant_streams > CFG_AVTP_REDUNDANT_STREAMS_MAX) {
				os_log(LOG_ERR, "entity(%p) cfg(%u): number of redundant streams %u exceeding max %u for stream output %u\n",
					entity, cfg, num_of_redundant_streams, CFG_AVTP_REDUNDANT_STREAMS_MAX, i);
				goto err;
			}
		}

		num_control = aem_get_descriptor_max(entity->aem_avail_descs[cfg], AEM_DESC_TYPE_CONTROL);
		for (i = 0; i < num_control; i++) {
			control_desc = aem_get_descriptor(entity->aem_avail_descs[cfg], AEM_DESC_TYPE_CONTROL, i, NULL);
			if (!control_desc) {
				os_log(LOG_ERR, "entity(%p) cfg(%u): invalid control index(%u)\n", entity, cfg, i);
				goto err;
			}
			...
			case AEM_CONTROL_LINEAR_UINT8:
				if (ntohs(control_desc->number_of_values) > AEM_NUM_VALUE_DETAILS_LINEAR_U8_MAX) {
					os_log(LOG_ERR, "entity(%p) cfg(%u): control descriptor(%d) control_value_type(%u): number_of_values(%u) exceeding max(%u)\n",
								entity, cfg, i, control_value_type, ntohs(control_desc->number_of_values), AEM_NUM_VALUE_DETAILS_LINEAR_U8_MAX);
					goto err;
				}
				break;
			...
			case AEM_CONTROL_UTF8:
				if (ntohs(control_desc->number_of_values) > AEM_NUM_VALUE_DETAILS_LINEAR_UTF8_MAX) {
					os_log(LOG_ERR, "entity(%p) cfg(%u): control descriptor(%d) control_value_type(%u): number_of_values(%u) exceeding max(%u)\n",
								entity, cfg, i, control_value_type, ntohs(control_desc->number_of_values), AEM_NUM_VALUE_DETAILS_LINEAR_UTF8_MAX);
					goto err;
				}
				break;

			default:
				os_log(LOG_ERR, "entity(%p) cfg(%u): control descriptor(%d) unsupported control_value_type(%u)\n", entity, cfg, i, control_value_type);
				goto err;
			}
		}

		num_clock_domain = aem_get_descriptor_max(entity->aem_avail_descs[cfg], AEM_DESC_TYPE_CLOCK_DOMAIN);
		for (i = 0; i < num_clock_domain; i++) {
			clock_domain_desc = aem_get_descriptor(entity->aem_avail_descs[cfg], AEM_DESC_TYPE_CLOCK_DOMAIN, i, NULL);
			if (!clock_domain_desc) {
				os_log(LOG_ERR, "entity(%p) cfg(%u): invalid clock domain index(%u)\n", entity, cfg, i);
				goto err;
			}

			for (j = 0; j < ntohs(clock_domain_desc->clock_sources_count); j++) {
				clock_source_desc = aem_get_descriptor(entity->aem_avail_descs[cfg], AEM_DESC_TYPE_CLOCK_SOURCE, ntohs(clock_domain_desc->clock_sources[j]), NULL);
				if (!clock_source_desc) {
					os_log(LOG_ERR, "entity(%p) cfg(%u): clock source(%u) referenced by clock domain(%u) does not exist\n",
						entity, cfg, ntohs(clock_domain_desc->clock_sources[j]), i);
					goto err;
				}
				...
				if ((clock_source_type != AEM_CLOCK_SOURCE_TYPE_INTERNAL) && (clock_source_type != AEM_CLOCK_SOURCE_TYPE_INPUT_STREAM)) {
					os_log(LOG_ERR, "entity(%p) cfg(%u): clock source(%u) of clock domain(%u) has an unsupported clock source type(%u)\n",
						entity, cfg, ntohs(clock_domain_desc->clock_sources[j]), i, clock_source_type);
					goto err;
				}

				if (clock_source_type == AEM_CLOCK_SOURCE_TYPE_INPUT_STREAM) {
					stream_desc = aem_get_descriptor(entity->aem_avail_descs[cfg], AEM_DESC_TYPE_STREAM_INPUT, ntohs(clock_source_desc->clock_source_location_index), NULL);
					if (!stream_desc) {
						os_log(LOG_ERR, "entity(%p) cfg(%u): stream input(%u) referenced by clock source(%u) does not exist\n",
							entity, cfg, ntohs(clock_source_desc->clock_source_location_index), ntohs(clock_domain_desc->clock_sources[j]));
						goto err;
					}
				}
			}
		...
		num_memory_object = aem_get_descriptor_max(entity->aem_avail_descs[cfg], AEM_DESC_TYPE_MEMORY_OBJECT);
		if (num_memory_object > AEM_NUM_MEMORY_OBJECTS_MAX) {
			os_log(LOG_ERR, "entity(%p) cfg(%u): unsupported num memory objects %u, max memory objects %u\n",
				entity, cfg, num_memory_object, AEM_NUM_MEMORY_OBJECTS_MAX);
			goto err;
		}

		for (i = 0; i < num_memory_object; i++) {
			memory_object_desc = aem_get_descriptor(entity->aem_avail_descs[cfg], AEM_DESC_TYPE_MEMORY_OBJECT, i, NULL);
			if (!memory_object_desc) {
				os_log(LOG_ERR, "entity(%p) cfg(%u): invalid memory object index(%u)\n", entity, cfg, i);
				goto err;
			}
			...
			if ((memory_obj_type != AEM_MEMORY_OBJECT_PNG_MANUFACTURER) &&
				...
				os_log(LOG_ERR, "entity(%p) cfg(%u): memory object(%u) unsupported type(%u), only PNG types are supported\n",
					entity, cfg, i, memory_obj_type);
				goto err;
			}
		}
	}
...
__init static int avdecc_dynamic_states_init(struct entity *entity, struct avdecc_entity_config *cfg)
{
	...
	entity_dynamic = aem_get_descriptor(entity->aem_avail_dynamic_descs[0], AEM_DESC_TYPE_ENTITY, 0, NULL);
	if (!entity_dynamic)
		goto err_entity;
	...
	for (cfg_idx = 0; cfg_idx < entity->num_cfgs; cfg_idx++) {

		num_desc = aem_get_descriptor_max(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_AVB_INTERFACE);
		for (i = 0; i < num_desc; i++) {
			avb_itf_dynamic = aem_get_descriptor(entity->aem_avail_dynamic_descs[cfg_idx], AEM_DESC_TYPE_AVB_INTERFACE, i, NULL);
			if (!avb_itf_dynamic)
				goto err_desc;
		...
		num_desc = aem_get_descriptor_max(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_STREAM_INPUT);
		for (i = 0; i < num_desc; i++) {
			stream_input_dynamic = aem_get_descriptor(entity->aem_avail_dynamic_descs[cfg_idx], AEM_DESC_TYPE_STREAM_INPUT, i, NULL);
		...
		num_desc = aem_get_descriptor_max(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_STREAM_OUTPUT);
		for (i = 0; i < num_desc; i++) {
			stream_output_dynamic = aem_get_descriptor(entity->aem_avail_dynamic_descs[cfg_idx], AEM_DESC_TYPE_STREAM_OUTPUT, i, NULL);
		...
		num_desc = aem_get_descriptor_max(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_CLOCK_DOMAIN);
		for (i = 0; i < num_desc; i++) {
			clock_domain_dynamic = aem_get_descriptor(entity->aem_avail_dynamic_descs[cfg_idx], AEM_DESC_TYPE_CLOCK_DOMAIN, i, NULL);
...
__init static int avdecc_dynamic_redundant_listener_set_init(struct entity *entity)
{
	...
	for (cfg_idx = 0; cfg_idx < entity->num_cfgs; cfg_idx++) {
		...
		num_desc = aem_get_descriptor_max(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_STREAM_INPUT);
		for (i = 0; i < num_desc; i++) {
			stream_desc = aem_get_descriptor(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_STREAM_INPUT, i, NULL);
			if (!stream_desc) {
				os_log(LOG_ERR, "entity(%p) cfg(%u): invalid stream input index(%u)\n", entity, cfg_idx, i);
				goto err;
			}
			...
			if (!STREAM_HAS_REDUNDANT_STREAMS(stream_desc)) {
				stream_input_desc = aem_get_descriptor(entity->aem_avail_dynamic_descs[cfg_idx], AEM_DESC_TYPE_STREAM_INPUT, i, NULL);
				if (!stream_input_desc) {
					os_log(LOG_ERR, "entity(%p) cfg(%u): invalid stream input index(%u)\n", entity, cfg_idx, i);
					goto err;
				}
				...
				os_log(LOG_INFO, "entity(%p) cfg(%u) stream input(%u): redundant set id(%u), redundant stream index(%u)\n",
					entity, cfg_idx, i, stream_input_desc->set_id, stream_input_desc->set_index);
		...
		for (i = 0; i < num_desc; i++) {
			stream_input_desc = aem_get_descriptor(entity->aem_avail_dynamic_descs[cfg_idx], AEM_DESC_TYPE_STREAM_INPUT, i, NULL);
			stream_desc = (struct stream_descriptor *)aem_get_descriptor(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_STREAM_INPUT, i, NULL);
			...
			for (j = 0; j < num_redundant_desc; j++) {
				...
				redundant_input_desc = aem_get_descriptor(entity->aem_avail_dynamic_descs[cfg_idx], AEM_DESC_TYPE_STREAM_INPUT, redundant_desc_index, NULL);
				redundant_desc = (struct stream_descriptor *)aem_get_descriptor(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_STREAM_INPUT, redundant_desc_index, NULL);
				if (!redundant_input_desc || !redundant_desc) {
					os_log(LOG_ERR, "entity(%p) cfg(%u): invalid redundant(%u, %u) stream input index(%u)\n",
						entity, cfg_idx, i, j, redundant_desc_index);
					goto err;
				}
				...
				if (new_set) {
					...
					if (redundant_input_desc->set_id == REDUNDANT_SET_ID_INVALID) {
						...
						os_log(LOG_INFO, "entity(%p) cfg(%u) stream input(%u): redundant set id(%u), redundant stream index(%u)\n",
							entity, cfg_idx, redundant_desc_index, redundant_input_desc->set_id, redundant_input_desc->set_index);

						set_index++;

					} else {
						os_log(LOG_ERR, "entity(%p) cfg(%u): redundant stream input(%u) is already referenced in another redundant set(%u), expected(%u)\n",
							entity, cfg_idx, redundant_desc_index, redundant_input_desc->set_id, stream_input_desc->set_id);
						goto err;
					}
					...
					if (redundant_desc->number_of_redundant_streams != stream_desc->number_of_redundant_streams) {
						os_log(LOG_ERR, "entity(%p) cfg(%u): stream input(%u) and redundant stream input(%u) reference a different number of redundant streams(%u), expected(%u)\n",
							entity, cfg_idx, i, redundant_desc_index, redundant_desc->number_of_redundant_streams,
					...
					if (!cmp_64(&stream_desc->current_format, &redundant_desc->current_format)) {
						os_log(LOG_ERR, "entity(%p) cfg(%u): stream input(%u)'s and redundant stream input(%u)'s format don't match\n",
							entity, cfg_idx, i, redundant_desc_index);
						goto err;
					}
					...
					if (!avdecc_are_streams_supporting_same_formats(stream_desc, redundant_desc)) {
						os_log(LOG_ERR, "entity(%p) cfg(%u): stream input(%u)'s and redundant stream input(%u)'s supported formats array don't match\n",
							entity, cfg_idx, i, redundant_desc_index);
						goto err;
					}
					...
					if (stream_desc->clock_domain_index != redundant_desc->clock_domain_index) {
						os_log(LOG_ERR, "entity(%p) cfg(%u): stream input(%u) and redundant stream input(%u) don't share the same clock domain, index(%u) != index(%u)\n",
							entity, cfg_idx, i, redundant_desc_index, ntohs(stream_desc->clock_domain_index), ntohs(redundant_desc->clock_domain_index));
						goto err;
					}
					...
					if (redundant_input_desc->set_id != stream_input_desc->set_id) {
						os_log(LOG_ERR, "entity(%p) cfg(%u): stream input(%u) has redundant stream input(%u) in a different redundant set(%u), expected(%u)\n",
							entity, cfg_idx, i, redundant_desc_index, redundant_input_desc->set_id, stream_input_desc->set_id);
						goto err;
					}
				}
				...
				if (!valid_set) {
					os_log(LOG_ERR, "entity(%p) cfg(%u): redundant stream input(%u) does not reference stream input(%u) back\n",
						entity, cfg_idx, redundant_desc_index, i);
					goto err;
				}
...
__init static int avdecc_dynamic_redundant_talker_set_init(struct entity *entity)
{
	...
	for (cfg_idx = 0; cfg_idx < entity->num_cfgs; cfg_idx++) {
		...
		num_desc = aem_get_descriptor_max(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_STREAM_OUTPUT);
		for (i = 0; i < num_desc; i++) {
			stream_desc = aem_get_descriptor(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_STREAM_OUTPUT, i, NULL);
			if (!stream_desc) {
				os_log(LOG_ERR, "entity(%p) cfg(%u): invalid stream output index(%u)\n", entity, cfg_idx, i);
				goto err;
			}
			...
			/* Non redundant streams */
			if (!STREAM_HAS_REDUNDANT_STREAMS(stream_desc)) {
				stream_output_desc = aem_get_descriptor(entity->aem_avail_dynamic_descs[cfg_idx], AEM_DESC_TYPE_STREAM_OUTPUT, i, NULL);
				...
				os_log(LOG_INFO, "entity(%p) cfg(%u) stream output(%u): redundant set id(%u), redundant stream index(%u)\n",
					entity, cfg_idx, i, stream_output_desc->set_id, stream_output_desc->set_index);
		...
		for (i = 0; i < num_desc; i++) {
			stream_output_desc = aem_get_descriptor(entity->aem_avail_dynamic_descs[cfg_idx], AEM_DESC_TYPE_STREAM_OUTPUT, i, NULL);
			stream_desc = (struct stream_descriptor *)aem_get_descriptor(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_STREAM_OUTPUT, i, NULL);
			...
			for (j = 0; j < num_redundant_desc; j++) {
				...
				redundant_output_desc = aem_get_descriptor(entity->aem_avail_dynamic_descs[cfg_idx], AEM_DESC_TYPE_STREAM_OUTPUT, redundant_desc_index, NULL);
				redundant_desc = (struct stream_descriptor *)aem_get_descriptor(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_STREAM_OUTPUT, redundant_desc_index, NULL);
				if (!redundant_output_desc || !redundant_desc) {
					os_log(LOG_ERR, "entity(%p) cfg(%u): invalid redundant(%u, %u) stream output index(%u)\n",
						entity, cfg_idx, i, j, redundant_desc_index);
					goto err;
				}
				...
				if (new_set) {
					...
					if (redundant_output_desc->set_id == REDUNDANT_SET_ID_INVALID) {
						...
						os_log(LOG_INFO, "entity(%p) cfg(%u) stream output(%u): redundant set id(%u), redundant stream index(%u)\n",
							entity, cfg_idx, redundant_desc_index, redundant_output_desc->set_id, redundant_output_desc->set_index);

						set_index++;
					} else {
						os_log(LOG_ERR, "entity(%p) cfg(%u): redundant stream output(%u) is already referenced in another redundant set(%u), expected(%u)\n",
							entity, cfg_idx, redundant_desc_index, redundant_output_desc->set_id, stream_output_desc->set_id);
						goto err;
					}
					 ...
					 */
					if (redundant_desc->number_of_redundant_streams != stream_desc->number_of_redundant_streams) {
						os_log(LOG_ERR, "entity(%p) cfg(%u): stream output(%u) and redundant stream output(%u) reference a different number of redundant streams(%u), expected(%u)\n",
							entity, cfg_idx, i, redundant_desc_index, redundant_desc->number_of_redundant_streams,
							stream_desc->number_of_redundant_streams);
						goto err;
					}
					 ...
					 */
					if (!cmp_64(&stream_desc->current_format, &redundant_desc->current_format)) {
						os_log(LOG_ERR, "entity(%p) cfg(%u): stream output(%u)'s and redundant stream output(%u)'s format don't match\n",
							entity, cfg_idx, i, redundant_desc_index);
						goto err;
					}
					...
					/* Check also that the streams from the same redundant set support the same set of formats */
					if (!avdecc_are_streams_supporting_same_formats(stream_desc, redundant_desc)) {
						os_log(LOG_ERR, "entity(%p) cfg(%u): stream output(%u)'s and redundant stream output(%u)'s supported formats array don't match\n",
							entity, cfg_idx, i, redundant_desc_index);
						goto err;
					}
					 ...
					 */
					if (stream_desc->clock_domain_index != redundant_desc->clock_domain_index) {
						os_log(LOG_ERR, "entity(%p) cfg(%u): stream output(%u) and redundant stream output(%u) don't share the same clock domain, index(%u) != index(%u)\n",
							entity, cfg_idx, i, redundant_desc_index, ntohs(stream_desc->clock_domain_index), ntohs(redundant_desc->clock_domain_index));
						goto err;
					}
					...
					if (redundant_output_desc->set_id != stream_output_desc->set_id) {
						os_log(LOG_ERR, "entity(%p) cfg(%u): stream output(%u) has redundant stream output(%u) in a different redundant set(%u), expected(%u)\n",
							entity, cfg_idx, i, redundant_desc_index, redundant_output_desc->set_id, stream_output_desc->set_id);
						goto err;
					}
				}
				...
				if (!valid_set) {
					os_log(LOG_ERR, "entity(%p) cfg(%u): redundant stream output(%u) does not reference stream output(%u) back\n",
						entity, cfg_idx, redundant_desc_index, i);
					goto err;
				}
...
__init static struct entity *avdecc_entity_init(struct avdecc_ctx *avdecc, int entity_num, struct avdecc_entity_config *cfg)
{
	...
	if (cfg->num_aems > CFG_MAX_AEM_CONFIGS) {
		os_log(LOG_CRIT, "entity (%d): Unsupported number of AEM configurations (%u), max(%u)\n",
			entity_num, cfg->num_aems, CFG_MAX_AEM_CONFIGS);
		goto err_num_cfgs;
	}

	if (cfg->num_aems) {
		/* Use provided AEM descriptions.
		 * Even though redundant with the check above, the second iterator check on CFG_MAX_AEM_CONFIGS is
		 * to silence gcc array-bounds error (when CFG_MAX_AEM_CONFIGS equals 1)
		 */
		for (i = 0; i < cfg->num_aems && i < CFG_MAX_AEM_CONFIGS; i++) {

			if (!cfg->aem[i]) {
				os_log(LOG_CRIT, "entity (%d): Invalid AEM for configuration(%u)/(%u)\n",
					entity_num, i, cfg->num_aems);
				goto err_num_cfgs;
			}

			entity->aem_avail_descs[i] = cfg->aem[i];
			entity->num_cfgs++;
		}
	} else {
		/* No AEM description is provided, use the static entity. */
		entity->aem_avail_descs[0] = aem_entity_static_init();
		entity->num_cfgs = 1;
	}

	aem_init(entity->aem_avail_descs, cfg, entity_num, entity->num_cfgs);

	for (cfg_idx = 0; cfg_idx < entity->num_cfgs; cfg_idx++) {
		entity->aem_avail_dynamic_descs[cfg_idx] = aem_dynamic_descs_init(entity->aem_avail_descs[cfg_idx], cfg,
										  (cfg_idx == 0) ? NULL : entity->aem_avail_dynamic_descs[0]);
		if (!entity->aem_avail_dynamic_descs[cfg_idx]) {
			os_log(LOG_CRIT, "entity (%d): Cannot init cfg(%u) dynamic descriptors\n", entity_num, cfg_idx);
			goto err_dynamic_desc_init;
		}
	}

	/* Get the ENTITY static and dynamic descriptors:
	 * Since all configurations share the same ENTITY (and CONFIGURATION)
	 * descriptors, get the pointers from the first configuration in the array.
	 */
	entity->desc = aem_get_descriptor(entity->aem_avail_descs[0], AEM_DESC_TYPE_ENTITY, 0, NULL);
	entity->dynamic_desc = aem_get_descriptor(entity->aem_avail_dynamic_descs[0], AEM_DESC_TYPE_ENTITY, 0, NULL);
	if (!entity->desc || !entity->dynamic_desc) {
		os_log(LOG_CRIT, "entity (%d): Cannot find ENTITY descriptor\n", entity_num);
		goto err_entity_desc;
	}

	if (ntohs(entity->desc->configurations_count) != entity->num_cfgs) {
		os_log(LOG_CRIT, "entity (%d): mismatch between configurations_count(%u) and provided configs(%u)\n",
			entity_num, ntohs(entity->desc->configurations_count), entity->num_cfgs);
		goto err_entity_desc;
	}

	current_config_index = ntohs(entity->desc->current_configuration);
	if (current_config_index >= entity->num_cfgs) {
		os_log(LOG_CRIT, "entity (%d): Unsupported current_configuration(%u) while num configs(%u)\n",
			entity_num, current_config_index, entity->num_cfgs);
		goto err_entity_desc;
	}

	/* Set the current configuration. */
	entity->aem_descs = entity->aem_avail_descs[current_config_index];
	entity->aem_dynamic_descs = entity->aem_avail_dynamic_descs[current_config_index];

	entity->index = entity_num;
	entity->system_unique_id = htonl(0);

	for (i = 0; i < CFG_MAX_AEM_CONFIGS; i++)
		entity->identify_crtl_index[i] = -1;
	...
	os_log(LOG_INIT, "entity(%d) (%p) desc(%p), num_cfgs(%u), current_config(%u), id : %016"PRIx64", name : %s\n",
		entity_num, entity, entity->desc, entity->num_cfgs, ntohs(entity->desc->current_configuration),
		ntohll(entity->desc->entity_id), entity->desc->entity_name);
...
static int avdecc_current_config_init(struct entity *entity)
{
	struct avdecc_ctx *avdecc = entity->avdecc;

	if (acmp_current_config_init(&entity->acmp) < 0)
		goto err_acmp;

	if (aecp_current_config_init(&entity->aecp) < 0)
		goto err_aecp;

	if (adp_current_config_init(&entity->adp) < 0)
		goto err_adp;

	if (avdecc_entity_mc_init(entity) < 0)
		goto err_mc;

	//FIXME Do a CLOCK_DOMAIN_STATUS_GET to update clock domain counters if the source is internal clock.

	avdecc_local_entity_updated(avdecc, entity);

	os_log(LOG_INFO, "init current configuration(%u)\n",
		ntohs(entity->desc->current_configuration));

	return 0;

err_mc:
	adp_current_config_exit(&entity->adp, true);
err_adp:
	aecp_current_config_exit(&entity->aecp);
err_aecp:
	acmp_current_config_exit(&entity->acmp);
err_acmp:
	return -1;
}

static void avdecc_current_config_exit(struct entity *entity)
{
	acmp_current_config_exit(&entity->acmp);
	aecp_current_config_exit(&entity->aecp);
	adp_current_config_exit(&entity->adp, true);
	...
	os_log(LOG_INFO, "exit current configuration(%u)\n",
		ntohs(entity->desc->current_configuration));
}

int avdecc_change_config(struct entity *entity, u16 new_config_index)
{
	int rc = 0;

	if (entity->desc->current_configuration == htons(new_config_index))
		goto exit;

	if (!avdecc_is_valid_configuration_index(entity, new_config_index)) {
		rc = -1;
		goto exit;
	}

	avdecc_current_config_exit(entity);

	/* Set the current configuration. */
	entity->desc->current_configuration = htons(new_config_index);
	entity->aem_descs = entity->aem_avail_descs[new_config_index];
	entity->aem_dynamic_descs = entity->aem_avail_dynamic_descs[new_config_index];

	if (avdecc_current_config_init(entity) < 0) {
		//FIXME : how to handle this error ? roll back to previous config ?
		os_log(LOG_CRIT, "error initializing new configuration (%u)\n", new_config_index);
		rc = -1;
		goto exit;
	}
...
/** This function Emulates a SET_CONFIGURATION AECP command with the current configuration to be sent through CONTROLLED channel.
 * \return 	none
 * \param 	avdecc 	pointer to AVDECC context
 * \param 	entity 	pointer to Entity context
 */
static void avdecc_ipc_send_set_configuration(struct avdecc_ctx *avdecc, struct entity *entity)
{
	struct aecp_aem_set_configuration_pdu *set_configuration_cmd;
	struct ipc_tx *ipc= &avdecc->ipc_tx_media_stack;
	struct aecp_aem_pdu *aem_pdu;
	struct ipc_desc *desc;
	...
	if (desc) {
		...
		desc->u.aecp_msg.status = AECP_AEM_SUCCESS;
		desc->u.aecp_msg.len = sizeof(struct aecp_aem_pdu) + sizeof(struct aecp_aem_set_configuration_pdu);

		aem_pdu = (struct aecp_aem_pdu *) &desc->u.aecp_msg.buf;
		copy_64(&aem_pdu->entity_id, &entity->desc->entity_id);
		AECP_AEM_SET_U_CMD_TYPE(aem_pdu, 0, AECP_AEM_CMD_SET_CONFIGURATION);

		set_configuration_cmd = (struct aecp_aem_set_configuration_pdu *)(aem_pdu + 1);
		set_configuration_cmd->configuration_index = entity->desc->current_configuration;
...
static void avdecc_local_entity_updated(struct avdecc_ctx *avdecc, struct entity *entity)
{
	...
	if (entity_ready(entity)) {
		...
		if (!entity->started) {
			...
			/* Send the current configuration as AECP command for entities that has opened
			 * the CONTROLLED channel
			 */
			if ((entity->channel_openmask & AVDECC_WAITMASK_CONTROLLED))
				avdecc_ipc_send_set_configuration(avdecc, entity);
			...
			/* If valid identify index for the current configuration was provided (from upper layer), advertise it. */
			if (avdecc_is_valid_current_identify_index(entity))
				entity->desc->entity_capabilities |= htonl(ADP_ENTITY_AEM_IDENTIFY_CONTROL_INDEX_VALID);
			else
				entity->desc->entity_capabilities &= ~(htonl(ADP_ENTITY_AEM_IDENTIFY_CONTROL_INDEX_VALID));

			aecp_aem_identify_sm(entity, IDENTIFY_EVENT_BEGIN);
...
void avdecc_ipc_rx_gptp(struct ipc_rx const *rx, struct ipc_desc *desc)
{
	...
	case GENAVB_MSG_GM_STATUS:
		if ((desc->len < (sizeof(struct genavb_msg_gm_status)))
			|| (desc->len != (sizeof(struct genavb_msg_gm_status) + desc->u.gm_status.num_ptlv * sizeof(struct ptp_clock_identity)))) {
		...
		for (i = 0; i < avdecc->num_entities; i++) {
			...
			for (cfg_idx = 0; cfg_idx < ntohs(entity->desc->configurations_count); cfg_idx++) {
				...
				bool is_current_config = (cfg_idx == ntohs(entity->desc->current_configuration)) ? true : false;
				bool path_sequence_changed = false;

				avb_itf = aem_get_descriptor(entity->aem_avail_descs[cfg_idx], AEM_DESC_TYPE_AVB_INTERFACE, port->port_id, NULL);
				avb_itf_dynamic = aem_get_descriptor(entity->aem_avail_dynamic_descs[cfg_idx], AEM_DESC_TYPE_AVB_INTERFACE, port->port_id, NULL);

				if (!avb_itf || !avb_itf_dynamic) /* Interface index not supported in this entity's configuration. */
					continue;
				...
				if (path_sequence_changed || os_memcmp(&avb_itf_dynamic->path_sequence, &desc->u.gm_status.path_sequence,
									avb_itf_dynamic->num_ptlv_entries * sizeof(struct ptp_clock_identity))) {

					os_memcpy(&avb_itf_dynamic->path_sequence, &desc->u.gm_status.path_sequence,
						  avb_itf_dynamic->num_ptlv_entries * sizeof(struct ptp_clock_identity));
				...
				if (!cmp_64(&avb_itf_dynamic->gptp_grandmaster_id, &desc->u.gm_status.gm_id)) {
					u64 old_gm_id = avb_itf_dynamic->gptp_grandmaster_id;
					...
					if (is_current_config) {
						os_log(LOG_INFO, "GTP GM id change for local entity(%p) old 0x%"PRIx64" new 0x%"PRIx64"\n",
							entity, ntohll(old_gm_id), ntohll(desc->u.gm_status.gm_id));
				...
				if (path_sequence_changed && entity_ready(entity) && is_current_config) {
...
static void avdecc_set_identify_index(struct entity *entity, u16 config_index, u16 identify_crtl_index)
{
	struct control_descriptor *control_desc;

	if (!avdecc_is_valid_configuration_index(entity, config_index)) {
		os_log(LOG_ERR, "entity(%p): Invalid config index (%u) for identify descriptor(%u)\n",
			entity, config_index, identify_crtl_index);
		goto out;
	}

	control_desc = aem_get_descriptor(entity->aem_avail_descs[config_index], AEM_DESC_TYPE_CONTROL, identify_crtl_index, NULL);
	if (!control_desc) {
		os_log(LOG_ERR, "entity(%p): invalid Identify control descriptor(cfg %u, index %u)\n",
			entity, config_index, identify_crtl_index);
		goto out;

	} else if (control_desc->control_type != htonll(AEM_CONTROL_TYPE_IDENTIFY)) {
		os_log(LOG_ERR, "entity(%p): invalid Identify control descriptor(cfg %u, index %u) type(%"PRIx64")\n",
			entity, config_index, identify_crtl_index, ntohll(control_desc->control_type));
		goto out;

	}

	entity->identify_crtl_index[config_index] = htons(identify_crtl_index);

	os_log(LOG_INFO, "entity(%p): has a valid Identify control descriptor(%u) for configuration(%u)\n",
		entity, identify_crtl_index, config_index);
...
void avdecc_ipc_rx_media_stack(struct ipc_rx const *rx, struct ipc_desc *desc)
{
	...
	case GENAVB_MSG_MEDIA_STACK_ENTITY_START:
		...
		if (!desc->u.media_stack_start.entity_id) {
		...
		if (!entity) {
			os_log(LOG_ERR, "avdecc(%p) Couldn't find any entity(%"PRIu64"), ignoring GENAVB_MSG_MEDIA_STACK_ENTITY_START message from application\n",
				avdecc, ntohll(desc->u.media_stack_start.entity_id));
			goto exit;
		...
		/* If valid identify control indexes are provided, use them. Otherwise, the valid identify index
		 * capabilities will not be advertised for the entity's current configuration.
		 */
		if (desc->u.media_stack_start.flags & GENAVB_ENTITY_START_FLAGS_IDENTIFY_IDX_CFG_0_VALID)
			avdecc_set_identify_index(entity, 0, desc->u.media_stack_start.identify_control_index);

		if (desc->u.media_stack_start.flags & GENAVB_ENTITY_START_FLAGS_IDENTIFY_IDX_CFG_1_VALID)
			avdecc_set_identify_index(entity, 1, desc->u.media_stack_start.identify_control_index_cfg_1);
	...
	case GENAVB_MSG_MEDIA_STACK_ENTITY_IDENTIFY:
		if (desc->len != sizeof(struct ipc_media_stack_identify)) {
			os_log(LOG_ERR, "avdecc(%p) wrong length received %u expected %zu\n", avdecc, desc->len, sizeof(struct ipc_media_stack_identify));
			goto exit;
		}

		if (!desc->u.media_stack_identify.entity_id) {
			/* A zero entity ID will return the first non-controller entity.
			 * Only a single non-controller entity is supported anyway (for now).
			 */
			entity = avdecc_get_local_controlled_any(avdecc);
			...
			entity = avdecc_get_entity_raw(avdecc, desc->u.media_stack_identify.entity_id);
		}

		if (!entity) {
			os_log(LOG_ERR, "avdecc(%p) Couldn't find any entity(%"PRIu64"), ignoring GENAVB_MSG_MEDIA_STACK_ENTITY_IDENTIFY message from application\n",
				avdecc, ntohll(desc->u.media_stack_identify.entity_id));
			goto exit;
		}

		if (entity_ready(entity)) {
			entity->aecp.identify_sm.button_pressed = (desc->u.media_stack_identify.control_value > 0) ? true : false;

			aecp_aem_identify_sm(entity, IDENTIFY_EVENT_RUN);
		}
	...
	case GENAVB_MSG_MEDIA_STACK_BIND:
		...
		if (entity) {
			...
			os_log(LOG_INFO, "avdecc(%p) entity(%p): received GENAVB_MSG_MEDIA_STACK_BIND: Controller (%016"PRIx64") bound listener "
					"stream (Entity %016"PRIx64", Cfg %u, Index %u, %s) to talker stream (%016"PRIx64", %u) \n",
					avdecc, entity, desc->u.media_stack_bind.controller_entity_id,
					desc->u.media_stack_bind.entity_id, desc->u.media_stack_bind.listener_config_index, desc->u.media_stack_bind.listener_stream_index,
	...
	case GENAVB_MSG_MEDIA_STACK_PERSISTENT_PARAM:
		...
		if (entity) {
			u16 config_index = desc->u.media_stack_persistent_param.config_index;
			...
			os_log(LOG_INFO, "avdecc(%p): received GENAVB_MSG_MEDIA_STACK_PERSISTENT_PARAM: Entity (%016"PRIx64") "
					"Config Index %u Descriptor Type %u, Descriptor Index %u, Parameter Type %u\n",
					avdecc, ntohll(desc->u.media_stack_persistent_param.entity_id), config_index,
					desc->u.media_stack_persistent_param.desc_type, desc->u.media_stack_persistent_param.desc_index,
					desc->u.media_stack_persistent_param.param_type);

			if ((desc->u.media_stack_persistent_param.desc_type == AEM_DESC_TYPE_ENTITY)
				&& (desc->u.media_stack_persistent_param.desc_index == 0)
				&& (desc->u.media_stack_persistent_param.param_type == AVDECC_PERSISTENT_PARAM_ENTITY_CONFIGURATION_INDEX)) {

				if (avdecc_change_config(entity, config_index) < 0) {
					os_log(LOG_ERR, "avdecc(%p) Unable to change to configuration index(%u) from persistent parameter\n",
						avdecc, config_index);
					goto exit;
				}
			} else {
				if (aecp_update_persistent_param(entity, &desc->u.media_stack_persistent_param) < 0) {
					os_log(LOG_ERR, "avdecc(%p) couldn't update persistent parameter on descriptor (cfg %u, index %u, type %u)\n",
						avdecc, config_index, desc->u.media_stack_persistent_param.desc_index, desc->u.media_stack_persistent_param.desc_type);
					goto exit;
				}
			}
		} else {
	...
	case IPC_MAC_SERVICE_STATUS:
		...
		for (i = 0; i < avdecc->num_entities; i++) {
			...
			for (cfg_idx = 0; cfg_idx < ntohs(entity->desc->configurations_count); cfg_idx++) {
				...
				avb_itf_dynamic = aem_get_descriptor(entity->aem_avail_dynamic_descs[cfg_idx], AEM_DESC_TYPE_AVB_INTERFACE, port->port_id, NULL);
				if (!avb_itf_dynamic) /* Interface index not supported in this entity's configuration. */
					continue;

				if (!avb_itf_dynamic->operational_state && status->operational) {
					avb_itf_dynamic->diagnostic_counters.link_up++;

					if (entity_ready(entity) && is_current_config) {
						if (avdecc->milan_mode)
