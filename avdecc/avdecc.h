...
struct inflight_ctx {
	...
	int(*cb)(struct inflight_ctx *ctx);
...
struct entity {
	unsigned int index;
	unsigned int flags;
	struct aem_desc_hdr *aem_avail_descs[CFG_MAX_AEM_CONFIGS];
	struct aem_desc_hdr *aem_avail_dynamic_descs[CFG_MAX_AEM_CONFIGS];
	struct aem_desc_hdr *aem_descs;		/**< pointer to the static descriptors of the current configuration */
	struct aem_desc_hdr *aem_dynamic_descs;	/**< pointer to the dynamic descriptors of the current configuration */
	struct aecp_ctx aecp;
	struct acmp_ctx acmp;
	struct adp_ctx adp;
	struct entity_descriptor *desc;
	struct entity_dynamic_desc *dynamic_desc;
	...
	bool started;
	u32 system_unique_id;		/**< System-wide unique id of the PAAD. In network order. */
	int identify_crtl_index[CFG_MAX_AEM_CONFIGS];	/**< Identify control descriptor indexes (-1 means invalid). In host order. */
};
...
static inline bool avdecc_is_valid_configuration_index(struct entity *entity, u16 config_index)
{
	return (config_index < ntohs(entity->desc->configurations_count));
}

static inline bool avdecc_is_valid_current_identify_index(struct entity *entity)
{
	u16 current_config_index = ntohs(entity->desc->current_configuration);

	return (entity->identify_crtl_index[current_config_index] >= 0);
}

static inline u16 avdecc_get_current_idenitfy_index(struct entity *entity)
{
	u16 current_config_index = ntohs(entity->desc->current_configuration);
	int identify_index = entity->identify_crtl_index[current_config_index];

	return (identify_index >= 0) ? identify_index : 0;
}
