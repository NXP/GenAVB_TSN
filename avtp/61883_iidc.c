...
static int listener_stream_61883_iidc_init(struct stream_listener *stream)
{
	...
	case IEC_61883_CIP_FMT_6:
		...
		stream->subtype_data.iec61883_6.syt_interval_ln2 = avtp_61883_6_syt_interval_ln2(format, stream->set->class);
		break;
...
int listener_61883_iidc_check(struct redundant_set_listener *set, u64 const *stream_id, struct ipc_avtp_connect *ipc)
{
	...
	switch (set->format.u.s.subtype_u.iec61883.sf) {
	case IEC_61883_SF_61883:
		break;

	case IEC_61883_SF_IIDC:
	default:
		os_log(LOG_ERR, "set_id(%u) stream_id(%016"PRIx64") Stream format (%u) not supported\n",
				set->id, ntohll(*stream_id), set->format.u.s.subtype_u.iec61883.sf);
		rc = -GENAVB_ERR_STREAM_PARAMS;
		goto exit;
	}


	switch (set->format.u.s.subtype_u.iec61883.fmt) {
	case IEC_61883_CIP_FMT_4:
		if (ipc->flags & IPC_AVTP_FLAGS_MCR) {
			os_log(LOG_ERR, "set_id(%u) stream_id(%016"PRIx64") Media clock recovery not supported\n", set->id, ntohll(*stream_id));
			rc = -GENAVB_ERR_STREAM_PARAMS;
			goto exit;
		}
	...
	case IEC_61883_CIP_FMT_6:
		rc = stream_61883_6_check_format(stream_id, &set->format.u.s.subtype_u.iec61883.format_u.iec61883_6);
		if (rc < 0)
			goto exit;
	...
	default:
		os_log(LOG_ERR, "set_id(%u) stream_id(%016"PRIx64") 61883 format (%u) not supported\n",
				set->id, ntohll(*stream_id), set->format.u.s.subtype_u.iec61883.fmt);

		rc = -GENAVB_ERR_STREAM_PARAMS;
		break;
	}

	set->stream_init = &listener_stream_61883_iidc_init;
...
static void talker_stream_61883_iidc_init(struct stream_talker *stream, unsigned int *hdr_len)
{
	...
	case IEC_61883_CIP_FMT_6:
		stream->subtype_data.iec61883.syt_interval_ln2 = avtp_61883_6_syt_interval_ln2(format, stream->set->class);
...
static void talker_redundant_set_61883_iidc_init(struct redundant_set_talker *set)
{
	set->common.flags |= SET_FLAG_CLOCK_GENERATION;
}
...
int talker_61883_iidc_check(struct redundant_set_talker *set, u64 const *stream_id, struct ipc_avtp_connect *ipc)
{
	...
	set->stream_init = &talker_stream_61883_iidc_init;
	set->init = &talker_redundant_set_61883_iidc_init;
...
void avtp_61883_4_net_rx(struct stream_listener *stream, struct avtp_rx_desc **desc, unsigned int n)
{
	...
	for (i = 0; i < n; i++) {
		...
		media_desc[media_n]->flags = desc[i]->flags & (AVTP_MEDIA_CLOCK_RESTART | AVTP_PACKET_LOST);
		...
		for (offset = 0; offset < payload_size; offset += IEC_61883_4_SP_SIZE) {
			...
			media_desc[media_n]->avtp_ts[j].flags |= (desc[i]->flags & AVTP_TIMESTAMP_UNCERTAIN) ? AVTP_FLAGS_TO_MEDIA_DESC(AVTP_TIMESTAMP_UNCERTAIN) : 0;
...
void avtp_61883_6_net_rx(struct stream_listener *stream, struct avtp_rx_desc **desc, unsigned int n)
{
	...
	for (i = 0; i < n; i++) {
		...
		iec61883_6 = &stream->set->format.u.s.subtype_u.iec61883.format_u.iec61883_6;
		if (unlikely(iec61883_6->dbs != iec_hdr->dbs)) {
			stream->stats.format_err++;
			os_log(LOG_ERR,"Frame stride mismatch: configured = %d but in-stream = %d\n", avdecc_fmt_sample_stride(&stream->set->format), (iec_hdr->dbs == 0?256:iec_hdr->dbs) << 2);
		}
		...
		if (unlikely(iec61883_6->fdf_u.fdf.sfc != iec_hdr->fdf_u.fdf.sfc)) {
			stream->stats.format_err++;
			os_log(LOG_ERR,"Sampling rate mismatch: configured = %d but in-stream = %d\n", avdecc_fmt_sample_rate(&stream->set->format), avtp_61883_6_sampling_freq[iec_hdr->fdf_u.fdf.sfc]);
		}
		...
		case IEC_61883_6_FDF_EVT_INT32:
			...
			if (desc[i]->flags & AVTP_TIMESTAMP_INVALID) {
				...
				media_desc[media_n]->avtp_ts[0].flags = (desc[i]->flags & AVTP_TIMESTAMP_UNCERTAIN) ? AVTP_FLAGS_TO_MEDIA_DESC(AVTP_TIMESTAMP_UNCERTAIN) : 0;
...
void avtp_61883_4_net_tx(struct stream_talker *stream)
{
	...
	u64 media_ts64_gptp[TS_TX_BATCH], media_ts64_avtp[TS_TX_BATCH];
	struct net_tx_desc *net_desc;
	u32 ts_gptp[TS_TX_BATCH], ts_avtp[TS_TX_BATCH];
	...
	if (unlikely((rc = stream_media_rx(stream, media_desc_array, &flags, &alignment_ts)) < 0)) {
		goto media_rx_fail;
	}
	...
	if (flags & MCG_FLAGS_RESET) {
		...
		stream->ts_media_prev = ts_gptp[0];
	}

	/* Timestamps provided by applications are always in AVTP domain, convert to target gPTP domain if needed. */
	if (stream->needs_avtp_ts_conversion) {
		unsigned int media_ts_n = 0;

		/* Extract all provided media timestamps and convert them to 64-bits timestamps. */
		for (i = 0; i < rc; i++) {
			for (j = 0; j < media_desc_array[i]->ts_n; j++)
				media_ts64_avtp[media_ts_n++] = genavb_avtp_ts_32to64(media_desc_array[i]->avtp_ts[j].val, stream->avtp_current);
		}

		if (os_clock_convert_multi(stream->clock_avtp, media_ts64_avtp, media_ts_n, stream->clock_gptp, media_ts64_gptp) < 0) {
			stream->stats.gptp_err++;
		}

		for (i = 0; i < rc; i++) {
			for (j = 0; j < media_desc_array[i]->ts_n; j++)
				media_desc_array[i]->avtp_ts[j].val = (u32)media_ts64_gptp[media_ts_n++];
		}
	}
	...
	while (i < n_now) {
		...
		if (unlikely(partial))
			frames_in_packet = (net_desc->len - stream->header_len) / avdecc_fmt_sample_stride(&stream->set->format);
		else
		...
		if (unlikely(partial)) {
			...
			data_hdr->stream_data_length = htons(ntohs(data_hdr->stream_data_length) - (stream->frames_per_packet - frames_in_packet)*avdecc_fmt_sample_stride(&stream->set->format));
...
void avtp_61883_6_net_tx(struct stream_talker *stream)
{
	...
	struct media_rx_desc *media_desc_array[NET_TX_BATCH], *media_desc;
	struct net_tx_desc *net_desc;
	u32 ts_gptp[STREAM_TX_BATCH_MAX], ts_avtp[STREAM_TX_BATCH_MAX];
	struct iec_61883_hdr *iec_hdr = stream->subtype_data.iec61883.iec_hdr;
	struct avtp_data_hdr *data_hdr;
	...
	if (ts_batch > STREAM_TX_BATCH_MAX) {
		os_log(LOG_ERR, "stream(%p) Unexpected number of timestamps needed(%u), clamping down to %u\n", stream, ts_batch, STREAM_TX_BATCH_MAX);
		ts_batch = STREAM_TX_BATCH_MAX;
	}
...
//		os_log(LOG_ERR, "talker(%p) offset %u write %u read %u ts_batch %d ts_n %d\n", stream, stream->consumer.offset, stream->consumer.grid->write_index, stream->consumer.read_index, ts_batch, ts_n);
	...
	while (i < n_now) {
		...
		if (stream->media_count == stream->frame_with_ts) {
			stream->ts_launch = ts_avtp[ts_n] - stream->set->max_transit_time;

			/* Update time_per_packet based on actual data rate */
			if (stream->ts_n)
				stream->time_per_packet = ((ts_gptp[ts_n] - stream->ts_last) * stream->frames_per_packet) >> syt_interval_ln2;
		} else
		...
		if (get_ts) {
			...
			avtp_data_header_set_timestamp(stream->avtp_hdr, ts_gptp[ts_n]);

			stream->stats.timestamp_valid++;

			stream->ts_last = ts_gptp[ts_n];
			stream->ts_n++;
			ts_n++;
