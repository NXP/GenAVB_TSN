...
//		os_log(LOG_ERR, "talker(%p) offset %u write %u read %u ts_batch %d ts_n %d\n", stream, stream->consumer.offset, stream->consumer.grid->write_index, stream->consumer.read_index, ts_batch, ts_n);
	...
	if (ts_n) {
		u32 delay = ts_gptp[0] - (u32)stream->gptp_current;
	...
	while (i < n_now) {
		...
		if (unlikely(sparse)) {
			if (stream->subtype_data.aaf.tx_count == stream->frame_with_ts) {
				...
				if (stream->ts_n)
					stream->time_per_packet = (ts_gptp[ts_n] - stream->ts_last) / AAF_PACKETS_PER_TIMESTAMP_SPARSE;
			} else
...
int listener_aaf_check(struct redundant_set_listener *set, u64 const *stream_id, struct ipc_avtp_connect *ipc)
{
	...
	rc = aaf_check_format(&set->format, 0);
	if (rc < 0)
		goto err;

	set->stream_init = &listener_stream_aaf_init;
...
static void talker_redundant_set_aaf_init(struct redundant_set_talker *set)
{
...
int talker_aaf_check(struct redundant_set_talker *set, u64 const *stream_id, struct ipc_avtp_connect *ipc)
{
	...
	rc = aaf_check_format(&set->format, 1);
	if (rc < 0)
		goto err;

	set->stream_init = &talker_stream_aaf_init;
	set->init = &talker_redundant_set_aaf_init;
