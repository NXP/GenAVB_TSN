...
static void acf_tscf_net_tx(struct stream_talker *stream)
{
	...
	if ((rc = media_rx(&stream->set->media, media_desc_array, tx_batch)) < 0) {
...
//		os_log(LOG_ERR, "talker(%p) media.rx failed %d %d\n", stream, rc, stream->tx_batch);
	...
	if (rc && media_desc_array[0]->ts_n) {
		u32 delay = media_desc_array[0]->avtp_ts[0].val - (u32)stream->gptp_current;

		if ((delay < stream->set->max_transit_time) ||
			(delay > (stream_presentation_offset(stream->set->max_transit_time, stream->latency) + stream->latency))) {
	...
	while (i < n_now) {
		...
		if (media_desc->ts_n) {
			...
			stream->ts_launch = media_desc->avtp_ts[0].val - stream->set->max_transit_time;
...
static unsigned int avtp_ntscf_prepare_header(struct stream_talker *stream)
{
	...
	if (stream->set->common.flags & SET_FLAG_SR)
		hdr->sv = 1;
	else
		hdr->sv = 0;
...
static void acf_ntscf_net_rx(struct stream_listener *stream, struct avtp_rx_desc **desc, unsigned int n)
{
	...
	for (i = 0; i < n; i++) {
		...
		if (likely(stream->set->stats.media_tx)) {
...
int listener_acf_tscf_check(struct redundant_set_listener *set, u64 const *stream_id, struct ipc_avtp_connect *ipc)
{
	...
	set->stream_init = &listener_stream_acf_tscf_init;
...
int listener_acf_ntscf_check(struct redundant_set_listener *set, u64 const *stream_id, struct ipc_avtp_connect *ipc)
{
	...
	set->stream_init = &listener_acf_ntscf_init;
exit:
	return rc;
}
...
static void talker_redundant_set_acf_tscf_init(struct redundant_set_talker *set)
{
	set->common.flags |= SET_FLAG_MEDIA_WAKEUP;
}
...
int talker_acf_tscf_check(struct redundant_set_talker *set, u64 const *stream_id, struct ipc_avtp_connect *ipc)
{
	...
	if ((!ipc->talker.max_interval_frames) || (ipc->talker.max_interval_frames > sr_class_max_interval_frames(set->class))) {
		rc = -GENAVB_ERR_INVALID_PARAMS;
		goto err;
	}

	set->stream_init = &talker_stream_acf_tscf_init;
	set->init = &talker_redundant_set_acf_tscf_init;
err:
	return rc;
}

static void talker_stream_acf_ntscf_init(struct stream_talker *stream, unsigned int *hdr_len)
{
...
static void talker_redundant_set_acf_ntscf_init(struct redundant_set_talker *set)
{
	set->common.flags |= SET_FLAG_MEDIA_WAKEUP;
}
...
int talker_acf_ntscf_check(struct redundant_set_talker *set, u64 const *stream_id, struct ipc_avtp_connect *ipc)
{
	...
	set->stream_init = &talker_stream_acf_ntscf_init;
	set->init = &talker_redundant_set_acf_ntscf_init;
err:
	return rc;
}
