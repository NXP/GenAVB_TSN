...
#define GENAVB_CLOCK_SOURCE_TYPE_INVALID	((unsigned int)(-1))
...
static inline void clock_domain_source_set(struct clock_domain *domain, struct clock_source *source, unsigned int source_type)
{
				...
				struct genavb_msg_clock_domain_set_source *set_source)
{
	...
	if (IS_CLOCK_SOURCE_INPUT_SET(set_source->source_type)) {
		...
		case GENAVB_CLOCK_SOURCE_PTP_CLK:
			...
			if (source->grid.domain && (source->grid.domain != domain)) {
				os_log(LOG_ERR, "domain(%p) failed to use ptp source(%p). Already linked to domain(%p)\n",
						domain, source, source->grid.domain);
				source = NULL;
			}
			break;

		default:
			os_log(LOG_ERR, "domain(%p): unsupported internal local_id(%u)\n",
					domain, set_source->local_id);
			break;
		}
	} else {
		os_log(LOG_ERR, "domain(%p): unsupported source_type(%d)\n",
				domain, set_source->source_type);
	}

	return source;
}

/* For stream sources, caller should guarantee that this function is called once per redundant
 * stream (On first elementary stream connect or last elementary stream disconnect with null stream)
 */
int __clock_domain_update_source(struct clock_domain *domain, struct clock_source *new_source, void *data)
{
	...
	/* If the domain is now being attached with a valid set (e.g from stream_listener_create())
	 * but is still missing a valid set_id (domain previsouly updated by an elementary stream
	 * before its connection), update it now
	 */
	if (data && (domain->source_type == GENAVB_CLOCK_SOURCE_TYPE_INPUT_SET)
		 && (domain->set.id == REDUNDANT_SET_ID_INVALID)) {
		struct redundant_set_listener *set = (struct redundant_set_listener *) data;

		domain->set.id = set->id;
	}
...
{
	struct avtp_ctx *avtp = container_of(domain, struct avtp_ctx, domain[domain->id]);
	struct redundant_set_listener *set = NULL;
	struct avtp_port *port;
	int i;

	os_log(LOG_INFO, "new source(%p) for domain(%p): %d\n", new_source, domain, domain->id);

	/* If new source is an input set, look it up  */
	if (domain->source_type == GENAVB_CLOCK_SOURCE_TYPE_INPUT_SET) {
		for (i = 0; i < avtp->port_max; i++) {
			port = &avtp->port[i];

			set = redundant_set_listener_find(avtp_port_to_context(port), domain->set.id);
			if (set)
				break;
		}
	}
	...
	/*
	 * set can be NULL and attached later
	 * but domain needs to be updated with new source (set)
	 */

	return __clock_domain_update_source(domain, new_source, set);
}
...
static void clock_domain_status_indication(struct clock_domain *domain, struct ipc_tx *tx, unsigned int ipc_dst)
{
	...
	if (desc) {
		...
		} else if (domain->source_type == GENAVB_CLOCK_SOURCE_TYPE_INPUT_SET) {
			/* For backward compatiblity, keep using INPUT_STREAM notifications
			 * with both set_id and stream id information. Stream ID information
			 * is only valid for set source commands with INPUT_STREAM type (legacy API)
			 */
			desc->u.clock_domain_status.source_type = GENAVB_CLOCK_SOURCE_TYPE_INPUT_STREAM;
			os_memcpy(desc->u.clock_domain_status.set.stream_id, domain->set.stream_id, 8);
			desc->u.clock_domain_status.set.id = domain->set.id;
		}
...
{
	u64 stream_id_invalid = STREAM_INPUT_ID_INVALID;
	...
	if (set_source->source_type == GENAVB_CLOCK_SOURCE_TYPE_INTERNAL) {
		if ((domain->source_type == GENAVB_CLOCK_SOURCE_TYPE_INTERNAL)
			&& (domain->local_id == set_source->local_id))
			goto skip_update; /* Same source, skip domain update. */

		domain->local_id = set_source->local_id;
		domain->source_type = GENAVB_CLOCK_SOURCE_TYPE_INTERNAL;
	} else if (set_source->source_type == GENAVB_CLOCK_SOURCE_TYPE_INPUT_STREAM) {
		/* If using legacy elementary stream command, lookup the set id if the stream is connected
		 * If not found, set to invalid to be updated later on stream connect.
		 */
		...
		for (i = 0; i < avtp->port_max; i++) {
			...
			stream_source = stream_listener_find(port, set_source->stream_id);
			if (stream_source)
				break;
		}

		set_id = (stream_source) ? stream_source->set->id : REDUNDANT_SET_ID_INVALID;

		if (domain->source_type == GENAVB_CLOCK_SOURCE_TYPE_INPUT_SET) {
			/* If same set id or same elementary stream id, no need to update the domain.
			 * Since STREAM_INPUT_ID_INVALID is always used for the INPUT_SET set sources below,
			 * make sure that legacy set sources with INPUT_STREAM having invalid stream id
			 * also resets the domain if necessary.
			 */
			if ((set_id == domain->set.id) ||
							(os_memcmp(set_source->stream_id, &stream_id_invalid, 8)
							 && !os_memcmp(domain->set.stream_id, set_source->stream_id, 8)))
				goto skip_update;
		}

		domain->source_type = GENAVB_CLOCK_SOURCE_TYPE_INPUT_SET;
		copy_64(domain->set.stream_id, set_source->stream_id);
		domain->set.id = set_id;
	} else if (set_source->source_type == GENAVB_CLOCK_SOURCE_TYPE_INPUT_SET) {

		if ((domain->source_type == GENAVB_CLOCK_SOURCE_TYPE_INPUT_SET)
		     && (set_source->set_id == domain->set.id))
			/* Updates with the same set_input source: skip domain update. */
			goto skip_update;

		domain->source_type = GENAVB_CLOCK_SOURCE_TYPE_INPUT_SET;
		copy_64(domain->set.stream_id, &stream_id_invalid);
		domain->set.id = set_source->set_id;
	} else {
		os_log(LOG_ERR, "domain(%p): unsupported set clock source type(%d)\n", domain, set_source->source_type);
		goto err;
	}
...
unsigned int clock_domain_is_source_set(struct clock_domain *domain, u16 set_id, void *stream_id)
{
	if ((!domain->source) || (domain->source_type != GENAVB_CLOCK_SOURCE_TYPE_INPUT_SET))
		return 0;

	if (domain->set.id != REDUNDANT_SET_ID_INVALID) {
		return (domain->set.id == set_id);
	} else {
		return cmp_64(domain->set.stream_id, stream_id);
	}
}
...
{
	genavb_clock_source_type_t new_source_type;

	/* Save the requested new source type to avoid its override on domain
	 * invalidation.
	 */
	new_source_type = domain->source_type;

	/* Close previous source */
	if (domain->source) {
		clock_source_close(domain->source);
		clock_domain_remove_grid(&domain->source->grid);
		clock_domain_source_set(domain, NULL, GENAVB_CLOCK_SOURCE_TYPE_INVALID);
	}

	/* Add new source */
	clock_domain_source_set(domain, source, new_source_type);
