...
struct clock_domain {
	unsigned int id;
	unsigned int source_type; 	/* GENAVB_CLOCK_SOURCE_TYPE_INPUT_SET, GENAVB_CLOCK_SOURCE_TYPE_INTERNAL or GENAVB_CLOCK_SOURCE_TYPE_INVALID. */
	union {
		u16 local_id;
		struct {
			u16 id;		 /* ID of the redundant set */
			u8 stream_id[8]; /* Only valid for backward compatibility for set source with INPUT_STREAM. Otherwise, set to STREAM_INPUT_ID_INVALID. */
		} set;
	};
