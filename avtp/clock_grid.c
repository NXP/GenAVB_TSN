...
{
	grid->valid_count = genavb_min(grid->count, grid->max_valid_count);
}

unsigned int clock_grid_start_count(struct clock_grid *grid)
{
	return genavb_min(grid->count, grid->max_start_count);
}
...
int clock_grid_init(struct clock_grid *grid, clock_grid_producer_type_t type, u32 *ring_base, unsigned int ring_size, u32 nominal_freq_p, u32 nominal_freq_q, void (*ts_update)(struct clock_grid *grid, unsigned int requested, unsigned int *reset))
{
	int rc = 0;
	...
	grid->max_valid_count = genavb_min(grid->ring_size / 2, grid->nominal_freq_p / (grid->nominal_freq_q * (USECS_PER_SEC / CLOCK_GRID_VALID_TIME_US)));
	...
	grid->max_start_count = genavb_min(grid->ring_size / 4, grid->nominal_freq_p / (2 * grid->nominal_freq_q * (USECS_PER_SEC / CLOCK_GRID_VALID_TIME_US)));
