...
static void clock_source_hw_update(struct clock_grid *grid, unsigned int requested, unsigned int *reset)
{
	...
	if (requested)
		avail = genavb_min(avail, requested);
...
static int clock_source_stream_open(struct clock_source *source, struct redundant_set_listener *set)
{
	struct clock_grid *grid = &source->grid;
	...
	ts_freq_p = avdecc_fmt_sample_rate(&set->format);
	ts_freq_q = avdecc_fmt_samples_per_timestamp(&set->format, set->class);
	...
	/*
	 * Source is not yet bound to a redundant set.
	 */
	if (!grid->producer.u.stream.set) {
		if (clock_producer_stream_open(grid, set, set->domain->hw_sync, ts_freq_p, ts_freq_q) < 0) {
			os_log(LOG_ERR, "set(%p) id(0x%x) clock_source_stream_open error\n", set, set->id); //FIXME bad identifier ?
			goto err;
		}
		...
		/* Legacy, do not fail is source is already setup on another set */
		goto err;
#endif
	return 0;
err:
...
static void clock_source_stream_close(struct clock_source *source)
{
	...
	if (grid->producer.u.stream.set) {
		grid->producer.u.stream.set->source = NULL;
...
__init static int clock_source_hw_init(struct clock_source *source, clock_grid_producer_type_t type, int id)
{
	...
	source->min_valid_count = genavb_min(grid->ring_size / 4, grid->nominal_freq_p / (grid->nominal_freq_q * (USECS_PER_SEC / CLOCK_SOURCE_MIN_VALID_TIME_US)));
