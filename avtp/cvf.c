...
static void avtp_cvf_h264_net_rx(struct stream_listener *stream, struct avtp_rx_desc **desc, unsigned int n)
{
	...
	spec_version = stream->set->format.u.s.subtype_u.cvf.format_u.h264.spec_version;
		...
		* - FIXME: We should send avtp timestamp with the h264 timestamp, for now we send
		...
		if (h264_avtp_hdr->M || (is_last_fu) || (is_single_packet) || (is_aggregation)) {
			...
			if (!h264_avtp_hdr->ptv) {
				...
				if (desc[i]->flags & AVTP_TIMESTAMP_UNCERTAIN)
					media_desc[media_n]->avtp_ts[media_desc[media_n]->n_ts].flags |= AVTP_FLAGS_TO_MEDIA_DESC(AVTP_TIMESTAMP_UNCERTAIN);
...
static void avtp_cvf_h264_net_tx(struct stream_talker *stream)
{
	...
	u32 ts[TS_TX_BATCH], ts_avtp[TS_TX_BATCH];
	int rc;
	int ts_n = 0, ts_idx;
	...
	ts_n = media_clock_gen_get_ts(stream, ts, ts_batch, ts_avtp, &flags, alignment_ts);
	...
	/* Timestamps provided by applications are always in AVTP media clock domain, convert to target gPTP domain if needed. */
...
static void cvf_mjpeg_net_rx(struct stream_listener *stream, struct avtp_rx_desc **desc, unsigned int n)
{
	...
	for (i = 0; i < n; i++) {
		...
		mjpeg = &stream->set->format.u.s.subtype_u.cvf.format_u.mjpeg;
		...
		if (mjpeg_avtp_hdr->M) {
			/* Signal the End-of-Frame on the last byte of the packet */
			media_desc[media_n]->avtp_ts[media_desc[media_n]->n_ts].offset = media_desc[media_n]->len - 1;
			media_desc[media_n]->avtp_ts[media_desc[media_n]->n_ts].flags = AVTP_FLAGS_TO_MEDIA_DESC(AVTP_END_OF_FRAME);
...
int stream_cvf_mjpeg_check_format(u64 const *stream_id, struct avdecc_format_cvf_mjpeg_t const *mjpeg)
{
	...
	if ((!mjpeg->width || !mjpeg->height)) {
		os_log(LOG_ERR, "stream_id(%016"PRIx64") MJPEG height/width (%u,%u) parameters cannot be 0\n",
				ntohll(*stream_id), mjpeg->height, mjpeg->width);
		goto err_format;
	}
...
int listener_cvf_check(struct redundant_set_listener *set, u64 const *stream_id, struct ipc_avtp_connect *ipc)
{
	...
	switch (set->format.u.s.subtype_u.cvf.format) { /* P1722_D14 table 19 */
	case CVF_FORMAT_RFC:
		break;

	default:
		os_log(LOG_ERR, "set_id(%u) stream_id(%016"PRIx64") Compressed Video Format (%u) not supported\n",
				set->id, ntohll(*stream_id), set->format.u.s.subtype_u.cvf.format);
		goto err_format;
	}
	...
	switch (set->format.u.s.subtype_u.cvf.subtype) { /* P1722_D14 table 20 */
	case CVF_FORMAT_SUBTYPE_MJPEG:
		rc = stream_cvf_mjpeg_check_format(stream_id, &set->format.u.s.subtype_u.cvf.format_u.mjpeg);
		break;
	...
	default:
		os_log(LOG_ERR, "set_id(%u) stream_id(%016"PRIx64") CVF subtype (%u) not supported\n",
				set->id, ntohll(*stream_id), set->format.u.s.subtype_u.cvf.subtype);
		goto err_format;
	}

	set->stream_init = &listener_stream_cvf_init;
...
static void talker_redundant_set_cvf_init(struct redundant_set_talker *set)
{
...
int talker_cvf_check(struct redundant_set_talker *set, u64 const *stream_id, struct ipc_avtp_connect *ipc)
{
	...
	switch (set->format.u.s.subtype_u.cvf.format) {
	case CVF_FORMAT_RFC:
		break;
	...
	switch (set->format.u.s.subtype_u.cvf.subtype) {
	...
	set->stream_init = &talker_stream_cvf_init;
	set->init = &talker_redundant_set_cvf_init;
