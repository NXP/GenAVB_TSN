...
static void stats_task(void *pvParameters)
{
	...
	if (ipc_rx_init(&ipc_rx_stats, IPC_AVTP_STATS, stats_ipc_rx, (unsigned int)event_queue_h) < 0)
		goto err_ipc_rx;
...
{
	struct avtp_config *avtp_cfg = pvParameters;
	 ...
	 * Create stats task
	 */
	if (xTaskCreate(stats_task, STATS_TASK_NAME, STATS_CFG_STACK_DEPTH, NULL, STATS_CFG_PRIORITY, &stats_task_h) != pdPASS) {
 ...
 *
 */
__init TaskHandle_t avtp_task_init(struct avtp_config *avtp_cfg)
{
	TaskHandle_t task;
	BaseType_t rc;

	rc = xTaskCreate(avtp_task, AVTP_TASK_NAME, AVTP_CFG_STACK_DEPTH, avtp_cfg, AVTP_CFG_PRIORITY, &task);
	if (rc != pdPASS) {
		os_log(LOG_ERR, "xTaskCreate(%s) failed\n", AVTP_TASK_NAME);
		goto err;
	}
...
__exit void avtp_task_exit(TaskHandle_t task)
{
}
