...
restart:
	if (os_clock_gettime32(avtp_to_clock_avtp(CFG_DEFAULT_PORT_ID), &local_time) < 0)
		goto exit;
...
//			os_log(LOG_ERR, "clock(%p) timestamp uncertain\n", rec);
		...
		case INIT:
			...
			rec->max_offset_val = genavb_min((3 * rec->array_size / 4) * period, 100 * NS_PER_MS);
...
init:
	if (mult->state == GEN_INIT) {
		 ...
		 * checked
		 */
		if (os_clock_gettime32(avtp_to_clock_avtp(CFG_DEFAULT_PORT_ID), &alignment_ts) < 0)
			return;
...
{
	int n_ts = genavb_min(10, ts_available(consumer));
	unsigned int mean = 0;
...
int clock_grid_consumer_get_ts(struct clock_grid_consumer *consumer, u32 *ts, unsigned int ts_n, unsigned int *flags, unsigned int alignment_ts)
{
	...
	ts_avail = ts_available(consumer);
	ts_n_actual = genavb_min(ts_n, ts_avail);
	if (ts_n_actual < ts_n) {
	...
	while (written < ts_n_actual) {
		*ts = ts_get(consumer);
		...
		if (!written)
			stats_update(&consumer->stats.ts_err, (int)*ts - (int)consumer->avtp_current);
...
/* Get grid timestamps in both domains (AVTP and gPTP target domain). */
int media_clock_gen_get_ts(struct stream_talker *stream, u32 *ts_gptp, unsigned int ts_n, u32 *ts_avtp, unsigned int *flags, unsigned int alignment_ts)
{
	u64 ts64_avtp[TS_TX_BATCH], ts64_gptp[TS_TX_BATCH];
	int num_ts = 0;
	int i;

	if (ts_n > TS_TX_BATCH) {
		os_log(LOG_ERR, "stream(%p): maximum timestamps(%u) exceeded(%u)\n", stream, TS_TX_BATCH, ts_n);
		goto out;
	}

	num_ts = clock_grid_consumer_get_ts(&stream->consumer, ts_avtp, ts_n, flags, alignment_ts);
	/* Grid timestamps are in AVTP media clock domain, convert them to target gptp domain if needed. */
	if (stream->needs_avtp_ts_conversion) {

		for (i = 0; i < num_ts; i++)
			ts64_avtp[i] = genavb_avtp_ts_32to64(ts_avtp[i], stream->avtp_current);

		if (os_clock_convert_multi(stream->clock_avtp, ts64_avtp, num_ts, stream->clock_gptp, ts64_gptp) < 0) {
			stream->stats.gptp_err++;
		}

		for (i = 0; i < num_ts; i++)
			ts_gptp[i] = (u32)ts64_gptp[i];

	} else {
		os_memcpy(ts_gptp, ts_avtp, num_ts * sizeof(u32));
	}
...
int clock_producer_stream_open(struct clock_grid *grid, struct redundant_set_listener *set, struct media_clock_rec *rec,
					unsigned int ts_freq_p, unsigned int ts_freq_q)
{
	struct clock_grid_producer_stream *producer = &grid->producer.u.stream;
	...
	os_log(LOG_INFO, "set(0x%x, %p) rec(%p)\n", set->id, set, rec);
