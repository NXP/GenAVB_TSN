...
#define STREAM_TX_LATE_EXTRA_MEDIA	1	/* Number of extra (in addition to tx_batch) packets to be read from media queue to recover from late received packets. */
#define STREAM_TX_BATCH_MAX	(STREAM_TX_BATCH + STREAM_TX_LATE_EXTRA_MEDIA)
...
struct stream_listener {
	...
	void (*net_rx)(struct stream_listener *stream, struct avtp_rx_desc **desc, unsigned int n);
...
struct stream_talker {
	...
	void (*net_tx)(struct stream_talker *stream);
