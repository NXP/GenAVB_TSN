...
bool avdecc_fmt_supports_milan_redundancy(const struct avdecc_format *format, sr_class_t sr_class)
{
	...
	case AVTP_SUBTYPE_AAF:
	{
		...
		/* Milan Base Audio stream Format: Milan v1.2 Table 6.2 */
		if ((aaf_fmt->format == AAF_FORMAT_INT_32BIT)
			&& (AVDECC_FMT_AAF_PCM_CHANNELS_PER_FRAME(format) >= 1) && (AVDECC_FMT_AAF_PCM_CHANNELS_PER_FRAME(format) <= 8)
			&& (((aaf_fmt->nsr == AAF_NSR_48000) && (AVDECC_FMT_AAF_PCM_SAMPLES_PER_FRAME(format) == 6))
				|| ((aaf_fmt->nsr == AAF_NSR_96000) && (AVDECC_FMT_AAF_PCM_SAMPLES_PER_FRAME(format) == 12))
				|| ((aaf_fmt->nsr == AAF_NSR_192000) && (AVDECC_FMT_AAF_PCM_SAMPLES_PER_FRAME(format) == 24))))
			is_supported = true;
		break;
	}
	case AVTP_SUBTYPE_CRF:
	{
		...
		/* Avnu Pro Audio CRF Media Clock Stream: Milan v1.2 Table 7.1 */
		if ((crf_fmt->pull == CRF_PULL_1_1)
			&& (AVDECC_FMT_CRF_BASE_FREQUENCY(format) == 48000)
			&& (AVDECC_FMT_CRF_TIMESTAMP_INTERVAL(format) == 96)
			&& (crf_fmt->type == CRF_TYPE_AUDIO_SAMPLE)
			&& (crf_fmt->timestamps_per_pdu == 1))
			is_supported = true;
