...
#define ipc_media_stack_persistent_param genavb_msg_media_stack_persistent_param
#define ipc_media_stack_memory_object genavb_msg_media_stack_memory_object
#define ipc_media_stack_start genavb_msg_media_stack_start
#define ipc_media_stack_identify genavb_msg_media_stack_identify
...
struct ipc_avtp_listener_connect_response {
	...
	os_clock_id_t clock_avtp; /* Clock of the stream's AVTP timestamps */
};
...
struct ipc_avtp_counters {
	u32 stream_listener_count;
	u32 stream_talker_count;

	u8 data[]; /* Flexible size array: ipc_avtp_listener_counters * stream_listener_count followed by ipc_avtp_talker_counters * stream_talker_count */
};

struct ipc_avtp_listener_counters {
	u64 stream_id;
	struct {
		u32 rx;
		u32 pkt_lost;
...
struct ipc_avtp_talker_counters {
	u64 stream_id;
	struct {
		u32 tx;
		u32 mr;
...
#define IPC_AVTP_FLAGS_MAX_TRANSIT_TIME_VALID   GENAVB_STREAM_FLAGS_MAX_TRANSIT_TIME_VALID
#define IPC_AVTP_FLAGS_REDUNDANT_STREAM         GENAVB_STREAM_FLAGS_REDUNDANT
#define IPC_AVTP_FLAGS_SET_ID_VALID             GENAVB_STREAM_FLAGS_SET_ID_VALID
