...
struct list {
	...
	uintptr_t (*dynamic_handler)(void *data, uintptr_t base);
...
struct leaf {
	...
	enum leaf_type type;
	uint8_t *val;
	void (*handler)(void *data, struct leaf *l, enum node_operation operation, uint8_t *buf, uintptr_t base);
	void *handler_data;
};
...
struct node *list_init(struct list *list, const char *name, uintptr_t (*handler)(void *data, uintptr_t base), void *handler_data, uint16_t num_keys, uint16_t *keys_id);
...
struct node *leaf_init(struct leaf *leaf, const char *name, enum leaf_type type, enum leaf_flags flags, void *storage,
			void (*handler)(void *data, struct leaf *l, enum node_operation operation, uint8_t *buf, uintptr_t base), void *handler_data);
