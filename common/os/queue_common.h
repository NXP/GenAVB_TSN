...
struct queue {
	...
	common_os_atomic_t read;
	common_os_atomic_t write;
...
static inline void queue_init(struct queue *q, void (*entry_free)(void *data, unsigned long entry), unsigned int extra)
{
	...
	common_os_atomic_set(&q->read, 0);
	common_os_atomic_set(&q->write, 0);
}
...
{
	u32 read = common_os_atomic_read(&q->read);
	u32 write = common_os_atomic_read(&q->write);
...
{
	*write = common_os_atomic_read(&q->write);
}
...
{
	smp_wmb();
	common_os_atomic_set(&q->write, write);
}
...
{
	*read = common_os_atomic_read(&q->read);
}
...
{
	smp_wmb();
	common_os_atomic_set(&q->read, read);
}
...
static inline unsigned long queue_peek(struct queue *q)
{
	if (queue_empty(q))
		...
		return q->entry[common_os_atomic_read(&q->read)];
}
