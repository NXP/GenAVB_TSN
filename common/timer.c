 ...
 * Copyright 2016, 2018-2021, 2023, 2025-2026 NXP
 *
...
__init unsigned int timer_pool_size(unsigned int n)
{
	return sizeof(struct timer_ctx) + n * sizeof(struct timer_sys);
}

__init int timer_pool_init(struct timer_ctx *tctx, unsigned int n, unsigned long priv)
{
	int i;
...
__exit void timer_pool_exit(struct timer_ctx *tctx)
{
	os_log(LOG_INFO, "timer_ctx(%p)\n", tctx);
}
