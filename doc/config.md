...
serial_number		| 64 bytes UTF8 string (default: none) | Serial number of the entity. If not configured, the one defined in the AEM entity description file will be used.
...
channel_waitmask	| bitmask, between 0x0 and 0xf included. default: 0x0	| The stack waits for the specified control channels to be opened (or specific command to be received) before starting AVDECC for the entity: bit 0 => MEDIA_STACK channel, bit 1 => CONTROLLER channel, bit 2 => CONTROLLED channel, bit 3 => wait for START command in the MEDIA_STACK channel.
...
* Audio Milan redundant talker/listener, AAF/CRF streams
  + AVDECC Entity configuration file: listener_talker_audio_redundant_milan.aem
