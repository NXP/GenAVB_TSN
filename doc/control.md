...
(send, receive) control messages through different control channels. Several channel types are currently defined (see @ref genavb_control_id_t):
* @ref GENAVB_CTRL_AVDECC_MEDIA_STACK : this channel can be used by a talker or listener application to be notified about multiple AVDECC events
and needs application's intervention like: stream connections/disconnections (either by an external controller, or through the fast-connect mechanism),
handling clock source changes, handling persistent parameters changes and even startng the AVDECC entity advertisement on the network. (see table below for full description
of messages supported in this channel)
...
* @ref GENAVB_CTRL_MAAP : this channel can be used by a talker or listener application to create or delete MAAP ranges.
* @ref GENAVB_CTRL_MSRP : this channel can be used by a talker or listener application, on a system with an endpoint stack running on port 0, to establish stream reservations and be notified
...
* @ref GENAVB_CTRL_MVRP : this channel can be used by a talker or listener application, on a system with an endpoint stack running on port 0,  to establish VLAN registrations.
domain and receive clock domain status indications.
* @ref GENAVB_CTRL_GPTP : this channel can be used by a talker or listener application, on a system with an endpoint stack running on port 0, to retrieve grand master information from GPTP
stack component.
* @ref GENAVB_CTRL_MSRP_BRIDGE : this channel can be used by a application, on a system with a bridge stack, to establish stream reservations and be notified
about the status of existing reservations on the network.
* @ref GENAVB_CTRL_MVRP_BRIDGE : this channel can be used by a application, on a system with a bridge stack, to establish VLAN registrations.
domain and receive clock domain status indications.
* @ref GENAVB_CTRL_GPTP_BRIDGE : this channel can be used by a application, on a system with a bridge stack, to retrieve grand master information from GPTP
stack component.
* @ref GENAVB_CTRL_MSRP_1 : this channel can be used by a talker or listener application, on a system with an endpoint stack running on port 1, to establish stream reservations and be notified
about the status of existing reservations on the network. If AVDECC is not being used, this API becomes mandatory to be able to
transmit and receive AVTP streams in an AVB network.
* @ref GENAVB_CTRL_MVRP_1 : this channel can be used by a talker or listener application, on a system with an endpoint stack running on port 1,  to establish VLAN registrations.
domain and receive clock domain status indications.
* @ref GENAVB_CTRL_GPTP_1 : this channel can be used by a talker or listener application, on a system with an endpoint stack running on port 1, to retrieve grand master information from GPTP
stack component.
...
* @ref GENAVB_MSG_MEDIA_SET_CLOCK_SOURCE, @ref GENAVB_MSG_MEDIA_STACK_BIND, @ref GENAVB_MSG_MEDIA_STACK_UNBIND, @ref GENAVB_MSG_MEDIA_STACK_PERSISTENT_PARAM, @ref GENAVB_MSG_MEDIA_STACK_ENTITY_START and @ref GENAVB_MSG_MEDIA_STACK_ENTITY_IDENTIFY
* @ref GENAVB_MSG_AECP
...
* @ref GENAVB_MSG_GPTP_PORT_PARAMS
* @ref GENAVB_MSG_MAAP_CREATE_RANGE, @ref GENAVB_MSG_MAAP_CREATE_RANGE_RESPONSE, @ref GENAVB_MSG_MAAP_DELETE_RANGE, @ref GENAVB_MSG_MAAP_DELETE_RANGE_RESPONSE and @ref GENAVB_MSG_MAAP_STATUS
* @ref GENAVB_MSG_MANAGED_GET, @ref GENAVB_MSG_MANAGED_GET_RESPONSE, @ref GENAVB_MSG_MANAGED_SET and @ref GENAVB_MSG_MANAGED_SET_RESPONSE

Each message type (except messages specific for managed objects) has a corresponding message C structure (defined in detail in the GenAVB/TSN public headers) and
...
MEDIA_STACK_BIND		| genavb_msg_media_stack_bind			| GENAVB_CTRL_AVDECC_MEDIA_STACK				| FROM/TO stack	| Indication
MEDIA_STACK_UNBIND		| genavb_msg_media_stack_unbind			| GENAVB_CTRL_AVDECC_MEDIA_STACK				| FROM stack	| Indication
MEDIA_STACK_SET_CLOCK_SOURCE	| genavb_msg_clock_domain_set_source 		| GENAVB_CTRL_AVDECC_MEDIA_STACK				| FROM stack	| Indication
MEDIA_STACK_PERSISTENT_PARAM	| genavb_msg_media_stack_persistent_param 	| GENAVB_CTRL_AVDECC_MEDIA_STACK				| FROM/TO stack	| Indication
MEDIA_STACK_ENTITY_START	| genavb_msg_media_stack_start 			| GENAVB_CTRL_AVDECC_MEDIA_STACK				| TO stack	| Indication
...
LISTENER_DECLATAION_STATUS	| genavb_msg_listener_declaration_status	| GENAVB_CTRL_MSRP						| FROM stack	| Indication
...
TALKER_DECLARATION_STATUS	| genavb_msg_talker_declaration_status		| GENAVB_CTRL_MSRP						| FROM stack	| Indication
...
* receive persistent parameters change notifications
* send persistent parameters and sending indication to start entity advertising on the network.
...
This channel can be used by a media stack application to be notified about stream connections/disconnections, audio domains' clock sources, and changes in
the AVDECC persistent parameters (including binding parameters) for Milan. It can be used in the other direction by the application to notify the stack
about saved persistent parameters and to synchronize the AVDECC entity startup.

### Changes in the stream connections/disconnections
...
### Clock sources notifications

The AVDECC stack notifies the stack about clock source informations either on stack startup with the default clock sources in the AEM entity for the different
clock domains, or on changes of the sources triggered by an AVDECC controller action. the stack will send a @ref GENAVB_MSG_MEDIA_SET_CLOCK_SOURCE message, whose
content maps to the @ref genavb_msg_clock_domain_set_source structure. The stack normally uses the information in this message to initiate a clock domain change
the clock domain control API mentioned above.

### Persistent parameters exchanges

When one of the Milan specified persistent parameters are changed the stack notifies the application to save the new parameters in non-volatile memory.
Then, the application can, on power cycle for example, provide the persistent parameters back to the stack before starting the AVDECC entity.
The following persistent parameters are supported:
* Binding stream parameters: The stack notifies the application with bind/unbind events using the @ref GENAVB_MSG_MEDIA_STACK_BIND and @ref GENAVB_MSG_MEDIA_STACK_UNBIND messages. The stack
can use the @ref GENAVB_MSG_MEDIA_STACK_BIND to send back any previous binding parameters after a restart (and before sending an @ref GENAVB_MSG_MEDIA_STACK_ENTITY_START)
* AEM Descriptors persistent parameters: On changes of some of the descriptors parameters (through AECP commands), the stack notifies the application about the new parameters with a @ref GENAVB_MSG_MEDIA_STACK_PERSISTENT_PARAM. The application can use the same message (multiple ones if needed) to send back any previous persistent parameters after a restart (and before sending an @ref GENAVB_MSG_MEDIA_STACK_ENTITY_START). Refer to @ref genavb_msg_media_stack_persistent_param and @ref genavb_avdecc_persistent_param_t for the supported parameters.

### Avdecc entity start

To make sure that the stack takes into account all sent persistent params, the application should use the @ref GENAVB_MSG_MEDIA_STACK_ENTITY_START to make the stack start the AEM entity. It uses a @ref genavb_msg_media_stack_start structure.

Notes: 
* Using an entity ID of 0 make the stack start the first non-controller entity.
* The stack should be configured with the proper channel_waitmask to make the stack wait for the @ref GENAVB_MSG_MEDIA_STACK_ENTITY_START message before entity startup.
...
AECP_AEM_CMD_ACQUIRE_ENTITY                      | Partial (1)          | N
AECP_AEM_CMD_ENTITY_AVAILABLE                    | Y                    | N
AECP_AEM_CMD_LOCK_ENTITY                         | Y                    | N
AECP_AEM_CMD_SET_CONFIGURATION                   | Y (2)                | N
AECP_AEM_CMD_GET_CONFIGURATION                   | Y                    | N
AECP_AEM_CMD_SET_STREAM_FORMAT                   | Y                    | N
AECP_AEM_CMD_GET_STREAM_FORMAT                   | Y                    | N
...
AECP_AEM_CMD_GET_AVB_INFO                        | Y (3)                | N
AECP_AEM_CMD_SET_STREAM_INFO                     | Y                    | N
AECP_AEM_CMD_GET_STREAM_INFO                     | Y                    | N
AECP_AEM_CMD_SET_NAME                            | Y                    | N
AECP_AEM_CMD_GET_NAME                            | Y                    | N
AECP_AEM_CMD_SET_SAMPLING_RATE                   | Y (4)                | N
AECP_AEM_CMD_GET_SAMPLING_RATE                   | Y                    | N
AECP_AEM_CMD_SET_CLOCK_SOURCE                    | Y                    | Y (5)
AECP_AEM_CMD_GET_CLOCK_SOURCE                    | Y                    | N
AECP_AEM_CMD_START_STREAMING                     | Y                    | Y
AECP_AEM_CMD_STOP_STREAMING                      | Y                    | Y
AECP_AEM_CMD_GET_COUNTERS                        | Y                    | N
AECP_AEM_CMD_ADD_AUDIO_MAPPINGS                  | Y                    | Y
AECP_AEM_CMD_REMOVE_AUDIO_MAPPINGS               | Y                    | Y
AECP_AEM_CMD_GET_AUDIO_MAP                       | Y                    | Y
AECP_AEM_CMD_GET_AS_PATH                         | Y                    | N
AECP_AEM_CMD_GET_DYNAMIC_INFO                    | Y                    | N
Others                                           | N                    | N

> (1) The ACQUIRE_ENTITY command is implemented, with the following limitations:
...
> (2) SET_CONFIGURATION command is implemented but only a single configuration is supported at stack level for the ENTITY descriptor.

> (3) GET_AVB_INFO command is implemented with the following limitations:
> * MSRP mappings are using default parameters and not SRP domain indications.
> * gPTP propagation delay information is not supported.

> (4) SET_SAMPLING_RATE command is implemented but only a single rate is supported at stack level for the AUDIO_UNIT descriptor.

> (5) SET_CLOCK_SOURCE command is handled to the application but through a @ref GENAVB_MSG_MEDIA_SET_CLOCK_SOURCE message in the GENAVB_CTRL_AVDECC_MEDIA_STACK channel and not directly in the GENAVB_CTRL_AVDECC_CONTROLLED channel.
...
#### Handling of AECP_AEM_CMD_ADD_AUDIO_MAPPINGS and AECP_AEM_CMD_REMOVE_AUDIO_MAPPINGS command
When receiving an ADD_AUDIO_MAPPINGS or REMOVE_AUDIO_MAPPINGS command from the network, the stack will (in that order):
* Make sure the requested STREAM_PORT descriptor can be found in the AEM model loaded in memory and is not read-only,
* Make sure that the mappings contained in the command are valid per the specification.
* Make sure that any mapping referencing a redundant stream is duplicated (if not already there) before being sent to application.
* Pass the command to the application,
* Send IN_PROGRESS responses to the requesting controller until the application provides a response.

#### Handling of AECP_AEM_CMD_GET_AUDIO_MAP command
When receiving a GET_AUDIO_MAP command from the network, the stack will (in that order):
* Make sure the requested STREAM_PORT descriptor can be found in the AEM model loaded in memory and is not read-only,
* Pass the command to the application,
* Send IN_PROGRESS responses to the requesting controller until the application provides a response.
