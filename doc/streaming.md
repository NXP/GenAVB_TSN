...
Event timestamps for both directions (talker and listener) described below shall be in the AVTP
media clock domain associated to the stream. The application shall use ::genavb_stream_avtp_clock to get
the right clock of the stream. The returned clock may be used with the clock API to get the current
time and perform the needed stream synchronization.
