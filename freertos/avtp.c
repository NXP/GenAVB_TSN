...
void avtp_socket_unbind(struct net_socket *sock)
{
	...
	if (is_avtp_stream(addr->subtype) || is_avtp_alternative(addr->subtype)) {
		...
		case AVTP_SUBTYPE_MAAP:
			generic_unbind(avtp_rx_hdlr.maap.rx_hdlr, sock);
			break;
#endif
		case AVTP_SUBTYPE_AVDECC:
			generic_unbind(avtp_rx_hdlr.avdecc.rx_hdlr, sock);
			break;
...
#if 0
		case AVTP_SUBTYPE_MAAP:
			generic_bind(avtp_rx_hdlr.maap.rx_hdlr, sock, addr->port);
			break;
#endif
		case AVTP_SUBTYPE_AVDECC:
			generic_bind(avtp_rx_hdlr.avdecc.rx_hdlr, sock, addr->port);
			break;
...
int avtp_socket_connect(struct logical_port *port, struct net_socket *sock, struct net_address *addr)
{
	...
	if (is_avtp_stream(addr->u.avtp.subtype) || is_avtp_alternative(addr->u.avtp.subtype)) {
		...
		sock->tx.qos_queue = qos_queue_connect(net_port->qos, AVDECC_DEFAULT_PRIORITY, &sock->tx.queue, 0);
		if (!sock->tx.qos_queue)
			goto err;
...
static int avtp_stream_rx(struct net_rx_ctx *net, struct net_port *phys_port, struct net_rx_desc *desc, struct avtp_common_hdr *avtp, struct net_rx_stats *stats)
{
...
static int avtp_alternative_rx(struct net_rx_ctx *net, struct net_port *phys_port, struct net_rx_desc *desc, struct avtp_common_hdr *avtp, struct net_rx_stats *stats)
{
	void *stream_id;
	struct net_socket *sock;
	struct logical_port *port = physical_to_logical_port(phys_port);
...
static int avtp_control_rx(struct net_rx_ctx *net, struct net_port *phys_port, struct net_rx_desc *desc, struct avtp_common_hdr *avtp, struct net_rx_stats *stats)
{
	struct net_socket *sock;
	struct logical_port *port = physical_to_logical_port(phys_port);
...
int avtp_rx(struct net_rx_ctx *net, struct net_port *phys_port, struct net_rx_desc *desc, void *hdr, unsigned int is_vlan)
{
	...
	if (likely(is_avtp_stream(avtp->subtype))) {
		...
		if (unlikely(!is_vlan))
			goto drop;
#endif
		return avtp_stream_rx(net, phys_port, desc, avtp, stats);
	} else if (is_avtp_alternative(avtp->subtype))
		return avtp_alternative_rx(net, phys_port, desc, avtp, stats);

	return avtp_control_rx(net, phys_port, desc, avtp, stats);
