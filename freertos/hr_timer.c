...
#define HR_TIMER_STACK_DEPTH		(configMINIMAL_STACK_SIZE + 64)
...
static int hr_timer_task_schedule(struct hr_timer_task_ctx *timer_task, uint64_t wake_time)
{
	...
	period = pdMS_TO_TICKS_AT_LEAST(wake_time / NSECS_PER_MS, 1);

	if (xTimerChangePeriod(timer_task->timeout_handle, period, pdMS_TO_TICKS(0)) == pdFALSE)
		goto err;

	if (xTimerStart(timer_task->timeout_handle, pdMS_TO_TICKS(0)) == pdFAIL)
		goto err;
...
static void hr_timer_task_timeout(TimerHandle_t t)
{
	...
	if (xQueueSend(timer_task->queue_handle, &e, pdMS_TO_TICKS(0)) != pdPASS)
					...
					struct hr_timer *t, unsigned int type)
{
	...
	if (xQueueSendToBack(timer_task->queue_handle, &e, pdMS_TO_TICKS(0)) != pdTRUE)
		goto err;
