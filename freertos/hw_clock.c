...
{
	clk->last_cycles = clk->read_counter(clk);
	clk->nsecs = 0;
	clk->frac = 0;
...
__init int hw_clock_register(hw_clock_id_t id, struct hw_clock *clk)
{
	...
	os_log(LOG_INIT, "hw clock id: %d registered\n", id);
...
__exit void hw_clock_unregister(hw_clock_id_t id)
{
	...
	os_log(LOG_INIT, "hw clock id: %d unregistered\n", id);
}
