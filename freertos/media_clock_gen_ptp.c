...
static int mclock_gen_ptp_start(struct mclock_dev *dev, struct mclock_start *start)
{
	...
	*dev->count = 0;
	...
	rc = mclock_register_timer(dev, mclock_gen_ptp_timer_irq, dev->timer_period / HW_AVB_TIMER_PERIOD_NS);
	if (rc < 0) {
...
int mclock_gen_ptp_init(struct mclock_gen_ptp *ptp)
{
	...
	dev->sh_mem = pvPortMalloc(MCLOCK_GEN_MMAP_SIZE);
	if (!dev->sh_mem) {
		rc = -1;
