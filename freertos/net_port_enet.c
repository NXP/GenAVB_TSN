...
AT_NONCACHEABLE_SECTION_ALIGN(static enet_rx_bd_struct_t rx_desc[NUM_ENET_MAC][ENET_RXBD_NUM], ENET_BUFF_ALIGNMENT);
AT_NONCACHEABLE_SECTION_ALIGN(static enet_tx_bd_struct_t tx_desc[NUM_ENET_MAC][ENET_TXBD_NUM], ENET_BUFF_ALIGNMENT);

static enet_frame_info_t txFrameInfoArray[NUM_ENET_MAC][ENET_TXBD_NUM];

SDK_ALIGN(static uint8_t rx_buf[NUM_ENET_MAC][ENET_RXBD_NUM][SDK_SIZEALIGN(NET_DATA_SIZE, ENET_BUFF_ALIGNMENT)], ENET_BUFF_ALIGNMENT);
SDK_ALIGN(static uint8_t tx_buf[NUM_ENET_MAC][ENET_TXBD_NUM][SDK_SIZEALIGN(NET_DATA_SIZE, ENET_BUFF_ALIGNMENT)], ENET_BUFF_ALIGNMENT);

#define get_drv(port) ((struct enet_drv *)port->drv)
static unsigned int init_index = 0;
...
static struct enet_drv enet_drivers[NUM_ENET_MAC] = {
	[0] = {
		.config = {
...
static uint64_t enet_1588_read_counter(struct hw_clock *clock)
{
	struct enet_drv *drv = container_of(clock, struct enet_drv, clock);
...
static int enet_1588_clock_adj_freq(struct hw_clock *clock, int32_t ppb)
{
	struct enet_drv *drv = container_of(clock, struct enet_drv, clock);
	uint32_t pc, inc, cor;
...
static int enet_send_frame(struct net_port *port, uint8_t *data, uint32_t length, struct net_tx_desc *desc, uint32_t queue, bool need_ts)
{
	...
	rc = ENET_SendFrame(drv->base, &drv->handle, data, length, queue, need_ts, (void *)desc->priv);
	if (rc == kStatus_Success)
		return 0;
	else
		return -1;
...
#endif
{
	...
	if (event == kENET_TxEvent) {
		...
		if (frameInfo->isTsAvail == true) {
			...
			ptp_tx_ts(port, ts, (uint32_t)frameInfo->context);
		}
	}
}
...
static void enet_link_up(struct net_port *port)
{
	...
	ENET_SetCallback(&drv->handle, enet_callback, (void *)port);
...
static void enet_exit(struct net_port *port)
{
	struct enet_drv *drv = get_drv(port);
...
int enet_init(struct net_port *port)
{
	...
	if (port->drv_index > (FSL_FEATURE_SOC_ENET_COUNT - 1))
		return -1;

	if (init_index >= NUM_ENET_MAC)
		return -1;
	...
	port->drv = &enet_drivers[init_index++];
