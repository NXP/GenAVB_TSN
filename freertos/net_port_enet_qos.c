...
#if FSL_FEATURE_SOC_ENET_QOS_COUNT && BOARD_USE_ENET_QOS

#include "fsl_enet_qos.h"

#define NUM_ENET_QOS_MAC FSL_FEATURE_SOC_ENET_QOS_COUNT
...
#define ENET_QOS_PTP_CLOCK 125000000
...
static struct enet_qos_drv enet_driver = {
	...
	.ptp_config = {
		...
		.systemTimeClock_Hz = ENET_QOS_PTP_CLOCK,
	},
	.multi_queue = {
...
static void enet_qos_link_up(struct net_port *port)
{
	...
	ENET_QOS_Up(drv->base, &drv->config, port->mac_addr, 1, drv->ptp_clk);
...
program:
	rc_sdk = ENET_QOS_EstProgramGcl(drv->base, &gcl, ENET_QOS_PTP_CLOCK);
...
/*
 * ptp_clk: the system time clock frequency (ssinc * ptp_clk = 1sec)
 * ref_clk: the reference clock frequency (clk_ptp_ref_i)
 */
static uint32_t compute_default_addend(uint32_t ptp_clk, uint32_t ref_clk)
{
	uint64_t tmp;

	/* Default addend: 2^32 / (ref_clk / ptp_clk) */
	tmp = (uint64_t)ptp_clk << 32;
...
static void enet_qos_setup_rx(struct enet_qos_drv *drv, unsigned int max_queue)
{
	...
	for (queue = 0; queue < max_queue; queue++) {
		...
		for (i = 0; i < buf_cfg->rxRingLen; i++) {
			buf_cfg->rxBufferStartAddr[i] = (uint32_t)&rx_buf[buf_offset];
			buf_offset++;
		}
	}
}
...
static void enet_qos_exit(struct net_port *port)
{
	struct enet_qos_drv *drv = get_drv(port);
...
int enet_qos_init(struct net_port *port)
{
	...
	drv->ptp_clk = dev_get_enet_1588_freq(drv->base);
	if (!drv->ptp_clk) {
	...
	drv->ptp_config.defaultAddend = compute_default_addend(ENET_QOS_PTP_CLOCK, drv->ptp_clk);
	...
	os_log(LOG_INIT, "port(%u) enet(%u) ptp clock: %u Hz, core clock: %u Hz num TX queue: %u, num RX queue: %u\n",
	       port->index, port->drv_index, drv->ptp_clk, drv->config.csrClock_Hz, port->num_tx_q, port->num_rx_q);
