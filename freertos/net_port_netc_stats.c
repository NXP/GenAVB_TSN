...
static int netc_ep_stats_get_strings(struct net_port *port, char *buf, unsigned int buf_len)
{
	...
	if (buf_len < (NETC_EP_PORT_STATS_MAX * GENAVB_PORT_STATS_STR_LEN))
		return -1;
	...
	for (i = 0; i < NETC_PORT_RX_DISCARD_STATS_MAX; i++) {
		...
		strncpy(buf, netc_port_rx_discard_stats_str[i], GENAVB_PORT_STATS_STR_LEN);
		buf += GENAVB_PORT_STATS_STR_LEN;
	}
...
#define NETC_SW_STATS_MAX	(NETC_PORT_STATS_MAX + NETC_PORT_TX_DISCARD_STATS_MAX + \
				NETC_SW_DISCARD_STATS_MAX + NETC_QUEUE_STATS_MAX)
...
static int netc_sw_pseudo_stats_get_strings(struct net_port *port, char *buf, unsigned int buf_len)
{
	int i;

	if (buf_len < (NETC_SW_PSEUDO_PORT_STATS_MAX * GENAVB_PORT_STATS_STR_LEN))
		return -1;
	...
	for (i = 0; i < NETC_PORT_TX_DISCARD_STATS_MAX; i++) {
		...
		strncpy(buf, netc_port_tx_discard_stats_str[i], GENAVB_PORT_STATS_STR_LEN);
		buf += GENAVB_PORT_STATS_STR_LEN;
	}
