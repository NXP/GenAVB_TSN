...
static int netc_sw_read_frame(struct net_bridge *sw, uint8_t *data, uint32_t length, uint8_t *port_index, uint64_t *ts, uint8_t *hr)
{
	struct netc_sw_drv *drv = get_drv(sw);
...
static int netc_sw_send_frame(struct net_bridge *sw, unsigned int bridge_port, uint8_t *data, uint32_t length, struct net_tx_desc *desc, uint8_t priority, bool need_ts)
{
...
static int netc_sw_set_tx_idle_slope(struct net_port *port, unsigned int idle_slope, uint32_t queue)
{
	...
	cbs_config.hiCredit = ((uint64_t)port->max_sdu * 8 * idle_slope + port_tx_rate - 1) / port_tx_rate;
	/* hiCredit (credits) = hiCredit * (NETC Clock Frequency / port_tx_rate) * 100 */
	cbs_config.hiCredit = cbs_config.hiCredit * (CLOCK_GetRootClockFreq(kCLOCK_Root_Netc) / (port_tx_rate / 100));
...
static int netc_sw_set_fp(struct net_port *port, unsigned int type, struct genavb_fp_config *config)
{
	...
	case GENAVB_FP_CONFIG_802_1Q:
		map = priority_to_traffic_class_map(8, 0);
		if (map == NULL) {
			rc = -1;
			goto err;
		}
	...
	case GENAVB_FP_CONFIG_802_3:
		base_eth->MAC_MERGE_MMCSR = NETC_ETH_LINK_MAC_MERGE_MMCSR_ME(config->u.cfg_802_3.enable_tx) |
...
static int netc_sw_get_fp(struct net_port *port, unsigned int type, struct genavb_fp_config *config)
{
	...
	case GENAVB_FP_CONFIG_802_3:
		config->u.cfg_802_3.support = (base_eth->MAC_MERGE_MMCSR & NETC_ETH_LINK_MAC_MERGE_MMCSR_LPA_MASK) >> NETC_ETH_LINK_MAC_MERGE_MMCSR_LPA_SHIFT;
...
static int netc_sw_vft_update(struct net_port *port, uint16_t vid, bool dynamic, struct genavb_vlan_port_map *map)
{
	...
	if (netc_sw_vft_find_entry(drv, vid, &entry_id, &config.cfge) < 0) {
		...
		config.cfge.mlo = drv->config.bridgeCfg.dVFCfg.mlo;
...
static int netc_sw_vft_delete(struct net_bridge *sw, uint16_t vid, bool dynamic)
{
	struct netc_sw_drv *drv = get_drv(sw);
...
static int netc_sw_vft_update_mlo(struct net_bridge *sw, uint16_t vid, uint8_t mlo)
{
	struct netc_sw_drv *drv = get_drv(sw);
	netc_tb_vf_config_t config;
	uint32_t entry_id;
...
static int netc_sw_software_maclearn(struct net_bridge *sw, bool enable)
{
	...
	if (drv->config.bridgeCfg.dVFCfg.mlo == mlo)
		goto done;

	drv->config.bridgeCfg.dVFCfg.mlo = mlo;
	...
	token = 0;
	while (!netc_sw_vft_dump(sw, &token, &vid, &dynamic, map)) {
		if (netc_sw_vft_update_mlo(sw, vid, mlo) < 0) {
			rc = -1;
			goto done;
		}
	}
...
__init static void netc_sw_vlan_init(struct netc_sw_drv *drv)
{
	...
	for (i = 0; i < CFG_NUM_NETC_SW_PORTS; i++) {
		...
		map.control = GENAVB_VLAN_ADMIN_CONTROL_FIXED;
		netc_sw_vft_update(port, VLAN_PVID_DEFAULT, false, &map);
	}
}
...
__init static void port_pcp_to_queue_init(struct net_port *port, swt_config_t *config, unsigned int bridge_port_id)
{
	...
	port->max_sdu = QOS_MAX_SDU_BYTES;
...
__init int netc_sw_init(struct net_bridge *sw)
{
	...
	sw->drv_ops.software_maclearn = netc_sw_software_maclearn;

	netc_sw_frer_init(sw);
	...
	for (net_port_idx = 0; net_port_idx < CFG_PORTS; net_port_idx++) {
		...
		if (port->drv_type == NETC_SW_t) {
			switch_port_init(port, &drv->config, port->base);
			drv->ports++;
		}
	}
	...
	if (netc_si_init(sw) < 0) {
		os_log(LOG_ERR, "netc_si_init failed\n");
		goto err_si;
	}

	netc_sw_psfp_init(sw);
