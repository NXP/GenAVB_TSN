...
int socket_set_option(struct net_socket *sock, unsigned int type, unsigned long val)
{
	...
	if (xQueueSendToBack(net_rx_ctx.queue_handle, &e, pdMS_TO_TICKS(0)) != pdTRUE)
		goto err;
...
//	struct net_rx_ctx *net = sock->net_rx;
	...
	if (xQueueSendToBack(net_tx_ctx.queue_handle, &e, pdMS_TO_TICKS(0)) != pdTRUE)
		goto err;
...
static void socket_event_callback(void *arg)
{
	...
	xQueueSendToBack(sock->handle, &e, pdMS_TO_TICKS(0));
}
