...
{
	generic_unbind(ptp_rx_hdlr, sock);
}
...
int ptp_socket_bind(struct net_socket *sock, struct net_address *addr)
{
	...
	return generic_bind(ptp_rx_hdlr, sock, addr->port);
}
...
void ptp_tx_ts(struct net_port *phys_port, u64 ts, u32 priv)
{
	...
	xQueueSendToBack(port->ptp_sock->handle, &e, 0);
}
...
int ptp_rx(struct net_rx_ctx *net, struct net_port *phys_port, struct net_rx_desc *desc, void *hdr)
{
	...
	clock_time_from_hw(phys_port->clock_local, desc->ts64, &local_time);
	desc->ts = local_time;
