...
*
*/
static void gm_indication_handler(struct fgptp_gm_info *info)
{
}
...
*
*/
static void pdelay_indication_handler(struct fgptp_pdelay_info *info)
{
...
static void gptp_status(struct fgptp_ctx *fgptp, int status)
{
	pthread_mutex_lock(&fgptp->status_mutex);

	fgptp->gptp_status = status;

	pthread_cond_signal(&fgptp->gptp_cond);
...
void *gptp_thread_main(void *arg)
{
	...
	struct gptp_ctx *gptp;
	struct fgptp_config *cfg = &fgptp->gptp_linux_cfg.gptp_cfg;
	int epoll_fd;
	...
	memcpy(gptp_linux.nvram_file, fgptp->gptp_linux_cfg.nvram_file, 256);
	...
	*/
	...
	while (1) {
		...
		log_update_time(fgptp->gptp_linux_cfg.clock_log);
