...
static inline uint8_t hsr_logic_port_to_bridge_port(uint8_t *map, uint8_t logical_port)
{
	int i;

	for (i = 0; i < HSR_SWITCH_PORT_MAX; i++)
		if (map[i] == logical_port)
			return i;
...
static int hsr_node_list_add(struct list_head *head, uint8_t *mac, uint16_t vid,
			     uint8_t fwdmask, uint32_t streamid, uint32_t gen_id,
			     uint32_t rec_id, bool isring)
{
...
static int hsr_node_streamid_add(struct hsr_node *node, uint32_t streamid, uint16_t vid)
{
...
static bool hsr_fdb_is_entry_exist(struct hsr_port_config *port_cfg, uint8_t *mac, uint16_t vid, uint8_t fwdmask)
{
	...
	vid = ((vid == VLAN_VID_NONE) ? VLAN_PVID_DEFAULT : vid);
...
static int hsr_stream_identify_add(struct hsr_ctx *hsr, uint8_t *mac, uint16_t vid, uint32_t streamid, uint8_t fwdmask)
{
...
static int hsr_mac_filter_drop(struct hsr_ctx *hsr, uint8_t *mac, uint16_t vid, uint8_t portmask)
{
	...
	if (hsr_stream_identify_add(hsr, mac, vid, streamid, portmask) < 0)
...
static void hsr_maclearn_process(struct hsr_ctx *hsr, struct net_rx_desc *desc)
{
	...
	uint8_t ring_port_mask, fwdmask;
	...
	port = hsr_logic_port_to_bridge_port(hsr->port_cfg.logical_br_port_list, desc->port);
	...
	if (hsr_fdb_is_entry_exist(&hsr->port_cfg, mac, vid, fwdmask))
		return;
	...
	if (hsr_stream_identify_add(hsr, mac, vid, streamid, fwdmask) < 0)
	...
	if (is_ring_port) {
		...
		if (node) {
			hsr_node_streamid_add(node, streamid, vid);
		...
		rc = hsr_node_list_add(&hsr->ring_list, mac, vid, fwdmask, streamid, gen_id, rec_id, 0);
		if (rc < 0)
			goto list_add_err;
	} else {
		...
		if (node) {
			...
			hsr_mac_filter_drop(hsr, mac, vid, ring_port_mask);
			return;
		}
		...
		rc = hsr_node_list_add(&hsr->proxy_list, mac, vid, fwdmask, streamid, gen_id, rec_id, isring);
		if (rc < 0)
			goto list_add_err;
...
static void hsr_supervision_process(struct hsr_ctx *hsr, struct net_rx_desc *desc)
{
	...
	mac = (uint8_t *)desc + desc->l2_offset + 20;
...
{
	uint8_t dmac[] = {0x1, 0x15, 0x4e, 0x00, 0x01, 0x01};
	uint8_t ethertype[] = {0x88, 0xFB};
	uint8_t suppath = 0;
...
static void hsr_nodeforget_timer_handler(void *data)
{
	...
	for (pos = list_first(head); next = list_next(pos), pos != head; pos = next) {
		...
		for (i = 0; i < entry->stream_max; i++) {
			...
			if (hsr_fdb_is_entry_exist(&hsr->port_cfg, entry->mac, vid, entry->fwdmask))
				continue;
