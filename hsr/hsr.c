...
static void hsr_fdb_clean(void)
{
	struct genavb_fdb_port_map port_map[CFG_BR_DEFAULT_NUM_PORTS] = {0};
	genavb_fdb_status_t status = 0;
	uint8_t address[6];
	uint32_t next = 0;
	uint16_t vid;
	bool dynamic;

	while(fdb_dump(&next, address, &vid, &dynamic, port_map, &status) == 0) {
		fdb_delete(address, vid, dynamic);
		next = 0;
	}
}
...
static void hsr_node_operation_mode_set(struct hsr_ctx *hsr, genavb_hsr_mode_t mode)
{
	if (hsr->mode == mode)
		return;
...
static void hsr_stack_start(struct hsr_ctx *hsr)
{
...
static void hsr_stack_stop(struct hsr_ctx *hsr)
{
	hsr->enabled = 0;
	...
	timer_stop(&hsr->lifecheck_timer);
	timer_stop(&hsr->nodeforget_timer);

	net_tx_exit(&hsr->net_tx);
}

static void hsr_ipc_rx(struct ipc_rx const *rx, struct ipc_desc *desc)
{
	genavb_hsr_mode_t mode;
	struct hsr_ctx *hsr;
	uint8_t ring_port_mask;

	hsr = container_of(rx, struct hsr_ctx, ipc_rx_stack);

	switch (desc->type) {
	case IPC_HSR_OPERATION_MODE_SET:
		mode = desc->u.data[0];
		hsr_node_operation_mode_set(hsr, mode);
		break;
	case IPC_HSR_PORT_CONFIG_SET:
		if (hsr->enabled)
			hsr_stack_stop(hsr);

		ring_port_mask = desc->u.data[0];
		if (ring_port_mask) {
			hsr->port_cfg.ring_port_mask = ring_port_mask;
			hsr_stack_start(hsr);
		}
		break;
	default:
...
__init void *hsr_init(struct hsr_config *cfg, unsigned long priv)
{
	...
	if (ipc_rx_init(&hsr->ipc_rx_stack, IPC_HSR_STACK, &hsr_ipc_rx, priv) < 0)
