...
struct genavb_msg_media_stack_bind {
	...
	avb_u16 listener_config_index;				/**< Avdecc configuration index of the listener stream */
...
typedef enum {
	...
	AVDECC_PERSISTENT_PARAM_ENTITY_CONFIGURATION_INDEX	/**< Current configuration index from an ENTITY */
...
struct genavb_msg_media_stack_persistent_param {
	...
	avb_u16 config_index;				/**< Avdecc configuration descriptor index */
};
...
/**
 * \ingroup control
 * Avdecc start message's flags
 */
typedef enum {
	GENAVB_ENTITY_START_FLAGS_IDENTIFY_IDX_VALID       = (1 << 0),	/**< Identify control index for configuration 0 is valid */
	GENAVB_ENTITY_START_FLAGS_IDENTIFY_IDX_CFG_0_VALID = GENAVB_ENTITY_START_FLAGS_IDENTIFY_IDX_VALID,
	GENAVB_ENTITY_START_FLAGS_IDENTIFY_IDX_CFG_1_VALID = (1 << 1),	/**< Identify control index for configuration 1 is valid */
} genavb_entity_start_flags_t;
...
struct genavb_msg_media_stack_start {
	...
	avb_u16 identify_control_index;		/**< Identify control descriptor index for configuration 0 */
	genavb_entity_start_flags_t flags;	/**< Entity start flags ::genavb_entity_start_flags_t */
	avb_u16 identify_control_index_cfg_1;	/**< Identify control descriptor index for configuration 1 */
};

/**
 * \ingroup control
 * Avdecc Identify message to control the Identification Notification process
 */
struct genavb_msg_media_stack_identify {
	avb_u64 entity_id;		/**< Avdecc entity id in network order (an ID of 0 start the first non-controller entity) */
	avb_u8 control_value;		/**< Control value for the Identification */
};
...
union genavb_media_stack_msg {
	...
	struct genavb_msg_media_stack_identify media_stack_identify;		/**< MEDIA STACK Identify message */
};
