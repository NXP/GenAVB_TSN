...
typedef enum {
	...
	GENAVB_CLOCK_SOURCE_TYPE_INPUT_STREAM,		/**< Non redundant stream clock source (slave) */
	GENAVB_CLOCK_SOURCE_TYPE_INPUT_SET		/**< Stream clock source (slave): Can be both for redundant or non-redundant streams. */
} genavb_clock_source_type_t;

#define IS_CLOCK_SOURCE_INPUT_SET(source_type) ((source_type == GENAVB_CLOCK_SOURCE_TYPE_INPUT_STREAM) || (source_type == GENAVB_CLOCK_SOURCE_TYPE_INPUT_SET))
 ...
 */
typedef struct {
	avb_u8 stream_id[8];	/**< Keep as first element: Stream ID of the member of the set used as source */
	avb_u16 id;		/**< set ID */
} genavb_clock_source_set_t;
...
struct genavb_msg_clock_domain_set_source {
	...
	union {
		...
		avb_u16 set_id;  				/**< Only valid for clock sources of type INPUT_SET */
	};
};
...
struct genavb_msg_clock_domain_status {
	...
	union {
		union {
			avb_u8 stream_id[8];				/**< For backward compatibility: Only valid for clock sources of type INPUT_STREAM */
			genavb_clock_source_set_t set;			/**< Only valid for clock sources of type INPUT_STREAM */
		};
		genavb_clock_source_local_id_t local_id;		/**< Only valid for clock sources of type INTERNAL */
	};
