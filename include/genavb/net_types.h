...
#include "config.h"
#include "types.h"
#include "_os/net_types.h"

#define NET_RX_BATCH	8
#define STREAM_TX_BATCH	8 			/* Maximum supported (per talker latency configuration) packet number to be transmitted per wakeup period */
#define NET_TX_BATCH	(STREAM_TX_BATCH * 2)	/* The maximum transmit burst should be two times the maximum tx packet number per period (keep it coherent/less than the TX socket transmit queue size) */
...
#define NET_TX_REUSE			(1 << 8)
#define NET_TX_BUSY			(1 << 9)
#define NET_TX_FLAGS_LOOPBACK_ONLY	(1 << 10)
