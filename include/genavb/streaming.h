...
typedef enum {
	...
	GENAVB_STREAM_FLAGS_REDUNDANT = (1 << 3),	/**<  This stream is a redundant stream */
	GENAVB_STREAM_FLAGS_SET_ID_VALID = (1 << 4),	/**<  This stream is part of a redundant set with a valid set_id */
...
struct genavb_stream_params {
	...
	avb_u16 set_id;				/**< Stream redundant set ID (valid only when GENAVB_STREAM_FLAGS_REDUNDANT is set) */
	avb_u16 set_index;			/**< Stream index inside redundant set (valid only when GENAVB_STREAM_FLAGS_REDUNDANT is set) */
};
 ...
 * - event array cannot hold all the timestamps
 ...
 *
 * For the last two cases, the data will stop at the last packet before the "event", i.e. last packet before the discontinuity, or last packet before the
 * one with the mr bit set. flags will also be set on the *next* batch to signal the event at the start of that new batch.
 *
 * For the first two cases, no flags will be set and the call will be considered successful (data and timestamps are copied into data_iov and event_iov, whichever fills up first).
 *
 ...
 * \param event			array where events matching the data are to be copied (see ::genavb_event).
...
/** Return the AVTP clock associated to a given stream
 * \ingroup stream
 * \return		Returns the ::genavb_clock_id_t associated to the stream.
 * \param handle	Handle of the stream to get the clock for.
 */
genavb_clock_id_t genavb_stream_avtp_clock(const struct genavb_stream_handle *handle);

/* OS specific headers */
#include "_os/streaming.h"
