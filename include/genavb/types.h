 ...
 * * ::AVTP_TIMESTAMP_INVALID will be set to indicate that the ts field in this event is not valid.
 * * ::AVTP_TIMESTAMP_UNCERTAIN will be set to indicate that the ts field in this event is uncertain (packet received a timestamp with the tu bit set).
 ...
 * * ::AVTP_FLUSH can be set as the first event to flush all stream data, up to the end of the data being sent through the
 *     single genavb_stream_send/genavb_stream_send_iov call that will include that event.
 * * ::AVTP_FRAME_END can be set as the first event to signal that the last byte of the data being sent corresponds to the end of
 *     a frame (e.g end of a NALU in a CVF H264 stream). This will have the same effect as ::AVTP_FLUSH also.
*/
struct genavb_event {
