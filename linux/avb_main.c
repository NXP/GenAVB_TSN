...
static int process_section_avdecc(struct _SECTIONENTRY *configtree, struct avdecc_config *avdecc_cfg)
{
	...
	if (strcmp(stringvalue, "1")) {
		avdecc_cfg->enabled = 0;
		goto out;
	}

	avdecc_cfg->enabled = 1;

	if (cfg_get_string(configtree, "AVB_AVDECC", "srp_enabled", "1", stringvalue))
		goto exit;

	if (!strcmp(stringvalue, "0"))
		avdecc_cfg->srp_enabled = 0;
	else
		avdecc_cfg->srp_enabled = 1;
	...
	if (cfg_get_uint(configtree, "AVB_AVDECC", "max_entities_discovery", CFG_ADP_DEFAULT_NUM_ENTITIES_DISCOVERY, CFG_ADP_MIN_NUM_ENTITIES_DISCOVERY, CFG_ADP_MAX_NUM_ENTITIES_DISCOVERY, &avdecc_cfg->max_entities_discovery))
		goto exit;
