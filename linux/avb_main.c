...
#define AVB_DEFAULT_NET_MODE	"avb"
...
static int process_section_avdecc(struct _SECTIONENTRY *configtree, struct avdecc_config *avdecc_cfg)
{
	...
	for (entity_index = 0; entity_index < CFG_AVDECC_NUM_ENTITIES; entity_index++) {
		...
		if (rc == 0 && strcmp(stringvalue, "none")) {
			...
			/* Init all AEM pointers (one per AVDECC configuration) to NULL. */
			for (i = 0; i < CFG_MAX_AEM_CONFIGS; i++)
				entity_cfg->aem[i] = NULL;

			aem = aem_entity_load_from_file(stringvalue, cfg_sizes, &cfg_count);
			if (!aem || cfg_count > CFG_MAX_AEM_CONFIGS)
				goto err;

			for (i = 0; i < cfg_count; i++) {
				entity_cfg->aem[i] = aem;
				aem = (void *)((char *)aem + cfg_sizes[i]);
			}

			entity_cfg->num_aems = cfg_count;

			printf("Processing section %s: loading %u configuration(s) from entity file %s\n", section_name, entity_cfg->num_aems, stringvalue);
			...
			if (cfg_get_string(configtree, section_name, "serial_number", "none", stringvalue))
				goto err;

			memset(entity_cfg->serial_number, 0, AEM_STR_LEN_MAX);

			stringlen = strlen(stringvalue);
			if (stringlen > AEM_STR_LEN_MAX) {
				printf("AVB cfg file: Warning, serial_number is larger than a %u bytes UTF8 string. Truncating.\n", AEM_STR_LEN_MAX);
				stringlen = AEM_STR_LEN_MAX;
			}

			if (strcmp(stringvalue, "none"))
				memcpy(entity_cfg->serial_number, stringvalue, stringlen);

			if (cfg_get_uint(configtree, section_name, "max_listener_streams", CFG_ACMP_DEFAULT_NUM_LISTENER_STREAMS,
					 CFG_ACMP_MIN_NUM_LISTENER_STREAMS, CFG_ACMP_MAX_NUM_LISTENER_STREAMS, &entity_cfg->max_listener_streams))
				goto err;

			if (cfg_get_uint(configtree, section_name, "max_talker_streams", CFG_ACMP_DEFAULT_NUM_TALKER_STREAMS,
					 CFG_ACMP_MIN_NUM_TALKER_STREAMS, CFG_ACMP_MAX_NUM_TALKER_STREAMS, &entity_cfg->max_talker_streams))
				goto err;

			if (cfg_get_uint(configtree, section_name, "max_inflights", CFG_AVDECC_DEFAULT_NUM_INFLIGHTS,
					 CFG_AVDECC_MIN_NUM_INFLIGHTS, CFG_AVDECC_MAX_NUM_INFLIGHTS, &entity_cfg->max_inflights))
				goto err;

			if (cfg_get_uint(configtree, section_name, "max_unsolicited_registratons", CFG_AECP_DEFAULT_NUM_UNSOLICITED,
					 CFG_AECP_MIN_NUM_UNSOLICITED, CFG_AECP_MAX_NUM_UNSOLICITED, &entity_cfg->max_unsolicited_registrations))
				goto err;

			if (cfg_get_uint(configtree, section_name, "max_ptlv_entries", CFG_AEM_DEFAULT_NUM_PTLV_ENTRIES,
					 CFG_AEM_MIN_NUM_PTLV_ENTRIES, CFG_AEM_MAX_NUM_PTLV_ENTRIES, &entity_cfg->max_ptlv_entries))
				goto exit;

			if (cfg_get_uint(configtree, section_name, "channel_waitmask", 0x0, 0x0, 0xf, &entity_cfg->channel_waitmask))
				goto err;

			if ((entity_cfg->channel_waitmask & AVDECC_WAITMASK_MEDIA_STACK_START) &&
				!(entity_cfg->channel_waitmask & AVDECC_WAITMASK_MEDIA_STACK)) {

				entity_cfg->channel_waitmask |= AVDECC_WAITMASK_MEDIA_STACK;
				printf("Config file parser: in section [%s], key 'channel_waitmask', value is forced to '%u'\n",
					section_name, entity_cfg->channel_waitmask);
			}
...
err:
	...
	for (i = 0; i < avdecc_cfg->num_entities; i++) {
		if (avdecc_cfg->entity_cfg[i].aem[0])
			free(avdecc_cfg->entity_cfg[i].aem[0]);
	}
...
static int process_system_config(struct os_net_config *net_config, const char *filename)
{
	struct _SECTIONENTRY *configtree;
	...
	if (cfg_get_net_mode(configtree, "avdecc_net_mode", AVB_DEFAULT_NET_MODE, &net_config->avdecc_net_mode, &net_config->enabled_modes_flag))
		goto err_parse;

	if (cfg_get_net_mode(configtree, "maap_net_mode", AVB_DEFAULT_NET_MODE, &net_config->maap_net_mode, &net_config->enabled_modes_flag))
		goto err_parse;

	if (cfg_get_net_mode(configtree, "avtp_net_mode", AVB_DEFAULT_NET_MODE, &net_config->avtp_net_mode, &net_config->enabled_modes_flag))
		goto err_parse;
...
{
#if defined(CONFIG_GENAVB_TSN_AVTP) || defined(CONFIG_GENAVB_TSN_MAAP) || defined(CONFIG_GENAVB_TSN_AVDECC)
	int rc;
#endif
...
#endif
#if defined(CONFIG_GENAVB_TSN_AVTP) || defined(CONFIG_GENAVB_TSN_MAAP)
err_thread_init:
#endif
#if defined(CONFIG_GENAVB_TSN_MAAP) && defined(CONFIG_GENAVB_TSN_AVDECC)
...
int main(int argc, char *argv[])
{
	...
	while ((option = getopt(argc, argv,"vhH:f:p:s:")) != -1) {
		switch (option) {
		case 'v':
			print_version();
			goto exit;
	...
	if (process_system_config(&net_config, SYSTEM_CONF_FILENAME) < 0) /* System cfg file processing failed, exit */
		goto err_config;
	...
	/* Hybrid mode with external clock synchronization means there is no gptp endpoint running:
	 * Use the gptp bridge stack to get gptp informations in avdecc stack.
	 */
	avb->avdecc_cfg.use_gptp_bridge_stack = (hybrid_mode == HYBRID_MODE_EXTERNAL_CLOCK_SYNC) ? true : false;
...
err_endpoint_init:
	...
	if (avb->avdecc_cfg.enabled) {
		for (i = 0; i< avb->avdecc_cfg.num_entities; i++) {
			if (avb->avdecc_cfg.entity_cfg[i].aem[0])
				free(avb->avdecc_cfg.entity_cfg[i].aem[0]);
		}
	}
