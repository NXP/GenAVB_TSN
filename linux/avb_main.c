...
static int process_sr_class_config(char* stringvalue, struct _SECTIONENTRY *configtree)
{
	int i, n;
	...
	for (i = 0; i < CFG_MAX_LOGICAL_PORTS; i++)
		if (net_port_sr_config(i, sr_class) < 0)
			goto err;

	if (sr_class_config(sr_class) < 0) {
...
static int process_section_general(struct _SECTIONENTRY *configtree, struct avb_ctx *avb)
{
	...
	/* enable sr_class */
	if ((rc = process_sr_class_config(stringvalue, configtree)) < 0)
		goto exit;
