...
static void os_clock_config_init(struct os_clock_config *config)
{
	...
	for (i = 0; i < CFG_MAX_ENDPOINTS; i++) {
		...
		/* If one of the target clocks is a physical PHC, force local to be a SW clock
		 * in order to have a more stable local clock when target clock adjustements affect
		 * hardware clock
		 */
...
int os_clock_convert(os_clock_id_t clk_id_src, u64 ns_src, os_clock_id_t clk_id_dst, u64 *ns_dst)
{
	...
	if (OS_CLOCK_IS_VIRTUAL_PHC(c_src) || OS_CLOCK_IS_VIRTUAL_PHC(c_dst)) {
		...
		convert_ts.src_ts[0].sec = ns_src / NSECS_PER_SEC;
		convert_ts.src_ts[0].nsec = ns_src % NSECS_PER_SEC;
		convert_ts.n_ts = 1;
		...
		*ns_dst = convert_ts.dst_ts[0].nsec + convert_ts.dst_ts[0].sec * NSECS_PER_SEC;
	} else {
