 ...
 * Copyright 2016-2017, 2019-2021, 2023, 2025 NXP
 *
...
/* Clock is a virtual PHC (Valid only for CLOCK_TYPE_PHC) */
#define OS_CLOCK_FLAGS_PHC_VIRTUAL (1 << 2)
/* Clock supports remapping to another clock (both as source and destination of the mapping) */
#define OS_CLOCK_FLAGS_SUPPORTS_REMAP (1 << 3)
...
struct os_clock {
	...
	int phc_index;	            /* used for PHC based clocks. */
	bool enabled;
	unsigned int type;
	unsigned int flags; /* used for hardware clock adjustment capabilities and phc type information (virtual or physical). */
...
bool os_clock_is_virtual_phc(os_clock_id_t id);
int os_clock_to_phc_index(os_clock_id_t id);
