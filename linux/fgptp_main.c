...
static int process_section_port_params(struct _SECTIONENTRY *configtree, struct fgptp_config *cfg)
{
	char stringvalue[CFG_STRING_MAX_LEN] = "";
	int rc = 0;
	int i;
...
static int process_section_timing(struct _SECTIONENTRY *configtree, struct fgptp_config *cfg)
{
	int rc = 0;

	/* initial pdelay request transmit interval */
	if (cfg_get_schar(configtree, "FGPTP_TIMING", "initialLogPdelayReqInterval", CFG_GPTP_DFLT_LOG_PDELAY_REQ_INTERVAL, CFG_GPTP_MIN_LOG_PDELAY_REQ_INTERVAL, CFG_GPTP_MAX_LOG_PDELAY_REQ_INTERVAL, &cfg->initialLogPdelayReqInterval)) {
		rc = -1;
		goto exit;
	}

	/*  initial sync transmit interval  */
	if (cfg_get_schar(configtree, "FGPTP_TIMING", "initialLogSyncInterval", CFG_GPTP_DFLT_LOG_SYNC_INTERVAL, CFG_GPTP_MIN_LOG_SYNC_INTERVAL, CFG_GPTP_MAX_LOG_SYNC_INTERVAL, &cfg->initialLogSyncInterval)) {
		rc = -1;
		goto exit;
	}

	/*  initial announce transmit interval	*/
	if (cfg_get_schar(configtree, "FGPTP_TIMING", "initialLogAnnounceInterval", CFG_GPTP_DFLT_LOG_ANNOUNCE_INTERVAL, CFG_GPTP_MIN_LOG_ANNOUNCE_INTERVAL, CFG_GPTP_MAX_LOG_ANNOUNCE_INTERVAL, &cfg->initialLogAnnounceInterval)) {
		rc = -1;
		goto exit;
	}
	...
	/* initial pdelay request transmit interval */
	if (cfg_get_schar(configtree, "FGPTP_TIMING", "operLogPdelayReqInterval", CFG_GPTP_DFLT_LOG_PDELAY_REQ_INTERVAL, CFG_GPTP_MIN_LOG_PDELAY_REQ_INTERVAL, CFG_GPTP_MAX_LOG_PDELAY_REQ_INTERVAL, &cfg->operLogPdelayReqInterval)) {
		rc = -1;
		goto exit;
	}

	/*  initial sync transmit interval  */
	if (cfg_get_schar(configtree, "FGPTP_TIMING", "operLogSyncInterval", CFG_GPTP_DFLT_LOG_SYNC_INTERVAL, CFG_GPTP_MIN_LOG_SYNC_INTERVAL, CFG_GPTP_MAX_LOG_SYNC_INTERVAL, &cfg->operLogSyncInterval)) {
		rc = -1;
		goto exit;
	}
exit:
...
static int process_config(struct gptp_linux_config *linux_cfg, struct _SECTIONENTRY *configtree[])
{
	...
	if (process_section_port_params(configtree[0], cfg))
		goto exit;

	if (process_section_timing(configtree[0], cfg))
		goto exit;
