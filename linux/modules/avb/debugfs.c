...
static int mclock_rec_pll_show(struct seq_file *s, void *data)
{
	...
	struct mclock_rec_pll_stats *stats_linux = &rec->stats;
	...
	seq_printf(s, "start				= %u\n", stats->start);
	seq_printf(s, "stop				= %u\n", stats->stop);
	seq_printf(s, "gPTP start error			= %u\n", stats->err_gptp_start);
	seq_printf(s, "gPTP gettime error		= %u\n", stats->err_gptp_gettime);
	seq_printf(s, "gPTP reload error		= %u\n", stats->err_gptp_reload);
	...
	seq_printf(s, "port down error (start)		= %u\n", stats_linux->err_start_port_down);
	seq_printf(s, "port down error (stop)		= %u\n", stats_linux->err_stop_port_down);
				      ...
				      size_t count, loff_t *ppos)
{
	...
	return __mclock_rec_pll_unsigned_param_read(file, buf, count, ppos, &rec->c.pi.ki);
}
				      ...
				      size_t count, loff_t *ppos)
{
	...
	return __mclock_rec_pll_unsigned_param_read(file, buf, count, ppos, &rec->c.pi.kp);
}
					     ...
					     size_t count, loff_t *ppos)
{
	...
	return __mclock_rec_pll_signed_param_write(file, buf, count, ppos, &rec->c.req_pll_shift_ns);
}
					    ...
					    size_t count, loff_t *ppos)
{
	...
	return __mclock_rec_pll_signed_param_read(file, buf, count, ppos, &rec->c.total_pll_shift_ns);
}
