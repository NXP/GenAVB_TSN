...
#define FTM_AVB_DEVICE_COMPATIBLE	"fsl,avb-ftm"

static const struct of_device_id ftm_dt_ids[] = {
	{ .compatible = FTM_AVB_DEVICE_COMPATIBLE, },
	{ /* sentinel */ }
};

MODULE_DEVICE_TABLE(of, ftm_dt_ids);

bool is_ftm_hw_timer_available(void)
{
	struct device_node *node;
	bool ret = false;

	node = of_find_compatible_node(NULL, NULL, FTM_AVB_DEVICE_COMPATIBLE);
	if (!node)
		goto exit;
