...
static int gpt_rec_timer_irq(struct mclock_dev *dev, void *data, unsigned int ticks)
{
	...
	unsigned int mclock_rec_events;
	unsigned long flags;
	u32 icr_val, cnt_val;

	raw_spin_lock_irqsave(&dev->eth->lock, flags);

	mclock_rec_events = (dev->eth->flags & PORT_FLAGS_ENABLED) ? 0 : MCLOCK_REC_PLL_FLAGS_PORT_DOWN;

	/* get status  */
	icr_val = readl_relaxed(drv->baseaddr + GPT_SR);
	if (icr_val & GPT_IF(drv->cap_channel)) {

		mclock_rec_events |= MCLOCK_REC_PLL_FLAGS_GPTP_EVENT;

		/* get value */
		cnt_val = readl_relaxed(drv->baseaddr + GPT_ICR(drv->cap_channel));

		mclock_rec_pll_common_timer_irq(&rec->c, mclock_rec_events, (u32)(cnt_val - rec->c.audio_pll_cnt_last), cnt_val, ticks);
		...
		mclock_rec_pll_common_timer_irq(&rec->c, mclock_rec_events, 0, 0, ticks);
	}
...
static int gpt_sw_rec_timer_irq(struct mclock_dev *dev, void *data, unsigned int ticks)
{
	...
	if (rc >= 0)
		mclock_rec_pll_common_sw_sampling_irq(&rec->c, cnt_val, ptp_now, ticks);
...
static int gpt_probe(struct platform_device *pdev)
{
	...
	if (clock_dev) {
		...
		rec->c.pll_deferred_adjust = of_property_read_bool(pdev->dev.of_node, "fsl,pll-scu-controlled");
		if (rec->c.pll_deferred_adjust)
		...
		/* Devices with deferred PLL adjustmenets, do not have (nor need) the parent clock rate to set/get rate. */
		if (imx_pll_init(pll, &pdev->dev, !rec->c.pll_deferred_adjust) < 0) {
			rc = -EINVAL;
			pr_err("%s: cannot init the audio PLL\n", __func__);
			goto err_clk;
		}

		rec->c.pll_ref_freq = clk_get_rate(drv->clk_gpt) / drv->prescale;
		rec->c.pll_timer_clk_div = clk_get_rate(pll->clk_audio_pll) / rec->c.pll_ref_freq;
	...
	if (clock_dev) {
		...
		pr_info("%s: %s rec device(%p), domain: %d, sampling period: %d us, clk input: %ld Hz, pll frequency %lu pll parent (osc) frequency %lu pll gpt clock divider %u\n",
				__func__, rec->c.is_hw_recovery_mode ? "HW" : "SW", clock_dev, clock_dev->domain, rec->c.gptp_period.i / 1000, clk_get_rate(drv->clk_gpt),
				pll->current_rate, pll->parent_rate, rec->c.pll_timer_clk_div);
	}
