...
/**
 * mclock_rec_pll() - start and check the ENET sampling signal
 * @rec: pointer to the PLL media clock recovery context
 * @meas: audio PLL meas
 * Return : none
 */
int mclock_rec_pll_timer_irq(struct mclock_rec_pll *rec, int fec_event, unsigned int meas, unsigned int ticks)
{
	...
	err = (rec->pll_clk_target.i - pll_clk_last) - (rec->pll_clk_meas - pll_clk_meas_last);
	...
	switch (rec->state) {
	case START:
		if(rec->meas++ >= MCLOCK_REC_PLL_NB_MEAS) {
			rec->state = ADJUST;
