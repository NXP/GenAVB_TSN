...
//	pr_info("%s: %p %p\n", __func__, data, buf);
	if (buf) {
		...
		desc->common.pool_type = POOL_TYPE_AVB;

		desc->dma_addr = pool_dma_virt_to_dma(eth->buf_pool, (void *)desc + desc->common.offset, eth->index);
	}
...
static inline struct logical_port *eth_tx_get_logical_port(struct eth_avb *eth, struct net_tx_desc *desc)
{
	/* Either rely on port information in descriptor or DSA tag parsing if any
	 * Otherwise, default to first mapped port in the physical interface.
	 */
	return eth->logical_port[CFG_DEFAULT_LOGICAL_PORT_MAP_INDEX];
}
...
static int eth_avb_enqueue_tx_ts(void *data, struct avb_desc *desc)
{
	...
	struct logical_port *port = eth_tx_get_logical_port(eth, tx_desc);
...
static const struct avb_ops avb_ops = {
	.open = &eth_avb_open,
	.close = &eth_avb_close,
...
static void queue_free_tx_skb(void *data, unsigned long entry)
{
...
{
	struct avb_drv *avb = container_of(eth, struct avb_drv, eth[0]);
	int i, map_index, rc;
...
//	pr_info("%s: %p\n", __func__, eth);
	...
	for (i = 0; i < CFG_PORTS; i++) {
		queue_init(&eth[i].rx_queue, &pool_dma_free_virt, CFG_RX_EXTRA_ENTRIES);

		queue_init(&eth[i].tx_cleanup_queue, &pool_dma_free_virt, CFG_TX_CLEANUP_EXTRA_ENTRIES);
		...
		queue_init(&eth[i].tx_queue, &queue_free_tx_skb, 0);

		raw_spin_lock_init(&eth[i].lock);

		eth[i].dev = fec_enet_avb_get_device(physical_port_name(i));

		/* Get number of logical ports mapped to this physical interface and init the logical port mapping */
		rc = physical_port_get_num_logical_ports(i);
		if (rc < 0) {
			pr_err("%s: invalid logical port mapping for physical interface %u\n", __func__, i);
			goto err_logical_port;
		}

		eth[i].num_logical_ports = rc;

		for (map_index = 0; map_index < eth[i].num_logical_ports; map_index++) {
			/* Make sure mapped logical ports are always valid and enabled to have quick/unprotected access in critical path. */
			eth[i].logical_port[map_index] = physical_port_get_logical_port_by_map_index(avb, i, map_index);
			if (!eth[i].logical_port[map_index]) {
				pr_err("%s: invalid logical port mapping for physical interface %u (map index %u)\n",
					__func__, i, map_index);
				goto err_logical_port;
			}
		}
