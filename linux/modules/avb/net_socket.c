...
int socket_set_option(struct net_socket *sock, struct net_set_option *option)
{
	...
	raw_write_lock_irqsave(&ptype_lock, flags);
	...
	raw_write_unlock_irqrestore(&ptype_lock, flags);
...
static int socket_tx_connected(struct net_socket *sock, unsigned long *buf, unsigned int n)
{
	...
	for (i = 0; i < n; i++) {
		...
		if (sock->flags & SOCKET_FLAGS_VLAN) {
			struct vlan_hdr *vlan = (struct vlan_hdr *)(ethhdr + 1);

			vlan->label = sock->vlan_label;
		}

		desc->dma_addr = pool_dma_virt_to_dma(&avb->buf_pool, (void *)desc + desc->common.offset);
		dma_sync_single_for_device(avb->buf_pool.dma_device, desc->dma_addr, desc->common.len, DMA_TO_DEVICE);
