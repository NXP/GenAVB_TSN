...
static int tpm_rec_timer_irq(struct mclock_dev *dev, void *data, unsigned int ticks)
{
	...
	if (tpm_status & TPM_STATUS_CHnF(drv->cap_channel)) {
		...
		mclock_rec_pll_common_timer_irq(&rec->c, mclock_rec_events, ((u32)(cnt_val - drv->cnt_last) & drv->counter_mask), cnt_val, ticks);
...
static int tpm_sw_rec_timer_irq(struct mclock_dev *dev, void *data, unsigned int ticks)
{
	struct mclock_rec_pll *rec = mclock_dev_to_rec(dev);
	struct tpm_drv *drv = container_of(rec, struct tpm_drv, rec);
...
static int tpm_probe(struct platform_device *pdev)
{
	...
	if (clock_dev) {
		...
		if (imx_pll_init(pll, &pdev->dev, true) < 0) {
			rc = -EINVAL;
			pr_err("%s: cannot init the audio PLL\n", __func__);
			goto err_clk;
		}

		rec->c.pll_ref_freq = clk_get_rate(drv->clk_tpm) / drv->prescale;
	...
	if (clock_dev) {
		...
		pr_info("%s: %s rec device(%p), domain: %d, sampling period: %d us, clk input: %ld Hz, pll frequency %lu pll parent (osc) frequency %lu pll tpm divider %u\n",
				__func__, rec->c.is_hw_recovery_mode ? "HW" : "SW", clock_dev, clock_dev->domain, rec->c.gptp_period.i / 1000, clk_get_rate(drv->clk_tpm),
...
#else
static void tpm_remove(struct platform_device *pdev)
#endif
{
