...
{
	return rx->net_ops.__net_rx_multi(rx, desc, n);
}
...
int net_rx_init(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc *desc), unsigned long epoll_fd)
{
...
int net_rx_init_multi(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc **desc, unsigned int n), unsigned int packets, unsigned int time, unsigned long epoll_fd)
{
	if (net_ops_init(addr, &rx->net_ops, true))
		return -1;
...
{
	unsigned int pool_type = desc->pool_type;
	...
	return net_mem_handler[pool_type].net_tx_free(desc);
}
...
void net_rx_free(struct net_rx_desc *desc)
{
	...
	if (desc->pool_type >= POOL_TYPE_MAX || !net_mem_handler[pool_type].net_rx_free) {
		os_log(LOG_ERR, "desc(%p): Unsupported pool_type(%u)\n", desc, pool_type);
		return;
	}

	return net_mem_handler[pool_type].net_rx_free(desc);
}
...
int net_set_hw_ts(unsigned int port_id, bool enable)
{
	...
	if (enable) {
		config.rx_filter = logical_port_is_bridge(port_id) ? HWTSTAMP_FILTER_PTP_V2_L2_EVENT : HWTSTAMP_FILTER_ALL;
	} else {
