...
int net_avb_tx_alloc_multi(struct net_tx_desc **desc, unsigned int n, unsigned int size)
{
	...
	for (i = 0; i < rc; i++) {
		...
		desc[i]->pool_type = POOL_TYPE_AVB;
	}
...
static int __net_avb_rx_init(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc *desc),
		void (*func_multi)(struct net_rx *rx, struct net_rx_desc **desc, unsigned int n), unsigned int packets, unsigned int latency, int epoll_fd)
{
	...
	if (addr) {
		...
		if (addr->ptype != PTYPE_PTP) {
			if (addr->ptype == PTYPE_AVTP) {
				rx->clock_domain = logical_port_to_avtp_clock(rx->port_id);
			} else {
				rx->clock_domain = logical_port_to_gptp_clock(rx->port_id, PTP_DOMAIN_0);
			}
			/* AVB sockets do not provide hardware to virtual timestamp conversion.
			 * So, timestamps are always in hardware clock domain.
			 * FIXME to support virtual clocks (in non-PTP sockets), we need to either
			 * implement timestamp conversion in kernel module or let the caller ask for the
			 * proper clock domain from upper layers depending on usecase/configuration
			*/
			if (net_address_needs_rx_timestamping(addr) && os_clock_is_virtual_phc(rx->clock_domain)) {
				os_log(LOG_ERR, "net_rx(%p) ptype (0x%x): bind to target gPTP clock not supported with virtual PHCs\n",
					rx, addr->ptype);
...
int net_avb_rx_init(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc *desc), unsigned long epoll_fd)
{
	return __net_avb_rx_init(rx, addr, func, NULL, 1, 0, epoll_fd);
}

int net_avb_rx_init_multi(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc **desc, unsigned int n), unsigned int packets, unsigned int time, unsigned long epoll_fd)
{
...
static int __net_avb_rx_multi(struct net_rx *rx, struct net_rx_desc **desc, unsigned int n)
{
	unsigned long addr[n];
	int len;

	len = read(rx->fd, addr, n * sizeof(unsigned long));
	if (len < 0) {
		os_log(LOG_ERR, "read failed: %s\n", strerror(errno));
		len = 0;
		goto err;
	}
	...
	for (int i = 0; i < len; i++) {
		...
		if (logical_port_is_endpoint(rx->port_id))
			desc[i]->ts64 = hwts_to_u64(rx->clock_domain, desc[i]->ts);

		clock_time_from_hw(rx->clock_domain, desc[i]->ts64, &desc[i]->ts64);
	}
...
void net_avb_rx(struct net_rx *rx)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (logical_port_is_endpoint(rx->port_id))
			desc->ts64 = hwts_to_u64(rx->clock_domain, desc->ts);

		clock_time_from_hw(rx->clock_domain, desc->ts64, &desc->ts64);
...
static int __net_avb_tx_init(struct net_tx *tx, struct net_address *addr, bool needs_tx_timestamping)
{
	os_log(LOG_DEBUG, "enter\n");
	...
	if (addr) {
		...
		if (addr->ptype != PTYPE_PTP) {
			if (addr->ptype == PTYPE_AVTP)
				tx->clock_domain = logical_port_to_avtp_clock(tx->port_id);
			else
				tx->clock_domain = logical_port_to_gptp_clock(tx->port_id, PTP_DOMAIN_0);

			if (needs_tx_timestamping && os_clock_is_virtual_phc(tx->clock_domain)) {
				os_log(LOG_ERR, "net_tx(%p) bind to target gPTP clock not supported with virtual PHCs\n", tx);
				goto err_clock_domain;
			}
		} else {
...
{
	return __net_avb_tx_init(tx, addr, false);
}
...
int net_avb_tx_ts_init(struct net_tx *tx, struct net_address *addr, void (*func)(struct net_tx *tx, uint64_t ts, unsigned int ts_info), unsigned long priv)
{
	int epoll_fd = priv;
	...
	if (__net_avb_tx_init(tx, addr, true) < 0)
		goto err_tx_init;
...
static inline int __net_avb_port_status(struct net_tx *tx, unsigned int port_id, bool *up, bool *point_to_point, uint64_t *rate)
{
...
int net_avb_port_status(struct net_tx *tx, unsigned int port_id, bool *up, bool *point_to_point, uint64_t *rate)
{
...
static const struct net_tx_ops_cb net_tx_avb_ops = {
...
static const struct net_mem_ops_cb net_avb_mem_ops = {
...
int net_avb_socket_init(void *net_ops, bool is_rx)
{
	/* We copy the entire struct rather than just point to it, to reduce the number of
	 * indirections in performance-sensitive code.
	 */
	if (is_rx)
		memcpy(net_ops, &net_rx_avb_ops, sizeof(struct net_rx_ops_cb));
	else
		memcpy(net_ops, &net_tx_avb_ops, sizeof(struct net_tx_ops_cb));

	return 0;
}

int net_avb_init(struct net_mem_ops_cb *net_mem_ops)
{
	memcpy(net_mem_ops, &net_avb_mem_ops, sizeof(struct net_mem_ops_cb));
