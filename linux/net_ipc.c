...
#define IPC_DATA_OFFSET        (offsetof(struct ipc_desc, u.data))
...
static int net_ipc_rx_init(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc *desc), unsigned long epoll_fd)
{
	ipc_id_t ipc_id;
...
static int net_ipc_rx_init_multi(struct net_rx *rx, struct net_address *addr,
					void (*func)(struct net_rx *rx, struct net_rx_desc **desc, unsigned int n),
...
static int __net_ipc_rx_multi(struct net_rx *rx, struct net_rx_desc **desc, unsigned int n)
{
	return 0;
}
...
static int net_ipc_tx_ts_init(struct net_tx *tx, struct net_address *addr, void (*func)(struct net_tx *tx, uint64_t ts, unsigned int ts_info), unsigned long priv)
{
	return -1;
}
...
static int net_ipc_port_status(struct net_tx *tx, unsigned int port_id, bool *up, bool *point_to_point, uint64_t *rate)
{
	/* The MAC service layer in Hybrid mode uses the standard net mode to fetch the FEC port status.
	 * IPC net mode is only for SRP protocol.
	 */
	return -1;
}

static unsigned int net_ipc_port_mtu_size_get(unsigned int port_id)
{
	return (IPC_BUF_SIZE - IPC_DATA_OFFSET);
}

static const struct net_rx_ops_cb net_rx_ipc_ops = {
		.net_rx_init = &net_ipc_rx_init,
		.net_rx_init_multi = &net_ipc_rx_init_multi,
		.net_rx_exit = &net_ipc_rx_exit,
		.net_rx = &net_ipc_rx,
		.__net_rx_multi = &__net_ipc_rx_multi,
...
static const struct net_tx_ops_cb net_tx_ipc_ops = {
		...
		.net_port_mtu_get = &net_ipc_port_mtu_size_get,
};

static const struct net_mem_ops_cb net_ipc_mem_ops = {
