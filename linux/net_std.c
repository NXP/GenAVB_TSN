...
static bool net_address_is_supported(struct net_address *addr)
{
	...
	case PTYPE_L2:
		...
		break;
	case PTYPE_AVTP:
		if (is_avtp_control(addr->u.avtp.subtype))
			rc = true;
		else
			rc = false;
		break;
...
static os_clock_id_t net_address_get_clock_domain(struct net_address *addr)
{
	/* The socket timestamps domain (packet timestamps provided to upper layers) should be:
	 * - gPTP packets:    gPTP local clock domain
	 * - Other protocols: gPTP target clock domain
	 */
	if (addr->ptype != PTYPE_PTP)
		return logical_port_to_gptp_clock(addr->port, PTP_DOMAIN_0);
	else
		return logical_port_to_local_clock(addr->port);
}

#if defined(HAVE_VIRTUAL_PHC_SUPPORT)
static int __net_std_set_sotimestamping(struct net_address *addr, int fd, int flags)
{
	struct so_timestamping sock_timestamping;
	os_clock_id_t clock_domain;
	int phc_index = -1;

	clock_domain = net_address_get_clock_domain(addr);

	phc_index = os_clock_to_phc_index(clock_domain);
	if (phc_index < 0) {
		os_log(LOG_ERR, "logical_port(%u): error getting phc index for clock domain(%d)\n",
			addr->port, clock_domain);
		goto err;
	}

	/* SOF_TIMESTAMPING_BIND_PHC should only be used for virtual PHCs. */
	if (os_clock_is_virtual_phc(clock_domain))
		flags |= SOF_TIMESTAMPING_BIND_PHC;

	sock_timestamping.flags = flags;
	sock_timestamping.bind_phc = phc_index;

	if (setsockopt(fd, SOL_SOCKET, SO_TIMESTAMPING, &sock_timestamping, sizeof(sock_timestamping)) < 0) {
		os_log(LOG_ERR, "setsockopt(SO_TIMESTAMPING, {flags: 0x%x, bind_phc %d}) failed: %s\n",
			flags, phc_index, strerror(errno));
		goto err;
	}
...
#else
#pragma message "Building with kernel headers without support to SOF_TIMESTAMPING_BIND_PHC: no Linux virtual PHC support"
static int __net_std_set_sotimestamping(struct net_address *addr, int fd, int flags)
{
	os_clock_id_t clock_domain;
	...
	clock_domain = net_address_get_clock_domain(addr);
	if (os_clock_is_virtual_phc(clock_domain)) {
		os_log(LOG_ERR, "can not bind socket to virtual clock: build does not support it\n");
		goto err;
	}
...
static int net_std_set_socket_ts(struct net_address *addr, int fd, bool tx)
{
	int flags = 0;

	if (!logical_port_valid(addr->port)) {
	...
	if (__net_std_set_sotimestamping(addr, fd, flags) < 0)
		goto err_sockopt;
...
static int net_std_rx_bind(struct net_rx *rx, struct net_address *addr)
{
	...
	if (setsockopt(rx->fd, SOL_SOCKET, SO_BINDTODEVICE, logical_port_name(addr->port), strlen(logical_port_name(addr->port)))) {
		os_log(LOG_ERR, "SO_BINDTODEVICE failed: %s\n", logical_port_name(addr->port));
		goto err_bind;
	}
	...
	rx->clock_domain = net_address_get_clock_domain(addr);
...
static int __net_std_rx_init(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc *desc),
		void (*func_multi)(struct net_rx *rx, struct net_rx_desc **desc, unsigned int n), unsigned int packets, unsigned int latency, int epoll_fd)
{
	...
	if (net_address_needs_rx_timestamping(addr)) {
		if (net_std_set_socket_ts(addr, rx->fd, false) < 0) {
...
int net_std_rx_init(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc *desc), unsigned long epoll_fd)
{
	return __net_std_rx_init(rx, addr, func, NULL, 1, 0, epoll_fd);
}

int net_std_rx_init_multi(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc **desc, unsigned int n), unsigned int packets, unsigned int time, unsigned long epoll_fd)
{
...
struct net_rx_desc *__net_std_rx(struct net_rx *rx)
{
	...
	if (desc) {
		...
		if (net_std_get_cmsg_timestamp(&msg, &ts) > 0) {
			/* Convert from hardware clock domain to socket clock domain:
			 * - Hardware clock domain is the timestamp domain of the received packet and it
			 *   depends on the socket binding to the PHC device
			 * - Most of the cases, the bind is done to the socket domain and timestamp is
			 *   already in the right domain (so the conversion is just a pass through).Except, when
			 *   socket domain is a software clock on top of a PHC device.
			 */
...
static int __net_std_rx_multi(struct net_rx *rx, struct net_rx_desc **desc, unsigned int n)
{
	int i;
...
void net_std_rx_multi(struct net_rx *rx)
{
	...
	n = __net_std_rx_multi(rx, desc, NET_RX_BATCH);

	if (n)
		rx->func_multi(rx, desc, n);
}
...
static int net_std_tx_bind(struct net_tx *tx, struct net_address *addr)
{
	...
	if (setsockopt(tx->fd, SOL_SOCKET, SO_BINDTODEVICE, logical_port_name(addr->port), strlen(logical_port_name(addr->port)))) {
		os_log(LOG_ERR, "SO_BINDTODEVICE failed: %s\n", logical_port_name(addr->port));
		goto err_bind;
	}
...
int net_std_tx_init(struct net_tx *tx, struct net_address *addr)
{
	...
	if (addr) {
		...
		tx->clock_domain = net_address_get_clock_domain(addr);
...
int net_std_tx_ts_init(struct net_tx *tx, struct net_address *addr, void (*func)(struct net_tx *tx, uint64_t ts, unsigned int ts_info), unsigned long priv)
{
	int epoll_fd = priv;
	...
	if (net_std_set_socket_ts(addr, tx->fd, true) < 0) {
		os_log(LOG_ERR, "net_set_socket_ts error\n");
		goto err_set_ts;
	}
...
static const struct net_tx_ops_cb net_tx_std_ops = {
...
static const struct net_mem_ops_cb net_std_mem_ops = {
