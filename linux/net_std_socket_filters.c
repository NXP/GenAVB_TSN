...
static const struct sock_filter bpf_avtp_avdecc_filter[] = { /* Untagged packets with ETHERTYPE_AVTP and avtp header's subtype of AVTP_SUBTYPE_AECP, AVTP_SUBTYPE_ACMP or AVTP_SUBTYPE_ADP */
	BPF_FILTER_OUT_PACKET_OUTGOING_INSTR(7), /* filter out PACKET_OUTGOING */
	[2] = { BPF_LD | BPF_H | BPF_ABS, 0, 0, ETHER_ETYPE_OFFSET }, /* load etype value */
	[3] = { BPF_JMP | BPF_JEQ | BPF_K, 0, 5, ETHERTYPE_AVTP },
	[4] = { BPF_LD | BPF_B | BPF_ABS, 0, 0, ETH_HLEN }, /* load avtp subtype field (First byte in AVTP header) */
	[5] = { BPF_JMP | BPF_JEQ | BPF_K, 2, 0, AVTP_SUBTYPE_ADP },
	[6] = { BPF_JMP | BPF_JEQ | BPF_K, 1, 0, AVTP_SUBTYPE_ACMP },
	[7] = { BPF_JMP | BPF_JEQ | BPF_K, 0, 1, AVTP_SUBTYPE_AECP },
	[8] = { BPF_RET | BPF_K, 0, 0, 0x00040000 }, /* accept */
	[9] = { BPF_RET | BPF_K, 0, 0, 0x00000000 }, /* reject */
};

static const struct sock_filter bpf_avtp_maap_filter[] = { /* Untagged packets with ETHERTYPE_AVTP and avtp header's subtype of AVTP_SUBTYPE_MAAP */
	BPF_FILTER_OUT_PACKET_OUTGOING_INSTR(5), /* filter out PACKET_OUTGOING */
	[2] = { BPF_LD | BPF_H | BPF_ABS, 0, 0, ETHER_ETYPE_OFFSET }, /* load etype value */
	[3] = { BPF_JMP | BPF_JEQ | BPF_K, 0, 3, ETHERTYPE_AVTP },
	[4] = { BPF_LD | BPF_B | BPF_ABS, 0, 0, ETH_HLEN }, /* load avtp subtype field (First byte in AVTP header) */
	[5] = { BPF_JMP | BPF_JEQ | BPF_K, 0, 1, AVTP_SUBTYPE_MAAP },
	[6] = { BPF_RET | BPF_K, 0, 0, 0x00040000 }, /* accept */
	[7] = { BPF_RET | BPF_K, 0, 0, 0x00000000 }, /* reject */
};
...
int sock_filter_get_bpf_code(struct net_address *addr, void *buf, unsigned int *inst_count)
{
	...
	case PTYPE_AVTP:
		if (is_avtp_avdecc(addr->u.avtp.subtype)) {
			src_bpf = bpf_avtp_avdecc_filter;
			bpf_filter_inst_count = BPF_FILTER_ARRAY_SIZE(bpf_avtp_avdecc_filter);
		} else if (is_avtp_maap(addr->u.avtp.subtype)) {
			src_bpf = bpf_avtp_maap_filter;
			bpf_filter_inst_count = BPF_FILTER_ARRAY_SIZE(bpf_avtp_maap_filter);
		} else {
			rc = -1;
			os_log(LOG_ERR, "Unsupported AVTP subtype 0x%x\n", addr->u.avtp.subtype);
			goto exit;
		}
		break;
