 ...
 */
#define FILL_QUEUE_SIZE		(2*genavb_max(HW_RX_QUEUE_SIZE, 2*RX_QUEUE_SIZE))  // Must be a power of 2, bigger than the sum of both
...
typedef int  (*FUNC_bpf_map_update_elem)(int fd, const void *key, const void *value, __u64 flags);
typedef int  (*FUNC_bpf_obj_get)(const char *filename);
typedef int  (*FUNC_bpf_map_lookup_elem)(int fd, const void *key, void *value);
typedef int  (*FUNC_bpf_map_delete_elem)(int fd, const void *key);
typedef int  (*FUNC_xsk_umem__create)(struct xsk_umem **umem, void *umem_area, __u64 size, struct xsk_ring_prod *fill, struct xsk_ring_cons *comp, const struct xsk_umem_config *config);
typedef int  (*FUNC_xsk_umem__delete)(struct xsk_umem *umem);
typedef int  (*FUNC_xsk_socket__create)(struct xsk_socket **xsk, const char *ifname, __u32 queue_id, struct xsk_umem *umem, struct xsk_ring_cons *rx, struct xsk_ring_prod *tx, const struct xsk_socket_config *config);
typedef void (*FUNC_xsk_socket__delete)(struct xsk_socket *xsk);
typedef int  (*FUNC_xsk_umem__fd)(const struct xsk_umem *umem);
typedef int  (*FUNC_xsk_socket__fd)(const struct xsk_socket *xsk);
...
static struct net_xdp_umem *net_xdp_umem_create(unsigned int queue_index)
{
	...
	/* Make sure to init all umem's ring fields to zero before
	 * creating the umem.
	 */
	memset(umem, 0, sizeof(struct net_xdp_umem));
...
static int __net_xdp_rx_init(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc *desc),
...
int net_xdp_rx_init(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc *desc), unsigned long epoll_fd)
{
	return __net_xdp_rx_init(rx, addr, func, NULL, 1, 0, epoll_fd);
}

int net_xdp_rx_init_multi(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc **desc, unsigned int n), unsigned int packets, unsigned int time, unsigned long epoll_fd)
{
...
static int __net_xdp_rx_multi(struct net_rx *rx, struct net_rx_desc **desc, unsigned int n)
{
	int i;
...
void net_xdp_rx_multi(struct net_rx *rx)
{
	...
	n = __net_xdp_rx_multi(rx, desc, NET_RX_BATCH);
...
int net_xdp_tx_ts_init(struct net_tx *tx, struct net_address *addr, void (*func)(struct net_tx *tx, uint64_t ts, unsigned int ts_info), unsigned long priv)
{
	return -1;
}
...
static const struct net_tx_ops_cb net_tx_xdp_ops = {
...
static const struct net_mem_ops_cb net_xdp_mem_ops = {
