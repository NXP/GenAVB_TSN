...
static void xdp_dyn_load_libs_exit(struct xdp_dl_libs_ctx *dl_libs_ctx)
{
	if (dlclose(dl_libs_ctx->lib_bpf_hdl))
		os_log(LOG_ERR, "dlclose() failed: %s\n", dlerror());
}
...
#define LIBBPF_NAME	"libbpf.so.0"

#define LOAD_BPF_LIB_SYMBOL(ctx, func, status)                                          \
	do {                                                                            \
		ctx->func = (FUNC_##func) dlsym(ctx->lib_bpf_hdl, #func);                 \
		if (!ctx->func) {                                                        \
...
static int xdp_dyn_load_libs_init(struct xdp_dl_libs_ctx *dl_libs_ctx)
{
	...
	LOAD_BPF_LIB_SYMBOL(dl_libs_ctx, bpf_map_delete_elem, status);
	LOAD_BPF_LIB_SYMBOL(dl_libs_ctx, bpf_map_lookup_elem, status);
	LOAD_BPF_LIB_SYMBOL(dl_libs_ctx, bpf_map_update_elem, status);
	LOAD_BPF_LIB_SYMBOL(dl_libs_ctx, bpf_obj_get, status);
	LOAD_BPF_LIB_SYMBOL(dl_libs_ctx, xsk_umem__create, status);
	LOAD_BPF_LIB_SYMBOL(dl_libs_ctx, xsk_socket__create, status);
	LOAD_BPF_LIB_SYMBOL(dl_libs_ctx, xsk_socket__delete, status);
	LOAD_BPF_LIB_SYMBOL(dl_libs_ctx, xsk_umem__fd, status);
	LOAD_BPF_LIB_SYMBOL(dl_libs_ctx, xsk_socket__fd, status);
	LOAD_BPF_LIB_SYMBOL(dl_libs_ctx, xsk_umem__delete, status);
