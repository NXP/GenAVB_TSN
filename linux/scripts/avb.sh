...
get_alsa_device()
{
	...
	if [ -z "$1" ] || [ "$1" = "default" ]; then
		snd_soc_card_num=$("$alsa_tool" -l | grep -F "$2" | head -n1 | sed -n 's/.*card \([0-9]*\):.*/\1/p')
...
alsa_mixer_setup()
{
	local snd_soc_card_num=$(aplay -l | grep -F "$SND_SOC_CARD_NAME" | head -n1 | sed -n 's/.*card \([0-9]*\):.*/\1/p')
	if [ -z $snd_soc_card_num ]; then
		...
		'SabreSD')
			...
			amixer -c $snd_soc_card_num -q sset Headphone 70,70
		...
		'SxSabreSD')
			...
			# Capture
			amixer -c $snd_soc_card_num -q sset Capture 0,0
			;;
		'imx6ullEvkBoard')
			# Playback
			amixer -c $snd_soc_card_num -q sset Speaker 150,150
		...
		'SabreAI'|'Vybrid'|*)
			# Capture
			# 6.0dB Gain
			amixer -c $snd_soc_card_num -q sset ADC1 140,140

			# Muted
			amixer -c $snd_soc_card_num -q sset ADC2 0,0

			# Playback
			# 0.0dB Gain
			amixer -c $snd_soc_card_num -q sset DAC1 255,255

			# Muted
			amixer -c $snd_soc_card_num -q sset DAC2 0,0
			amixer -c $snd_soc_card_num -q sset DAC3 0,0
			amixer -c $snd_soc_card_num -q sset DAC4 0,0
			;;
		esac
