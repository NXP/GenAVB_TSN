...
set_machine_variables()
{
	...
	imx6ullEvkBoard*)
		...
		CFG_EXTERNAL_CONTROLS_APP_OPT="-m Headphone $CFG_EXTERNAL_CONTROLS_APP_OPT"
		CPU_FREQ="528000"
	...
	'imx8mmevk')
		...
		CFG_EXTERNAL_CONTROLS_APP_OPT="-m dummy $CFG_EXTERNAL_CONTROLS_APP_OPT"
	...
	'imx93evkauto')
		...
		if [ "$AVB_MODE" -eq 1 ]; then
			...
			ITF="$(get_network_interface "fec")"
			if [ -z "$ITF" ]; then
				echo "Failed to retrieve AVB interface using the fec driver"
				exit 1
			fi
			...
			ITF="$(get_network_interface "imx-dwmac")"
			if [ -z "$ITF" ]; then
				echo "Failed to retrieve TSN interface using the imx-dwmac driver"
				exit 1
			fi
	...
	*)
	...
	if [ -n "$SND_SOC_CARD_NAME" ]; then
		...
		CFG_EXTERNAL_CONTROLS_APP_OPT="-c hw:$(get_sound_card_number "$SND_SOC_CARD_NAME" "playback") $CFG_EXTERNAL_CONTROLS_APP_OPT"
	fi
}
...
avdecc_ipc_nodes()
{
	...
	mknod /dev/ipc_avdecc_media_stack_rx c "$major" 208
	mknod /dev/ipc_avdecc_media_stack_tx c "$major" 209
	mknod /dev/ipc_media_stack_avdecc_rx c "$major" 210
	mknod /dev/ipc_media_stack_avdecc_tx c "$major" 211
	...
	mknod /dev/ipc_avdecc_controller_rx c "$major" 212
	mknod /dev/ipc_avdecc_controller_tx c "$major" 213
	mknod /dev/ipc_controller_avdecc_rx c "$major" 214
	mknod /dev/ipc_controller_avdecc_tx c "$major" 215
	mknod /dev/ipc_avdecc_controller_sync_rx c "$major" 216
	mknod /dev/ipc_avdecc_controller_sync_tx c "$major" 217
}
...
avtp_ipc_nodes()
{
	...
	mknod /dev/ipc_avtp_stats_rx c "$major" 218
	mknod /dev/ipc_avtp_stats_tx c "$major" 219
