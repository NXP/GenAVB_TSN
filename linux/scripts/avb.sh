...
AVB_LOG_FILE_BR=/var/log/$AVB_APP_BR
AVB_PID_FILE=/var/run/$AVB_APP.pid
AVB_PID_FILE_BR=/var/run/$AVB_APP_BR.pid
...
get_alsa_device()
{
	...
	if [ -z "$1" ] || [ "$1" = "default" ]; then
		snd_soc_card_num=$(aplay -l | grep -F "$2" | head -n1 | sed -n 's/.*card \([0-9]*\):.*/\1/p')
...
'Vybrid')
	...
	CFG_ALSA_DEVICE=$(get_alsa_device "$CFG_ALSA_DEVICE" "$SND_SOC_CARD_NAME")
	;;
'SabreSD'|'SxSabreSD')
	# Sabre SD
	IRQ_DMA_NAME="sdma"
...
'imx8mmevk')
	...
	# i.MX8MM EVK board does not support Media Clock Recovery, disable CRF clock slaving
	CFG_MCR_SLAVE=""
	;;
'imx8mpevk')
...
stop_genavb_br()
{
	kill_process_pidfile $AVB_PID_FILE_BR
}
...
start_genavb_br()
{
	taskset "$CPU_MASK" $AVB_APP -b -f "$GENAVB_CFG_FILE" > $AVB_LOG_FILE_BR 2>&1 &
	echo $! > $AVB_PID_FILE_BR
}
