...
HAS_TC_FLOWER_PSFP_OFFLOAD=0
HAS_REGISTER_PRIORITY_MAPPING_OFFLOAD=0
...
'imx95evk')
	...
	HAS_REGISTER_PRIORITY_MAPPING_OFFLOAD=1

	PRIORITY_MAPPING_BASE="0x4CC00150"
	# based on PCP_TO_QOS_MAP_6TC_0SR mapping
	PRIORITY_MAPPING_VALUE="0x54332201"
...
'imx943evk')
	# Number of TX/RX queues
	NUM_TC=${IMX943EVK_TSN_ENDPOINT_NUM_TC}
	...
	PRIO_REGS="0x4CC80150 0x4CCC0150 0x4CD00150 0x4CD40150"
	PRIO_OFFSET=0x150
	PRIO_BASE=$( { awk '{print $1}' < /sys/class/net/"$ITF"/device/resource | head -n 1; } 2>/dev/null )
	if [ -z "$PRIO_BASE" ]; then
		echo "priority mapping error: Invalid resource for $ITF"
		exit 1
	fi

	PRIORITY_MAPPING_BASE=$(printf "0x%X" $((PRIO_BASE + PRIO_OFFSET)))

	if ! list_contains "$PRIO_REGS" "$PRIORITY_MAPPING_BASE"; then
		echo "priority mapping error: Invalid HW register $PRIORITY_MAPPING_BASE"
		exit 1
	fi

	# based on PCP_TO_QOS_MAP_4TC_0SR mapping
	PRIORITY_MAPPING_VALUE="0x33221100"
...
{
	if [ "$NUM_TC" -ne "4" ] && [ "$NUM_TC" -ne "5" ] && [ "$NUM_TC" -ne "6" ]; then
		echo "Error: Unsupported number of HW traffic classes"
		exit 1
	fi

	if [ "$NUM_TC" -eq "4" ]; then	
		# PCP mapping (802.1Q Table 8-5, no SR Class, 4 traffic classes):
		PCP_TO_QOS_MAP="${PCP_TO_QOS_MAP_4TC_0SR}"
	elif [ "$NUM_TC" -eq "5" ]; then
...
set_vlan_config()
{
	...
	# The XDP program expects vlan tagged packets for now.
	if [ "${AF_XDP_MODE}" = "true" ]; then
		echo "# Disable RX Vlan offload for XDP path"

		ethtool -K "$ITF" rx-vlan-offload off
	fi
}
...
setup_tx_scheduling_and_rx_classification_settings()
{
	...
	if ! grep -q cls_flower "/lib/modules/$(uname -r)/modules.builtin"; then
	...
	if [ "$HAS_TC_FLOWER_VLAN_CLASSIFICATION_OFFLOAD" -eq 1 ]; then
		...
		if [ "$HAS_REGISTER_PRIORITY_MAPPING_OFFLOAD" -eq 1 ]; then
			echo "# Offload priority to ring buffer mapping using Station interface IPV to ring mapping register (SIIPVBDRMR0)"
			/unit_tests/memtool -32 "$PRIORITY_MAPPING_BASE"="$PRIORITY_MAPPING_VALUE"
		fi
