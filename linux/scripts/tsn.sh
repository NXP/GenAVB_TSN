...
set_system_cfg_file()
{
	# For endpoints supporting both TSN and AVB, set the right symlink for the system.cfg file.
	# Both configs should be present for such endpoints.
	if [ ! -e ${GENAVB_SYSTEM_CONFIG_FILE}.avb ] || [ ! -e ${GENAVB_SYSTEM_CONFIG_FILE}.tsn ]; then
		return
	fi
	...
	echo "Using file '$(readlink -f ${GENAVB_SYSTEM_CONFIG_FILE})' as system configuration file"
}
