...
set_system_cfg_file()
{
	# For platforms supporting multiple configs Endpoint TSN, Endpoint AVB and/or Bridge, set the right symlink for the system.cfg file.
	# All needed configuration files should be present for these usecases.
	if ! (([ -e ${GENAVB_SYSTEM_CONFIG_FILE}.avb ] && [ -e ${GENAVB_SYSTEM_CONFIG_FILE}.tsn ]) ||
			([ -e ${GENAVB_SYSTEM_CONFIG_FILE}.bridge ] && [ -e ${GENAVB_SYSTEM_CONFIG_FILE}.avb ]) ||
			([ -e ${GENAVB_SYSTEM_CONFIG_FILE}.bridge ] && [ -e ${GENAVB_SYSTEM_CONFIG_FILE}.tsn ])); then
		return
	fi

	if [ -f ${GENAVB_SYSTEM_CONFIG_FILE} ] && [ ! -L ${GENAVB_SYSTEM_CONFIG_FILE} ]; then
		echo "Failed to set system.cfg: can not overwrite regular file ${GENAVB_SYSTEM_CONFIG_FILE} with a symlink"
		exit 1
	fi
...
start_tsn_endpoint_process()
{
	...
	if [ "$AVB_MODE" -eq 1 ]; then
		# AVB network mode
		tsn_app_net_mode="-n 1"
	else
		# Standard network mode
		tsn_app_net_mode="-n 2"
	fi

	if [ "$endpoint_number" -eq 1 ]; then
		$TSN_APP -f "$gptp_conf_file" -e $endpoint_number "$tsn_app_net_mode" > /var/log/$log_file 2>&1 &
		...
		$TSN_APP -f "$gptp_conf_file" -e $endpoint_number "$tsn_app_net_mode" >& >(tee -a /var/log/$TSN_APP | stdbuf -oL grep -v srp >> /var/log/$FGPTP_APP) &
...
start_tsn_stack_br()
{
	...
	# Bridge stack is always in standard network mode
	tsn_app_net_mode="-n 2"
	...
	$TSN_APP -b -f "$PTP_CFG_FILE_BR" "$tsn_app_net_mode" >& >(tee -a /var/log/$TSN_APP_BR | stdbuf -oL grep -v srp >> /var/log/$FGPTP_APP_BR) &
