...
set_system_cfg_file()
{
	...
	if [ "$CFG_SYSTEM_CFG_NO_OVERRIDE" -eq 0 ]; then
		if [ -n "$ITF_OVERRIDE" ]; then
			# Override the default system configuration interface.
			echo "Override '${system_cfg_filename}': Use interface '$ITF_OVERRIDE'"
			sed -e "s|eth[0-9]\+|$ITF_OVERRIDE|g" -i "${system_cfg_filename}"
		fi

		if [ -n "$PTP_DEVICE_OVERRIDE" ]; then
			# Override the default system configuration PTP device.
			echo "Override '${system_cfg_filename}': Use PTP device '$PTP_DEVICE_OVERRIDE'"
			sed -e "s|/dev/ptp[0-9]\+|${PTP_DEVICE_OVERRIDE}|g" -i "${system_cfg_filename}"
		fi
		...
		if [ -n "$TSN_ENDPOINT_NUM_TC_OVERRIDE" ]; then
			...
			if [ "$TSN_ENDPOINT_NUM_TC_OVERRIDE" -eq 5 ]; then
...
start_tsn_endpoint_process()
{
	...
	if [ "$CFG_USE_PHC2SYS" -eq 1 ]; then
		...
		if [ -z "${phc_running}" ]; then
			PHC_DEVICE=$(get_system_config_phc_device "${system_cfg_filename}")
