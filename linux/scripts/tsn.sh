...
disable_eee_on_endpoints()
{
	# Extract interfaces (comma seperated list) from endpoint field
	# in LOGICAL_PORT section in the system config file.
	ep_interfaces=$(awk '
	    /^\[LOGICAL_PORT\]/ { in_section=1; next }
	    /^\[/ { in_section=0 }
	    in_section && /^endpoint[[:space:]]*=/ {
	        sub(/^endpoint[[:space:]]*=[[:space:]]*/, "")
	        print
	        exit
	    }
	' "$GENAVB_SYSTEM_CONFIG_FILE")

	# Process each interface (comma-separated)
	if [ -n "$ep_interfaces" ]; then
		# Replace commas with spaces and iterate
		for iface in $(echo "$ep_interfaces" | tr ',' ' '); do
			# Trim whitespace
			iface=$(echo "$iface" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')

			if [ -n "$iface" ] && [ "$iface" != "off" ]; then
				echo "Disabling EEE on $iface"
				ethtool --set-eee "$iface" eee off
			fi
		done
	else
		echo "Can not disable EEE: No endpoint interfaces found in (${GENAVB_SYSTEM_CONFIG_FILE})"
	fi
}
