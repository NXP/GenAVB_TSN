...
#define TSN_DEFAULT_NET_MODE		"avb"
...
static int process_system_config(struct os_net_config *net_config, const char *filename, bool is_bridge)
{
	...
	if(cfg_get_net_mode(configtree, net_mode_name, TSN_DEFAULT_NET_MODE, &net_config->gptp_net_mode, &net_config->enabled_modes_flag))
		goto err_parse;

	net_mode_name = (is_bridge) ? "bridge_srp_net_mode" : "endpoint_srp_net_mode";
	if(cfg_get_net_mode(configtree, net_mode_name, TSN_DEFAULT_NET_MODE, &net_config->srp_net_mode, &net_config->enabled_modes_flag))
		goto err_parse;
