...
typedef enum {
	...
	OS_CLOCK_AVTP_MEDIA_0 = 18, /**< AVTP media clock 0 (Alias to either Local or Target gPTP): All timestamps (packet timestamps, avtp timestamps and application timestamps)
					should all be in this domain upon entry to AVTP layer. */
	OS_CLOCK_AVTP_MEDIA_1 = 19, /**< AVTP media clock 1 (Alias to either Local or Target gPTP): All timestamps (packet timestamps, avtp timestamps and application timestamps)
					should all be in this domain upon entry to AVTP layer. */
	OS_CLOCK_MAX
} os_clock_id_t;
...
/**
 * Converts multiple timestamps from one clock base to another.
 * Both clocks must be based on the same hardware clock.
 * \param id_src source clock id.
 * \param ns_src array to source timestamps
 * \param num_ts array size of source timestamps
 * \param id_dst destination clock id
 * \param ns_dst array to destination timestamps.
 * \return      0 on success, or negative value and all destination times are set to zero on error.
 */
 ...
 */
static inline int os_clock_convert(os_clock_id_t id_src, u64 ns_src, os_clock_id_t id_dst, u64 *ns_dst)
{
	return os_clock_convert_multi(id_src, &ns_src, 1, id_dst, ns_dst);
}
...
/**
 * Get avtp media clock id from port id.
 * \param port_id       port id
 * \return              clock id
 */
...
/**
 * Returns true if both clocks represent the same clock domain (i.e mapped to each other).
 * \param clk_id_0	clock id of the first clock
 * \param clk_id_1	clock id of the second clock
 * \return		boolean
 */
