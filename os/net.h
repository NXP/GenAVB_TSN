 ...
 * \param priv	platform dependent code private data
 */
int net_rx_init_multi(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc **desc, unsigned int n), unsigned int packets, unsigned int time, unsigned long priv);
...
/** Network receive handler
 *
 * The function dequeues up to n packets into an array of network descriptors and returns the number of packets received.
 *
 * \return	number of actual packets received
 * \param rx	pointer to network receive context
 * \param desc	pointer to rx network descriptor array
 * \param n		number of packets to dequeue
 */
 ...
 *
 * The function tries to transmit an array of descriptors and returns the number of successfully transmitted ones.
 * Processing of the array stops at the first transmit error.
 * If not all descriptors are trannsmitted successfully, the caller is responsible for handling the remaining
 * descriptors (typically freeing them).
 *
 * \return Number of successfully transmited descriptors.
