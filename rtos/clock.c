...
/*
 * Compact clock instances based on compile time configuration options.
 * clock_map[] is used to translate between public clock id's and internal ones.
 */
enum {
	CLK_INST_SYSTEM_MONOTONIC,
	CLK_INST_SYSTEM_MONOTONIC_COARSE,
	CLK_INST_SYSTEM_MONOTONIC_1,
#if defined(CONFIG_GENAVB_TSN_AVTP)
	CLK_INST_MEDIA_HW_0,
	CLK_INST_MEDIA_HW_1,
	CLK_INST_MEDIA_REC_0,
	CLK_INST_MEDIA_REC_1,
	CLK_INST_MEDIA_PTP_0,
	CLK_INST_MEDIA_PTP_1,
#endif
#if defined(CONFIG_GENAVB_TSN_GPTP) || defined(CONFIG_GENAVB_TSN_API)
	CLK_INST_GPTP_EP_0_0,
	CLK_INST_GPTP_EP_0_1,
#if CFG_BRIDGE_NUM
	CLK_INST_GPTP_BR_0_0,
	CLK_INST_GPTP_BR_0_1,
#endif
#endif /* CONFIG_GENAVB_TSN_GPTP || CONFIG_GENAVB_TSN_API */
	CLK_INST_LOCAL_EP_0,
#if CFG_BRIDGE_NUM
	CLK_INST_LOCAL_BR_0,
#endif
...
static struct os_clock clock_instances[CLK_INST_MAX];

/* Maps constant os_clock_id_t values to clock_instances[] entries; NULL = not compiled in. */
static struct os_clock * const clock_map[OS_CLOCK_MAX] = {
	[OS_CLOCK_SYSTEM_MONOTONIC] = &clock_instances[CLK_INST_SYSTEM_MONOTONIC],
	[OS_CLOCK_SYSTEM_MONOTONIC_COARSE] = &clock_instances[CLK_INST_SYSTEM_MONOTONIC_COARSE],
	[OS_CLOCK_SYSTEM_MONOTONIC_1] = &clock_instances[CLK_INST_SYSTEM_MONOTONIC_1],
#if defined(CONFIG_GENAVB_TSN_AVTP)
	[OS_CLOCK_MEDIA_HW_0] = &clock_instances[CLK_INST_MEDIA_HW_0],
	[OS_CLOCK_MEDIA_HW_1] = &clock_instances[CLK_INST_MEDIA_HW_1],
	[OS_CLOCK_MEDIA_REC_0] = &clock_instances[CLK_INST_MEDIA_REC_0],
	[OS_CLOCK_MEDIA_REC_1] = &clock_instances[CLK_INST_MEDIA_REC_1],
	[OS_CLOCK_MEDIA_PTP_0] = &clock_instances[CLK_INST_MEDIA_PTP_0],
	[OS_CLOCK_MEDIA_PTP_1] = &clock_instances[CLK_INST_MEDIA_PTP_1],
#endif
#if defined(CONFIG_GENAVB_TSN_GPTP) || defined(CONFIG_GENAVB_TSN_API)
	[OS_CLOCK_GPTP_EP_0_0] = &clock_instances[CLK_INST_GPTP_EP_0_0],
	[OS_CLOCK_GPTP_EP_0_1] = &clock_instances[CLK_INST_GPTP_EP_0_1],
#if CFG_BRIDGE_NUM
	[OS_CLOCK_GPTP_BR_0_0] = &clock_instances[CLK_INST_GPTP_BR_0_0],
	[OS_CLOCK_GPTP_BR_0_1] = &clock_instances[CLK_INST_GPTP_BR_0_1],
#endif
#endif /* CONFIG_GENAVB_TSN_GPTP || CONFIG_GENAVB_TSN_API */
	[OS_CLOCK_LOCAL_EP_0] = &clock_instances[CLK_INST_LOCAL_EP_0],
#if CFG_BRIDGE_NUM
	[OS_CLOCK_LOCAL_BR_0] = &clock_instances[CLK_INST_LOCAL_BR_0],
#endif
};
...
static os_clock_id_t clk_id_to_clk_id[OS_CLOCK_MAX] = {
	...
	[OS_CLOCK_AVTP_MEDIA_0] = OS_CLOCK_GPTP_EP_0_0, /* AVTP media clock 0 is by default mapped to interface 0 target gPTP clock */
	[OS_CLOCK_AVTP_MEDIA_1] = OS_CLOCK_GPTP_EP_1_0, /* AVTP media clock 1 is by default mapped to interface 1 target gPTP clock */
};

static unsigned int clk_flags[OS_CLOCK_MAX] = {
	[OS_CLOCK_GPTP_EP_0_0] = OS_CLOCK_FLAGS_SUPPORTS_REMAP | OS_CLOCK_FLAGS_HW_RATIO,
	[OS_CLOCK_GPTP_EP_0_1] = OS_CLOCK_FLAGS_SUPPORTS_REMAP,
	[OS_CLOCK_GPTP_EP_1_0] = OS_CLOCK_FLAGS_SUPPORTS_REMAP | OS_CLOCK_FLAGS_HW_RATIO,
	[OS_CLOCK_GPTP_EP_1_1] = OS_CLOCK_FLAGS_SUPPORTS_REMAP,
	[OS_CLOCK_LOCAL_EP_0] = OS_CLOCK_FLAGS_SUPPORTS_REMAP,
	[OS_CLOCK_LOCAL_EP_1] = OS_CLOCK_FLAGS_SUPPORTS_REMAP,
	[OS_CLOCK_GPTP_BR_0_0] = OS_CLOCK_FLAGS_SUPPORTS_REMAP | OS_CLOCK_FLAGS_HW_RATIO,
	[OS_CLOCK_GPTP_BR_0_1] = OS_CLOCK_FLAGS_SUPPORTS_REMAP,
	[OS_CLOCK_LOCAL_BR_0] = OS_CLOCK_FLAGS_SUPPORTS_REMAP,
	[OS_CLOCK_AVTP_MEDIA_0] = OS_CLOCK_FLAGS_SUPPORTS_REMAP,
	[OS_CLOCK_AVTP_MEDIA_1] = OS_CLOCK_FLAGS_SUPPORTS_REMAP,
};
...
/**
 * Checks if a clock can be remapped.
 * Called before the clock is initialized.
 */
static bool clock_id_supports_remap(os_clock_id_t clk_id)
{
	...
	if (!(clk_flags[clk_id] & OS_CLOCK_FLAGS_SUPPORTS_REMAP))
		goto invalid;
...
static struct os_clock *clock_id_to_clock(os_clock_id_t clk_id)
{
	...
	return clock_map[clock_id(clk_id)];
...
static void clock_task_timekeeping(void *pvParameters)
{
	...
	while (1) {
		for (i = 0; i < CLK_INST_MAX; i++)
			if (clock_instances[i].hw_clk)
				if (os_clock_gettime64(clock_instances[i].id, &now) < 0)
...
int os_clock_convert_multi(os_clock_id_t clk_id_src, uint64_t *ns_src, unsigned int num_ts, os_clock_id_t clk_id_dst, uint64_t *ns_dst)
{
	...
	if (c_src->hw_clk == c_dst->hw_clk) {
		...
		for (i = 0; i < num_ts; i++) {
			...
			if (hw_clock_convert(c_src->hw_clk, ns_dst[i], c_dst->hw_clk, &ns_dst[i]) < 0) {
				rtos_mutex_global_unlock();
...
int clock_time_from_hw_any(os_clock_id_t clk_id, struct hw_clock *hw_clk, uint64_t hw_ns, uint64_t *ns)
{
	struct os_clock *c;
	...
	if (c->hw_clk != hw_clk)
		if (hw_clock_convert(hw_clk, hw_ns, c->hw_clk, &hw_ns) < 0) {
			rtos_mutex_global_unlock();
			goto err;
		}
...
int clock_id_remap(os_clock_id_t clk_id, os_clock_id_t dst_clk_id)
{
	...
	/* Keep the static mapping between endpoint target gPTP and media clock */
	if (clk_id == OS_CLOCK_GPTP_EP_0_0)
		clk_id_to_clk_id[OS_CLOCK_AVTP_MEDIA_0] = dst_clk_id;
	else if (clk_id == OS_CLOCK_GPTP_EP_1_0)
		clk_id_to_clk_id[OS_CLOCK_AVTP_MEDIA_1] = dst_clk_id;
...
int clock_set_adjustment_flags(os_clock_id_t clk_id, unsigned int flags)
{
	if ((clk_id < 0) || (clk_id >= OS_CLOCK_MAX))
		return -1;

	clk_flags[clk_id] &= ~(OS_CLOCK_FLAGS_HW_ADJUSTEMENTS_MASK);

	clk_flags[clk_id] |= (flags & OS_CLOCK_FLAGS_HW_ADJUSTEMENTS_MASK);
