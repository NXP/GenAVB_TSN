...
/**
 @file
 @brief RTOS generic sw clock service implementation
 @details
*/
...
#define OS_CLOCK_FLAGS_HW_RATIO  	(1U << 0)
/* Clock offset adjustments affect the hardware clock */
#define OS_CLOCK_FLAGS_HW_OFFSET 	(1U << 1)
/* Clock supports remapping to another clock (both as source and destination of the mapping) */
#define OS_CLOCK_FLAGS_SUPPORTS_REMAP 	(1U << 2)

#define OS_CLOCK_FLAGS_HW_ADJUSTEMENTS_MASK (OS_CLOCK_FLAGS_HW_RATIO | OS_CLOCK_FLAGS_HW_OFFSET)
 ...
 * The time must come from the parent hw clock, of the sw clock
 * given as argument.
 * \param id		clock id.
...
/**
 * Convert hw clock time to sw clock time.
 * The time may come from a hw clock that is not the parent, of the sw clock
 * given as argument.
 * \param clk_id	clock id.
 * \param hw_clk	hw clock used for the hw time
 * \param hw_ns		hw time to convert
 * \param ns		pointer to u64 variable that will hold the result.
 * \return		0 on success, or negative value on error.
 */
 ...
 * The provided cycles is the raw counter value of the parent hw clock
 * of the sw clock time given as argument.
...
/**
 *  Remaps a clock id to another one, making them effectively the same.
 *  Must be called before the clock is initialized.
 */
int clock_id_remap(os_clock_id_t clk_id, os_clock_id_t dst_clk_id);

/**
 * Sets the clock parent hardware clock.
 * Must be called before the clock is initialized.
 */
