...
#if (defined(CONFIG_GENAVB_TSN_AVTP) && SHOW_MCLOCK_STATS)
extern struct mclock_drv *mclk_drv_h;
static void mclock_rec_pll_stats(struct mclock_rec_pll *rec)
{
	struct mclock_rec_pll_stats_common *stats = &rec->c.stats;
	struct mclock_rec_pll_stats *stats_rtos = &rec->stats;
	...
	os_log(LOG_INFO, "gpt_rec event               = %d\n", stats_rtos->irq_count);
	os_log(LOG_INFO, "gpt_rec event fec           = %d\n", stats_rtos->irq_count_fec_event);
	os_log(LOG_INFO, "gptp_reloaded               = %d\n", stats->gptp_reloaded);
	os_log(LOG_INFO, "numerator                   = %d\n", stats_rtos->pll_numerator);
	...
	os_log(LOG_INFO, "locked state transition     = %d\n", stats->locked_state);

#if MCLOCK_PLL_REC_TRACE
	os_log(LOG_INFO_RAW, "[%-3s] %-10s %-11s %-11s %-6s %-11s %-15s %-15s %-15s %-15s %-15s %-11s %-10s %-10s %-11s %-11s\n",
			"idx", "meas", "meas_diff",
	...
	for (i = 0; i < rec->c.trace_count; i++) {
		struct mclock_rec_trace *t = &rec->c.trace[i];
		os_log(LOG_INFO_RAW, "\n[%-3d] %-10u %-11d %-11u %-6u %-11d %-15llu %-15llu %-15llu %-15lld %-15llu %-11d %-10lu %-10lu %-11d %-11d\n",
...
#endif /* (defined(CONFIG_GENAVB_TSN_AVTP) && SHOW_MCLOCK_STATS) */
...
void net_port_show_stats(void)
{
	...
	for (p = 0; p < CFG_PORTS; p++) {
		...
		net_port_rx_show("gptp", &net_rx->ptype_hdlr[PTYPE_INST_PTP].stats[p]);
#endif
#if defined(CONFIG_GENAVB_TSN_AVTP)
		net_port_rx_show("avtp", &net_rx->ptype_hdlr[PTYPE_INST_AVTP].stats[p]);
#endif
#if defined(CONFIG_GENAVB_TSN_SRP)
		net_port_rx_show("mrp", &net_rx->ptype_hdlr[PTYPE_INST_MRP].stats[p]);
#endif
#if defined(CONFIG_GENAVB_TSN_SOCKET)
		net_port_rx_show("l2", &net_rx->ptype_hdlr[PTYPE_INST_L2].stats[p]);
#endif
		net_port_rx_show("other", &net_rx->ptype_hdlr[PTYPE_INST_OTHER].stats[p]);
#if defined(CONFIG_GENAVB_TSN_AVTP)
		net_port_rx_show("maap", avtp_maap_stats(p));
		net_port_rx_show("avdecc", avtp_avdecc_stats(p));
#endif
	}
