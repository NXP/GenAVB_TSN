...
__init int hw_timer_register(hw_clock_id_t id, struct hw_timer *timer, unsigned int flags)
{
...
struct hw_timer *hw_timer_request(hw_clock_id_t id, unsigned int flags, void (*func)(void *data), void *data)
{
	struct hw_timer *timer;
	...
	/* Try to get a timer with the exact feature set as requested */
	timer = __hw_timer_request(id, flags, hw_timer_user_flags(~flags));

	/* If no exact match, try to get a timer with extra features */
	if (!timer)
