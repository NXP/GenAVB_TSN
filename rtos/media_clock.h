...
struct mclock_timer {
	...
	int (*irq_func)(struct mclock_dev *dev, void *data, unsigned int ticks);
	unsigned int period;
	unsigned int count;
	unsigned int ticks;
...
struct mclock_dev {
	...
	int (*start)(struct mclock_dev *dev, struct mclock_start *start);
	int (*stop)(struct mclock_dev *dev);
	int (*clean)(struct mclock_dev *dev, struct mclock_clean *clean);
	int (*reset)(struct mclock_dev *dev);
	int (*config)(struct mclock_dev *dev, struct mclock_sconfig *config);
	int (*open)(struct mclock_dev *dev, int port);
	void (*release)(struct mclock_dev *dev);
...
int  mclock_register_timer(struct mclock_dev *dev, int (*irq_func)(struct mclock_dev *dev, void *data, unsigned int ticks), unsigned int period);
