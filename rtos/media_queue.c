/*
 * Copyright 2018-2019, 2021, 2023-2025 NXP
 *
...
static void rx_write_event(struct genavb_event *avb_event, struct media_desc *desc, struct event_info *event_info)
{
	...
	len_now = genavb_min(event_info->dst_len, event_info->src_len);
	...
	while (!finished) {
		...
		if (!skip_event) {
			if (total_dst_len < src_len) {
				...
				event_info.dst_len = genavb_min(event_info.src_len, event_info.dst_len);
			...
			if (event_info.total_dst_len < event_info.src_len) {
				...
				dst_len = genavb_min(src_len, dst_len);
				total_dst_len = src_len;
			}
		}

		/* Determine size of data copy */
		len_now = genavb_min(dst_len, src_len);
		...
		if (len_now) {
			...
			if (stride_overhead)
				src_len_now = genavb_min(len_now, src_frame_len);
			else
				src_len_now = len_now;
...
void media_api_set_callback(struct media_queue *mqueue, int (*callback)(void *data), void *data)
{
	rtos_mutex_lock(&table_mutex, RTOS_WAIT_FOREVER);
