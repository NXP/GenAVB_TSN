...
#define SOCKET_OTHER_RX_DEFAULT_PACKETS 8

static struct net_tx_desc *__net_tx_alloc(unsigned int size)
{
	struct net_tx_desc *desc = __net_alloc(size);
...
{
	return __net_tx_alloc(size);
}

struct net_rx_desc *net_pool_rx_alloc(unsigned int size)
{
	struct net_rx_desc *desc = __net_alloc(size);
...
{
	struct net_tx_desc *desc = __net_tx_alloc(src->len);
	if (!desc)
		goto exit;
...
static inline void __net_tx_free(struct net_tx_desc *desc)
{
	if (desc->flags & NET_TX_REUSE)
		desc->flags &= ~NET_TX_BUSY;
	else
		__net_free(desc);
}
...
{
	__net_tx_free((struct net_tx_desc *)entry);
}

void net_rx_desc_free(void *data, unsigned long entry)
{
	__net_free((void *)entry);
}
...
void net_free_multi(void **buf, unsigned int n)
{
	...
	for (i = 0; i < n; i++)
		__net_tx_free(buf[i]); /* ok also for net_rx_desc, even if not required */
}

int __net_rx_multi(struct net_rx *rx, struct net_rx_desc **desc, unsigned int n)
{
	return socket_read(rx->socket, desc, n);
}
...
void net_rx_multi(struct net_rx *rx)
{
	...
	len = socket_read(rx->socket, desc, rx->batch);
	if (len < 0)
		len = 0;

	rx->func_multi(rx, desc, len);

	socket_rx_enable_callback(rx->socket);
}
...
int net_rx_set_callback(struct net_rx *rx, void (*callback)(void *data), void *data)
{
...
{
	return socket_rx_enable_callback(rx->socket);
}

int net_tx_set_callback(struct net_tx *tx, void (*callback)(void *data), void *data)
{
	return socket_set_callback(tx->socket, callback, data);
}

int net_tx_enable_callback(struct net_tx *tx)
{
	return socket_tx_enable_callback(tx->socket);
}

int net_rx_set_option(struct net_rx *rx, unsigned int type, unsigned long val)
{
			...
			unsigned long handle)
{
	...
	rx->socket = socket_open((rtos_mqueue_t *)handle, rx, 1);
	if (!rx->socket)
		goto err_open;
	...
	if (addr) {
		...
		if (addr->ptype == PTYPE_OTHER)
			socket_set_option(rx->socket, SOCKET_OPTION_PACKETS, SOCKET_OTHER_RX_DEFAULT_PACKETS);
	}
...
int net_rx_init(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc *desc), unsigned long handle)
{
	return __net_rx_init(rx, addr, func, NULL, 0, 0, handle);
}

int net_rx_init_multi(struct net_rx *rx, struct net_address *addr, void (*func)(struct net_rx *rx, struct net_rx_desc **desc, unsigned int n), unsigned int packets, unsigned int time, unsigned long handle)
{
...
int __net_tx_init(struct net_tx *tx, struct net_address *addr, unsigned long handle)
{
	...
	tx->socket = socket_open((rtos_mqueue_t *)handle, tx, 0);
	if (!tx->socket)
		goto err_open;
...
void net_tx_enable_raw(struct net_tx *tx)
{
	...
	sock->flags |= SOCKET_FLAGS_TX_RAW;
}
...
int net_tx_multi(struct net_tx *tx, struct net_tx_desc **desc, unsigned int n)
{
	...
	return socket_write(sock, desc, n);
}

int net_tx_done(struct net_tx *tx, struct net_tx_desc **desc, unsigned int n)
{
	struct net_socket *sock = tx->socket;

	return socket_tx_done(sock, desc, n);
}
...
int net_tx_ts_init(struct net_tx *tx, struct net_address *addr, void (*func)(struct net_tx *tx, uint64_t ts, unsigned int ts_info), unsigned long handle)
{
	os_log(LOG_DEBUG, "\n");
...
int net_port_set_max_frame_size(unsigned int port_id, uint16_t size)
{
	...
	rc = port_set_max_frame_size(port->phys, size);
	if (rc < 0)
		goto err;
...
unsigned int net_port_priority_to_traffic_class(unsigned int port_id, uint8_t priority)
{
	...
	return port_priority_to_traffic_class(port->phys, priority);
}
...
int net_port_traffic_class_set(unsigned int port_id, struct genavb_traffic_class_config *config)
{
	...
	logical_port = logical_port_get(port_id);
	if (!logical_port) {
		rc = -GENAVB_ERR_INVALID_PORT;
		goto err;
	}

	port = logical_port->phys;

	rc = port_set_priority_to_traffic_class_map(port, config->tc);
	if (rc < 0)
		goto err;

	if (port->drv_ops.set_priority_to_tc_map)
		rc = port->drv_ops.set_priority_to_tc_map(port);
...
int net_port_status_get(unsigned int port_id, bool *up, bool *duplex, uint64_t *rate)
{
	...
	rc = port_status(port->phys, &status);
	if (rc < 0)
		goto err;

	*up = status.up;
	*duplex = status.full_duplex;
	*rate = status.rate;
