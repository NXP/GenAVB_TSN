...
{
	return mdio[1].write(mdio[1].handle, phyAddr, regAddr, data);
}
#endif
...
{
	return mdio[1].read(mdio[1].handle, phyAddr, regAddr, data);
}
#endif
...
__init static int __mdio_init(struct net_mdio *mdio)
{
	...
	case NETC_EMDIO_t:
		rc = netc_emdio_init(mdio);
		break;
	case NETC_PORT_EMDIO_t:
		rc = netc_port_emdio_init(mdio);
		break;
	case ENET_t:
	case ENET_1G_t:
