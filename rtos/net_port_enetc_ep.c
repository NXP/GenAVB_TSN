...
#ifdef BOARD_ENETC0_TXRING_NUM
#define ENETC_EP0_TXRING_NUM BOARD_ENETC0_TXRING_NUM
#else
#define ENETC_EP0_TXRING_NUM 4U
#endif
#ifdef BOARD_ENETC1_TXRING_NUM
#define ENETC_EP1_TXRING_NUM BOARD_ENETC1_TXRING_NUM
#else
...
#ifdef BOARD_ENETC0_RXRING_NUM
#define ENETC_EP0_RXRING_NUM BOARD_ENETC0_RXRING_NUM
#else
#define ENETC_EP0_RXRING_NUM 4U
#endif
#ifdef BOARD_ENETC1_RXRING_NUM
#define ENETC_EP1_RXRING_NUM BOARD_ENETC1_RXRING_NUM
#else
...
#if (defined(BOARD_ENETC0_RX_ZERO_COPY) || defined(BOARD_ENETC1_RX_ZERO_COPY))
#if (ENETC_EP_BUFF_SIZE_ALIGN > NET_ALLOC_ALIGNMENT)
#error NET_ALLOC_ALIGNMENT too small
#endif
...
void netc_ep_pseudo_stats_init(struct net_port *port);
void netc_ep_stats_init(struct net_port *port);
...
static status_t enetc_ep_reclaim_cb(ep_handle_t *handle, uint8_t ring, netc_tx_frame_info_t *frameInfo, void *userData)
{
	...
	if (frameInfo) {
		...
		if (desc) {
			if (unlikely(frameInfo->isTsAvail)) {
				uint64_t cycles, ts;
...
static void *enetc_ep_rxbuff_alloc(ep_handle_t *handle, uint8_t ring, uint32_t length, void *userData)
{
	...
	buff = (uint8_t *)desc + NET_DATA_OFFSET;
...
static void enetc_ep_rxbuff_free(ep_handle_t *handle, uint8_t ring, void *buff, void *userData)
{
	...
	desc = (struct net_rx_desc *)((uint8_t *)buff - NET_DATA_OFFSET);
	net_rx_free(desc);
}
...
static struct net_rx_desc *enetc_ep_rx_frames_to_rx_desc(struct net_port *port, netc_frame_struct_t *rx_frame, uint8_t ring)
{
	...
	if (rx_frame->length == 1) {
		desc = (struct net_rx_desc *)((uint8_t *)rx_frame->buffArray[0].buffer - NET_DATA_OFFSET);
		...
		if (!desc) {
			...
			for (i = 0; i < rx_frame->length; i++)
				enetc_ep_rxbuff_free(&drv->handle, ring, rx_frame->buffArray[i].buffer, drv->handle.cfg.userData);
...
static void enetc_ep_tx_cleanup(struct net_port *port, uint32_t queue)
{
	struct enetc_ep_drv *drv = net_port_drv(port);

	EP_ReclaimTxDescriptor(&drv->handle, queue);
}
...
static int enetc_ep_set_admin_config(struct net_port *port, struct genavb_st_config *config)
{
	...
	netc_tb_tgs_entry_id_t entry_id = 0;
	int rc = GENAVB_SUCCESS;
	int i;
...
static int enetc_ep_psi_get_st_config(struct net_port *port, genavb_st_config_type_t type, struct genavb_st_config *config, unsigned int list_length)
{
	...
	time_gate_control_list.entryID = 0;
	time_gate_control_list.gcList = gate_control_entries;
...
__init static int enetc_ep_post_init(struct net_port *port)
{
	return 0;
}

__exit static void enetc_ep_pre_exit(struct net_port *port)
{
...
__init int enetc_ep_init(struct net_port *port)
{
	...
	if (port->drv_index == 0) {
#if (defined(BOARD_ENETC0_TS_FREERUNNING) && BOARD_ENETC0_TS_FREERUNNING)
		use_freerunning = true;
#endif
...
#if (CFG_NUM_ENETC_EP_MAC > 1)
#if (defined(BOARD_ENETC1_TS_FREERUNNING) && BOARD_ENETC1_TS_FREERUNNING)
		use_freerunning = true;
#endif
	...
	/* Use freerunning clock for timestamping, if configured and available */
	if (use_freerunning && netc_1588_freerunning_available(port->hw_clock_id)) {
		ep_config.port.ethMac.txTsSelect = kNETC_FreeRunningTime;
		ep_config.port.common.rxTsSelect = kNETC_FreeRunningTime;
	...
	ep_config.port.common.ipfCfg.enIPFTable = true;

#if CFG_NUM_NETC_SW
	/* Map all endpoints clocks to the bridge clocks */
	if (use_freerunning && netc_1588_freerunning_available(port->hw_clock_id)) {
		...
		/* Packet timestamping is still based on synchronized clock */
		clock_id_remap(port->clock[PORT_CLOCK_LOCAL], OS_CLOCK_GPTP_BR_0_0);
		clock_id_remap(port->clock[PORT_CLOCK_GPTP_0], OS_CLOCK_GPTP_BR_0_0);
		clock_id_remap(port->clock[PORT_CLOCK_GPTP_1], OS_CLOCK_GPTP_BR_0_1);
	}
#endif
...
#if defined(BOARD_NETC_VSI_PREINIT_FUNC)
	ep_config.preinitVsi = BOARD_NETC_VSI_PREINIT_FUNC;
#endif
...
#else
__init int enetc_ep_init(struct net_port *port) { return -1; }
