...
static uint32_t fiper_event_mask(uint8_t fiper_id)
{
	uint32_t mask;

	switch (fiper_id) {
	case kNETC_TimerFiper1:
	default:
		mask = ENETC_PF_TMR_TMR_TEVENT_PP1EN_MASK;
		break;

	case kNETC_TimerFiper2:
		mask = ENETC_PF_TMR_TMR_TEVENT_PP2EN_MASK;
		break;

	case kNETC_TimerFiper3:
		mask = ENETC_PF_TMR_TMR_TEVENT_PP3EN_MASK;
		break;
	}
...
static void netc_1588_timer_clear_fiper(netc_timer_handle_t *handle, uint8_t fiper_id)
{
	handle->hw.base->TMR_TEVENT |= fiper_event_mask(fiper_id);
}

static bool netc_1588_timer_check_fiper(netc_timer_handle_t *handle, uint8_t fiper_id)
{
	uint32_t mask = fiper_event_mask(fiper_id);

	return ((handle->hw.base->TMR_TEVENT & mask) == mask);
}

static void netc_1588_timer_reset_fiper(struct netc_hw_clock *hw_clock, uint8_t fiper_id, netc_timer_fiper_t *fiper)
{
	uint64_t ts;

	fiper->pulsePeriod = 4 * hw_clock->gclk_period;
	netc_1588_timer_start_fiper(&hw_clock->timer_handle, fiper_id, fiper);

	netc_1588_timer_clear_fiper(&hw_clock->timer_handle, fiper_id);

	ts = netc_1588_read_counter(hw_clock);
	netc_1588_timer_start_alarm(&hw_clock->timer_handle, kNETC_TimerAlarm1, ts + 5000);

	while (!netc_1588_timer_check_fiper(&hw_clock->timer_handle, fiper_id))
		;

	netc_1588_timer_stop_fiper(&hw_clock->timer_handle, fiper_id);
}
...
static int netc_1588_timer_set_periodic_event(struct hw_timer *hw_timer, uint64_t offset, uint64_t period)
{
	...
	if (hw_clock->offset_changed) {

		netc_1588_timer_reset_fiper(hw_clock, netc_timer_id[netc_timer->id], &fiper);

		hw_clock->offset_changed = false;
	}
 ...
 *
 */
__exit void __netc_1588_exit(unsigned int id)
{
 ...
 */
__init int __netc_1588_init(unsigned int id)
{
	struct netc_hw_clock *netc_hw_clock = &hw_clock[id];
	struct netc_1588_timer *timer;
	struct hw_timer *hw_timer;
	...
	*/
	...
	refclk = BOARD_NETC_HW_CLOCK_0_FREQ;
	...
	if (netc_hw_clock->owner) {
		...
		timer_config.clockSelect = BOARD_NETC_HW_CLOCK_0_SELECT;
		timer_config.refClkHz = refclk;
		timer_config.enableTimer = false;
...
__exit void netc_1588_exit(void)
{
	int i;
...
__init int netc_1588_init(void)
{
	int i;
...
#else
__init int netc_1588_init(void) { return 0; }
__exit void netc_1588_exit(void) {}
