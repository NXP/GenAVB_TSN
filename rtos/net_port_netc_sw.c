...
void netc_sw_stats_init(struct net_port *port);
...
static int netc_sw_set_priority_to_tc_map(struct net_port *port)
{
	...
	if (SWT_SetPortIPV2QMR(&drv->handle, port_idx, port->map) != kStatus_Success)
...
static int netc_sw_set_st_config(struct net_port *port, struct genavb_st_config *config)
{
	...
	rc = netc_sw_enable_st_config(port, config->enable);
	if (rc < 0) {
	...
	if (config->enable) {
		rc = netc_sw_set_admin_config(port, config);
		if (rc < 0) {
...
{
	swt_handle_t *handle = netc_sw_get_handle(port);
	struct netc_sw_drv *drv = net_port_drv(port);
	int rc = 0, i;
	...
	case GENAVB_FP_CONFIG_802_1Q:
		...
		for (i = 0; i < QOS_PRIORITY_MAX; i++) {
			...
			SWT_TxTcConfigPreemption(handle, port->base, port_priority_to_traffic_class(port, i), enPreemption);
		}
	...
	default:
		...
		SWT_TxPortEthMacConfigPreemption(handle, port->base, &mac_config);

		fp_set_enabled(drv, port->base, mac_config.preemptMode ? true : false);
...
static int netc_sw_get_fp(struct net_port *port, unsigned int type, struct genavb_fp_config *config)
{
	...
	case GENAVB_FP_CONFIG_802_1Q:
		...
		for (i = 0; i < QOS_PRIORITY_MAX; i++) {
			SWT_TxGetTcPreemption(handle, port->base, port_priority_to_traffic_class(port, i), &enabled);
			config->u.cfg_802_1Q.admin_status[i] = enabled ? GENAVB_FP_ADMIN_STATUS_PREEMPTABLE : GENAVB_FP_ADMIN_STATUS_EXPRESS;
		}

		NETC_PortGetTGSFPConfig(netc_sw_get_port_handle(port), &tg_config);
		NETC_PortGetPhyMacPreemptionControl(netc_sw_get_link_handle(port), &mac_config);
	...
	default:
		...
		SWT_TxPortGetEthMacPreemption(handle, port->base, &mac_config, &preempt_status);
...
int netc_sw_add_or_update_et_table_entry(struct netc_sw_drv *drv, uint32_t et_eid, void (*update_entry)(struct netc_sw_drv *drv, netc_tb_et_config_t *entry, void *data), void *data)
{
	...
	if (status == kStatus_NETC_NotFound) {
		...
		update_entry(drv, &ett_entry, data);
...
int netc_sw_delete_et_table_entry(struct netc_sw_drv *drv, uint32_t et_eid)
{
	...
	netc_si_free_fm_eid(drv, et_eid);
...
int netc_sw_vft_find_entry(struct netc_sw_drv *drv, uint16_t vid, uint32_t *entry_id, netc_tb_vf_cfge_t *cfge)
{
	...
	switch (SWT_BridgeQueryVFTableEntry(&drv->handle, &keye, &rsp)) {
	case kStatus_Success:
...
static int netc_sw_vft_update(struct net_port *port, uint16_t vid, bool dynamic, struct genavb_vlan_port_map *map)
{
	...
	rc = netc_sw_vft_find_entry(drv, vid, &entry_id, &config.cfge);
	if (rc < 0)
		goto err;
...
static int netc_sw_vft_delete(struct net_bridge *bridge, uint16_t vid, bool dynamic)
{
	...
	rc = netc_sw_vft_find_entry(drv, vid, &entry_id, &cfge);
	if (rc < 0)
		goto err;
	else if (!rc)
		goto done;
...
static int netc_sw_vft_read(struct net_bridge *bridge, uint16_t vid, bool *dynamic, struct genavb_vlan_port_map *map)
{
	...
	rc = netc_sw_vft_find_entry(drv, vid, &entry_id, &cfge);
	if (rc < 0) {
		rc = -GENAVB_ERR_VLAN_HW_CONFIG;
		goto err;
	} else if (!rc) {
		rc = -GENAVB_ERR_VLAN_ENTRY_NOT_FOUND;
		goto err;
	}
...
static int netc_sw_vft_dump(struct net_bridge *bridge, uint32_t *token, uint16_t *vid, bool *dynamic, struct genavb_vlan_port_map *map)
{
	...
	int rc = -GENAVB_ERR_VLAN_ENTRY_NOT_FOUND;
	netc_tb_vf_rsp_data_t rsp;
	int i, n;
...
static int netc_sw_fdb_find_entry(struct netc_sw_drv *drv, uint8_t *address, uint16_t vid, uint32_t *entry_id, netc_tb_fdb_cfge_t *cfge)
{
	...
	switch (SWT_BridgeQueryFDBTableEntry(&drv->handle, &keye, &rsp)) {
	case kStatus_Success:
		*entry_id = rsp.entryID;
...
static int netc_sw_fdb_update(struct net_port *port, uint8_t *address, uint16_t vid, bool dynamic, genavb_fdb_port_control_t control)
{
	...
	rc = netc_sw_fdb_find_entry(drv, address, vid, &entry_id, &config.cfge);
	if (rc < 0)
		goto err;
	...
	if (!rc) {
		...
		/* Always override dynamic entries */
		if (!dynamic)
			config.cfge.dynamic = dynamic;
	}
...
err:
	return -GENAVB_ERR_FDB_HW_CONFIG;
}
...
static int netc_sw_fdb_delete(struct net_bridge *bridge, uint8_t *address, uint16_t vid, bool dynamic)
{
	...
	rc = netc_sw_fdb_find_entry(drv, address, vid, &entry_id, &cfge);
	if (rc < 0)
		goto err;
	else if (!rc)
		goto done;
...
static int netc_sw_fdb_read(struct net_bridge *bridge, uint8_t *address, uint16_t vid, bool *dynamic, struct genavb_fdb_port_map *map, genavb_fdb_status_t *status)
{
	...
	rc = netc_sw_fdb_find_entry(drv, address, vid, &entry_id, &cfge);
	if (rc < 0) {
		rc = -GENAVB_ERR_FDB_HW_CONFIG;
		goto err;
	} else if (!rc) {
		rc = -GENAVB_ERR_FDB_ENTRY_NOT_FOUND;
		goto err;
	}
...
static int netc_sw_fdb_dump(struct net_bridge *bridge, uint32_t *token, uint8_t *address, uint16_t *vid, bool *dynamic, struct genavb_fdb_port_map *map, genavb_fdb_status_t *status)
{
	...
	netc_tb_fdb_rsp_data_t rsp;
	int i, n;
	int rc = -GENAVB_ERR_FDB_ENTRY_NOT_FOUND;
...
__init static int netc_sw_post_init(struct net_port *port)
{
	return 0;
}

__exit static void netc_sw_pre_exit(struct net_port *port)
{
...
__init int netc_sw_port_init(struct net_port *port)
{
	...
	if (netc_1588_freerunning_available(port->hw_clock_id)) {
		...
		clock_set_adjustment_flags(port->clock[PORT_CLOCK_GPTP_0], OS_CLOCK_FLAGS_HW_RATIO | OS_CLOCK_FLAGS_HW_OFFSET);
		clock_set_adjustment_flags(port->clock[PORT_CLOCK_GPTP_1], 0);
	}
...
__exit static void netc_sw_exit(struct net_bridge *bridge)
{
	...
	netc_sw_psfp_exit(&bridge->drv_ops.psfp);
...
__init static void port_remove_vlan_tag_egress_treatment(struct netc_sw_drv *drv, netc_tb_et_config_t *entry, void *data)
{
...
__init static void switch_port_init(struct net_port *port, swt_config_t *config, unsigned int bridge_port_id)
{
	...
	/* Map each priority to a unique buffer pool */
	config->ports[bridge_port_id].ipvToBP[0] = BP_DEFAULT_ID;
	config->ports[bridge_port_id].ipvToBP[1] = BP_DEFAULT_ID;
	config->ports[bridge_port_id].ipvToBP[2] = BP_DEFAULT_ID;
	config->ports[bridge_port_id].ipvToBP[3] = BP_DEFAULT_ID;
	config->ports[bridge_port_id].ipvToBP[4] = BP_DEFAULT_ID;
	config->ports[bridge_port_id].ipvToBP[5] = BP_DEFAULT_ID;
	config->ports[bridge_port_id].ipvToBP[6] = BP_DEFAULT_ID;
	config->ports[bridge_port_id].ipvToBP[7] = BP_DEFAULT_ID;
...
__init static void buffering_init(struct netc_sw_drv *drv, swt_config_t *swt_config)
{
	...
	/* Buffering scheme
	 * ================
	 * - i.MX RT1180 hardware:
	 *	 a) 8 buffer pools + 2 shared pools
	 *   b) One congestion group per traffic class and port
	 * - Configuration:
	 *   a) One buffer pool, common to all ports and priorities
	 *   b) All traffic class queues have the same minimal size, to support the
	 *      port standard MTU
	 *   c) Best Effort/Background traffic class queues have double the size
	 *   d) Buffer starvation is avoided by having the sum of all traffic class
	 *      queues sizes lower than the buffer pool size
	 */
	...
	buffer_pool_init(drv, BP_DEFAULT_ID, words, 0, 0);
...
__init int netc_sw_alloc(struct net_bridge *bridge)
{
	...
	drv->fm_table_entries = SWT_FMGetTableMaxEntryNum(&drv->handle);
	drv->fm_table = rtos_malloc(drv->fm_table_entries/sizeof(rtos_atomic_t)/8 + 1);
	if (!drv->fm_table) {
...
__init int netc_sw_init(struct net_bridge *bridge)
{
	...
	tx_rx_config.rxCacheMaintain = true;
#endif
#ifdef BOARD_NET_TX_CACHEABLE
	tx_rx_config.txCacheMaintain = true;
#endif
	...
	bridge->drv_ops.si.drv = &drv->si;
	drv->si.hw.handle = &drv->handle;
	drv->si.drv.sw = drv;
	drv->si.total_ports = CFG_NUM_NETC_SW_PORTS;
	drv->si.port = &drv->port[0];

	if (netc_si_init(&bridge->drv_ops.si) < 0) {
		os_log(LOG_ERR, "netc_si_init failed\n");
		goto err_si;
	}

	bridge->drv_ops.psfp.drv = &drv->psfp;
	drv->psfp.hw.handle = &drv->handle;
	drv->psfp.si = drv;

	if (netc_sw_psfp_init(&bridge->drv_ops.psfp) < 0) {
