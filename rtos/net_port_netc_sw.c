...
static int netc_sw_send_frame(struct net_bridge *bridge, unsigned int bridge_port, uint8_t *data, uint32_t length, struct net_tx_desc *desc, uint8_t priority, bool need_ts)
{
...
static int netc_sw_set_st_max_sdu(struct net_port *port, struct genavb_st_max_sdu *queue_max_sdu, unsigned int n)
{
	...
	netc_port_tc_sdu_config_t config_sdu = {0};
	...
	for (i = 0; i < n; i++) {
		...
		if (queue_max_sdu[i].queue_max_sdu) {
			...
			config_sdu.maxSduSized = NETC_TX_MAX_SDU;
...
__init int netc_sw_port_init(struct net_port *port)
{
	...
	if (netc_1588_freerunning_available(port->hw_clock_id)) {
		clock_to_hw_clock_set(port->clock_local, HW_CLOCK_PORT_BASE + port->hw_clock_id + 1);
		clock_to_hw_clock_set(port->clock_gptp[1], HW_CLOCK_PORT_BASE + port->hw_clock_id + 1);

		clock_set_flags(port->clock_gptp[0], OS_CLOCK_FLAGS_HW_RATIO | OS_CLOCK_FLAGS_HW_OFFSET);
		clock_set_flags(port->clock_gptp[1], 0);
	}
...
__init static void port_pcp_to_queue_init(struct net_port *port, swt_config_t *config, unsigned int bridge_port_id)
{
	...
	port_set_priority_to_traffic_class_map(port, QOS_TRAFFIC_CLASS_MAX, QOS_SR_CLASS_MAX);
...
__init int netc_sw_alloc(struct net_bridge *bridge)
{
	...
	size = netc_sw_flow_meter_get_max_entries(bridge) * sizeof(struct netc_sw_fm);

	dynamic = rtos_malloc(size);
	if (!dynamic)
		goto err;

	memset(dynamic, 0, size);

	drv->fm_table = (struct netc_sw_fm *)dynamic;
