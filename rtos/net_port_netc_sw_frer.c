...
int netc_sw_frer_get_eid(struct netc_sw_drv *sw_drv, uint32_t stream_handle, uint32_t *isqg_eid, uint32_t *et_eid, uint32_t *et_port_map)
{
...
static void netc_sw_et_update_esqa_params(struct netc_sw_drv *drv, netc_tb_et_config_t *entry, void *data)
{
...
{
	unsigned int p, seqr_index;
	int i;

	if (netc_sw_frer_stream_seqi_apply(drv, handle, NULL) < 0)
		goto err;

	for (i = 0; i < CFG_NUM_NETC_SW_PORTS; i++) {
		if (port_mask & (1 << i)) {
			p = __logical_port_get(i)->phys->base;
			if (netc_sw_sequence_recovery_get(drv, handle, p, &seqr_index)) {
				if (netc_sw_et_update_eseqr(drv, handle, seqr_index, p) < 0)
					goto err;
			}
		}
	}
