...
int netc_sw_frer_apply(struct netc_sw_drv *drv, uint32_t handle, uint32_t port_mask)
{
	...
	for (i = 0; i < CFG_NUM_NETC_SW_PORTS; i++) {
		if (port_mask & (1 << i)) {
			if (netc_sw_sequence_recovery_get(drv, handle, i, &seqr_index)) {
				if (netc_sw_et_update_eseqr(drv, handle, seqr_index, i) < 0)
					goto err;
			}
		}
	}
...
__init void netc_sw_frer_init(struct net_bridge *bridge)
{
