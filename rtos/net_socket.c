...
static inline void __port_get_addr(struct logical_port *port, unsigned char *addr)
{
...
void socket_set_option_event(struct net_set_option *option)
{
	...
	case SOCKET_OPTION_TC_MASK:
		port = __logical_port_get(sock->addr.port);

		option->val &= (1 << port->phys->num_rx_q) - 1;

		sock->rx.queue_mask = option->val;
	...
	if ((sock->rx.max_packets > 1) && (sock->rx.max_latency > 0)) {
		if (!(sock->flags & SOCKET_FLAGS_RX_POLLING)) {
			slist_add_head(&net_rx->polling_list, &sock->rx.polling_node);
			sock->flags |= SOCKET_FLAGS_RX_POLLING;
		}
	} else {
		if (sock->flags & SOCKET_FLAGS_RX_POLLING) {
			slist_del(&net_rx->polling_list, &sock->rx.polling_node);
			sock->flags &= ~SOCKET_FLAGS_RX_POLLING;
		}
	}
...
{
	struct logical_port *port = sock->tx.port;
...
static inline int socket_rx_ready(struct net_rx_ctx *net, struct net_socket *sock)
{
	...
	if ((net->time - sock->rx.time) >= sock->rx.max_latency)
		return 1;
...
void socket_poll_all(struct net_rx_ctx *net)
{
	...
	slist_for_each (&net->polling_list, entry) {
		sock = container_of(entry, struct net_socket, rx.polling_node);
...
int socket_tx_done(struct net_socket *sock, struct net_tx_desc **desc, unsigned int n)
{
	struct logical_port *port = sock->tx.port;
	int i;
...
#if CFG_BRIDGE_NUM
		bridge_tx_cleanup(&bridges[logical_port_bridge_id(port->id)]);
#endif
	} else {
		port_tx_cleanup_queue(port->phys, sock->tx.qos_queue->tc->hw_queue_id);
	}

	for (i = 0; i < n; i++) {
		if (!(desc[i]->flags & NET_TX_REUSE) || (desc[i]->flags & NET_TX_BUSY))
			break;
	}
...
static inline int socket_rx_sync(struct net_socket *sock, struct net_rx_desc **desc, int n, unsigned int len)
{
	struct net_rx_ctx *net = sock->rx.net_rx;
	struct logical_port *port = __logical_port_get(sock->addr.port);
	unsigned int rx_now = CFG_NET_RX_PACKETS;
	int queue;
	uint32_t qp = 0, read;
	int i = n;
	...
	for (; i < len; i++)
		desc[i] = (struct net_rx_desc *)queue_dequeue_next(&sock->rx.queue, &read);
...
int socket_read(struct net_socket *sock, struct net_rx_desc **desc, unsigned int n)
{
	int i = 0;
	uint32_t read;
	unsigned int n_now, pending;

	pending = queue_pending(&sock->rx.queue);
	n_now = genavb_min(pending, n);
	...
	if (!sock->callback && (i < n))
		i = socket_rx_sync(sock, desc, i, n);
...
int socket_qos_enqueue(struct net_socket *sock, struct net_tx_desc **desc, unsigned int n)
{
	u32 write;
	...
	for (i = 0; i < n; i++)
		queue_enqueue_next(&sock->tx.queue, &write, (unsigned long)desc[i]);
...
int socket_port_tx(struct net_socket *sock, struct net_tx_desc **desc_array, unsigned int n)
{
	struct logical_port *port = sock->tx.port;
	...
	return port_tx(port->phys, desc_array, n, sock->tx.qos_queue->tc->hw_queue_id);
}

int socket_bridge_port_tx(struct net_socket *sock, struct net_tx_desc **desc, unsigned int n)
{
	struct logical_port *port = sock->tx.port;
	int i;

	for (i = 0; i < n; i++) {
		if (bridge_port_tx(port->phys, sock->addr.priority, desc[i]) < 0)
			break;
	}
...
static int socket_tx_connected(struct net_socket *sock, struct net_tx_desc **desc_array, unsigned int n)
{
	struct logical_port *port = sock->tx.port;
	struct net_tx_desc *desc;
	struct eth_hdr *eth;
	...
	return sock->tx.func(sock, desc_array, n);
}
...
int socket_write(struct net_socket *sock, struct net_tx_desc **desc, unsigned int n)
{
	if (likely(sock->flags & SOCKET_FLAGS_CONNECTED))
		return socket_tx_connected(sock, desc, n);
	else
		return 0;
}
...
int socket_set_callback(struct net_socket *sock, void (*callback)(void *data), void *data)
{
	struct event e;
...
void socket_close(struct net_socket *sock)
{
	if (sock->flags & SOCKET_FLAGS_RX) {
		...
		if (sock->flags & SOCKET_FLAGS_RX_POLLING) {
			slist_del(&sock->rx.net_rx->polling_list, &sock->rx.polling_node);
			sock->flags &= ~SOCKET_FLAGS_RX_POLLING;
		}
	} else {
		socket_disconnect(sock);
