...
#define SOCKET_OPTION_TC_MASK		3
...
struct net_socket {
	...
	union {
		struct {
			...
			struct queue queue;
			int (*func)(struct net_socket *sock, struct net_tx_desc **desc, unsigned int n);
			uint16_t vlan_label;
			uint32_t flags;
...
int socket_port_tx(struct net_socket *sock, struct net_tx_desc **desc, unsigned int n);
...
static inline uint64_t socket_time_from_hw(struct net_socket *sock, struct logical_port *port, uint64_t ts)
{
	...
	clock_time_from_hw_any(port->phys->clock[sock->clock], port->phys->hw_clock, ts, &socket_ts);
