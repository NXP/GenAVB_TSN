...
static inline bool net_rx_skip(void)
{
#if defined(CFG_NET_RX_PERIOD_MULT) && (CFG_NET_RX_PERIOD_MULT > 1)
	static int n = 0;

	n++;
	if (n < CFG_NET_RX_PERIOD_MULT)
		return true;
	...
	for (;;) {
		...
		case EVENT_RX_TIMER:
			...
			for (i = 0; i < CFG_PORTS; i++) {
				...
				for (queue = port->num_rx_q - 1, rx_now = CFG_NET_RX_PACKETS; queue >= 0 && rx_now; queue--)
