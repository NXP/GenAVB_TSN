...
static int sr_class_tx(struct port_qos *port, struct traffic_class *tc, struct qos_queue *qos_q)
{
	...
	if (port_tx(port->net_port, &desc, 1, tc->hw_queue_id) < 1) {
...
static inline void qos_queue_flush_disabled(struct traffic_class *tc)
{
...
static inline void sr_class_update(struct traffic_class *tc, unsigned int tnow)
{
	struct sr_class *class = tc->sr_class;
	struct qos_queue *qos_q;
...
static inline void traffic_class_update(struct traffic_class *tc)
{
	struct qos_queue *qos_q;
	unsigned long mask;
	int i;
...
static int net_qos_map_traffic_class_to_hw_queues(struct port_qos *port)
{
	...
	unsigned int tc_num[CFG_TRAFFIC_CLASS_MAX] = {0, };
	struct tx_queue_properties tx_q_cfg;
	struct traffic_class *tc;
	int i, j;

	os_log(LOG_INFO, "port(%u) num tc: %u, num sr: %u, num hw queues: %u\n",
	       port->net_port->index, port->traffic_class_max, port->sr_class_max, tx_q_cap->num_queues);

	/*
	 * Try to assign each traffic class to a dedicated hardware queue, starting with higher
	 * priority traffic classes. When not possible, assign to same hardware queue.
	 * Traffic classes mapped to dedicated hardware queue can use hardware packet scheduling.
	 * SR traffic classes always use software scheduling, but can use hardware CBS when mapped
	 * to a dedicated hardware queue (that supports CBS).
	 */

	if (tx_q_cap->num_queues > port->traffic_class_max)
		tx_q_cfg.num_queues = port->traffic_class_max;
	else
		tx_q_cfg.num_queues = tx_q_cap->num_queues;

	/* Assign traffic classes to hardware queues, top to bottom */
	/* Lower traffic classes may be assigned to same hardware queue */
	for (i = port->traffic_class_max - 1, j = tx_q_cfg.num_queues - 1; i >= 0; i--) {
		tc = &port->traffic_class[i];

		tc->hw_queue_id = j;
		tc->flags = 0;

		tx_q_cfg.queue_prop[j] = tx_q_cap->queue_prop[j];

		/* Track number of traffic classes assigned to an hardware queue */
		tc_num[j]++;
	...
	for (i = 0; i < port->traffic_class_max; i++) {
		...
		/* Check if traffic class has a dedicated hardware queue */
		if (tc_num[tc->hw_queue_id] == 1)
			tc->flags |= TC_FLAGS_HW_SP;

		if (tc->sr_class) {
			if ((tc->flags & TC_FLAGS_HW_SP) && (tx_q_cfg.queue_prop[tc->hw_queue_id] & TX_QUEUE_FLAGS_CREDIT_SHAPER))
				tc->flags |= TC_FLAGS_HW_CBS;
			else
				tx_q_cfg.queue_prop[tc->hw_queue_id] &= ~TX_QUEUE_FLAGS_CREDIT_SHAPER;

			/* Sr classes are always scheduled in software */
			tc->flags &= ~TC_FLAGS_HW_SP;
		} else {
			tx_q_cfg.queue_prop[tc->hw_queue_id] &= ~TX_QUEUE_FLAGS_CREDIT_SHAPER;
		}
...
__init static int net_qos_port_init(struct net_qos *net, struct port_qos *port)
{
	...
	port->traffic_class_max = port->net_port->traffic_class_max;
	port->sr_class_max = port->net_port->sr_class_max;
