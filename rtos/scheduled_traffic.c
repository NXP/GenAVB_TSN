...
static int __st_set_admin_config(struct logical_port *port, os_clock_id_t clk_id,
			    struct genavb_st_config *config)
{
	...
	if (!config->cycle_time_p || !config->cycle_time_q) {
		rc = -GENAVB_ERR_ST_INVALID_CYCLE_PARAMS;
		goto err;
	}
	...
	if (((uint64_t)cycle_time * config->cycle_time_q) !=
		...
		rc = -GENAVB_ERR_ST_INVALID_CYCLE_TIME;
		goto err;
	}
	...
	if (clock_time_to_cycles(clk_id, config->base_time, &config->base_time) < 0) {
		...
		rc = -GENAVB_ERR_ST_INVALID_BASETIME;
		goto err;
	}
			    ...
			    struct genavb_st_config *config)
{
	...
	rc = __st_set_admin_config(port, clk_id, config);
...
static int __st_get_config(struct logical_port *port, genavb_st_config_type_t type,
		      struct genavb_st_config *config, unsigned int list_length)
{
	int rc = GENAVB_SUCCESS;

	rc = port_get_st_config(port->phys, type, config, list_length);
	if (rc < 0)
		goto err;
		      ...
		      struct genavb_st_config *config, unsigned int list_length)
{
	...
	rc = __st_get_config(port, type, config, list_length);
...
static int st_update(unsigned int port_id, os_clock_id_t clk_id)
{
	...
	if (port->phys->st_enabled && (clock_id(port->phys->clock_st_psfp) == clock_id(clk_id))) {
		...
		rc = __st_get_config(port, GENAVB_ST_OPER, &config, max_entries);
		if (rc < 0)
			goto err_get_st_config;

		if (clock_has_hw_setoffset(clk_id) && config.enable) {
			config.enable = 0;
			__st_set_admin_config(port, clk_id, &config);
			config.enable = 1;
		}
