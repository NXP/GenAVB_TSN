...
#if defined(CONFIG_GENAVB_TSN_AVTP) && (CFG_NUM_TPM > 0) && defined(BOARD_TPM_REC_BASE)
...
struct tpm_rec {
	...
	struct tpm_input_capture capture; /* Hardware based sampling of PLL counter / gPTP timestamps using the TPM Input Capture feature. */
...
#if !defined(BOARD_TPM_REC_CHANNEL) && defined(BOARD_TPM_REC_SW_SAMPLING)
	.sw_sampling = true,
#else
	.sw_sampling = false,
...
#endif /* !defined(BOARD_TPM_REC_CHANNEL) && defined(BOARD_TPM_REC_SW_SAMPLING) */
};
...
static int tpm_rec_start(struct mclock_dev *dev, struct mclock_start *start)
{
	...
	if (rec->c.is_hw_recovery_mode)
		...
		rc = mclock_register_timer(dev, &tpm_sw_rec_timer_irq, 1);
#endif
...
__init int tpm_rec_init(void)
{
	...
	struct mclock_dev *clock_dev = &tpm_rec.rec.c.dev;
	unsigned int input_clk_rate, divider;

	input_clk_rate = BOARD_TPM_clk_freq(tpm_rec.base);
	divider = (1 << ((((TPM_Type *)tpm_rec.base)->SC & TPM_SC_PS_MASK) >> TPM_SC_PS_SHIFT));

	if (!tpm_rec.sw_sampling) {
	...
	rec->c.pll_ref_freq = input_clk_rate / divider;
	rec->c.pll_timer_clk_div = BOARD_TPM_clk_src_div(tpm_rec.base);
	...
	os_log(LOG_INIT, "TPM %s rec device(%p), clk input: %ld Hz, pll frequency %lu divided by %u, pll parent (osc) frequency %lu\n",
		tpm_rec.sw_sampling ? "Software" : "Hardware", clock_dev, input_clk_rate,
...
__exit void tpm_rec_exit(void)
{
	...
	imx_pll_deinit(&tpm_rec.rec.c.pll);
}
#else
