 ...
 */
static void talker_update_declaration(struct msrp_stream *stream, struct msrp_port *port, msrp_talker_declaration_type_t new_declaration_type, u8 failure_code, bool new)
{
	struct msrp_ctx *msrp = container_of(port, struct msrp_ctx, port[port->port_id]);
	struct mrp_attribute *prev_attr = stream->port[port->port_id].declared_talker_attribute;
